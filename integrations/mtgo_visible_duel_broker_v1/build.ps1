...
$liveTwoLocalBrokerOut = Join-Path $build 'mtgo_visible_duel_live_two_local_broker_v1.exe'
...
$liveTwoLocalBrokerObject = Join-Path $build 'VisibleDuelLiveTwoLocalBrokerV1.obj'
...
$liveTwoLocalBrokerCommand = "call `"$vcvars`" >nul && cl /nologo /std:c++20 /permissive- /W4 /WX /EHsc /guard:cf /GS /ZH:SHA_256 /DUNICODE /D_UNICODE /DMTGO_LIVE_PINNED_V1 /DMTGO_LIVE_PINNED_TWO_LOCAL_CLIENTS_V1 /Fo`"$liveTwoLocalBrokerObject`" `"$brokerSource`" /link /Brepro /OUT:`"$liveTwoLocalBrokerOut`" bcrypt.lib version.lib wintrust.lib"
& cmd.exe /d /c $liveTwoLocalBrokerCommand
if ($LASTEXITCODE -ne 0) {
    throw "native live two-local-client broker build failed"
}
...
$expectedLiveTwoLocalBrokerSha256 = '6281797239d1827fab4d3767a2273f86ee8b3e1b290ddebbd7d65f75878954b3'
...
$observedLiveTwoLocalBrokerSha256 = (Get-FileHash -LiteralPath $liveTwoLocalBrokerOut -Algorithm SHA256).Hash.ToLowerInvariant()
...
if ($observedLiveTwoLocalBrokerSha256 -ne $expectedLiveTwoLocalBrokerSha256) {
    throw "native live two-local-client observe-only broker differs from its release pin"
}
