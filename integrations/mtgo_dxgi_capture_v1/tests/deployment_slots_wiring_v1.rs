...
fn placeholder_deployment_drives_every_surface_without_authority() {
    ...
    let pregame_selection = score_checked_untrusted_competitive_native_pregame_v1(
        &pregame,
        &deployment,
        &mut slots.pregame_controller,
    )
    .unwrap();
    assert_eq!(
        pregame_selection.selected_action_v1(),
        &MtgoCompetitiveNativePregameActionV1::KeepOpeningHand
    );
    assert!(!pregame_selection.safe_for_live_input_v1());

    let sideboard = MtgoCompetitiveNativeSideboardModelInputV1 {
        next_game_number: 2,
        acting_player_games_won: 1,
        opponent_games_won: 0,
        current_configuration: deck_v1(),
    };
    let sideboard_selection = score_checked_untrusted_competitive_native_sideboard_v1(
        &sideboard,
        &deployment,
        &mut slots.sideboard_controller,
    )
    .unwrap();
    assert_eq!(
        sideboard_selection.selection_v1().target_configuration,
        deck_v1()
    );
    ...
    assert_eq!(
        slots.search_root_provider.search_root_v1(&duel),
    ...
    assert_eq!(
        slots
            .duel_scorer
            .score_player_visible_duel_v1(&duel)
            .unwrap_err(),
        MTGO_VISIBLE_KERNEL_SCORER_NOT_QUALIFIED_REASON_V1
    );
    assert_eq!(
        slots.unknown_card_policy,
