...
`verify_duel_perception_runtime_v1` and `perceive_admitted_duel_frame_v1` continue that opaque chain through an exact profile-pinned classifier executable. The runtime hashes the classifier binary, classifier-assets manifest, and card-database profile before and after each bounded child-process invocation. Pixels cross only the private stdin protocol. The strict response echoes the exact request commitment and contains both the observed decision and its complete current visible-control set. The decision must match the exact frame identity and geometry, every root pixel region is recomputed from the retained BGRA8 bytes, accessibility and manual-annotation evidence are rejected, every legal action belongs to a measured profile family, and the existing full decision validator passes. Every legal action must map one-to-one to an enabled current-frame control. `score_and_select_opaque_admitted_duel_perception_v1` then retains that exact source and decision through the existing model scorer, and `resolve_opaque_profile_bound_duel_control_v1` applies the existing unique-control resolver while retaining the chosen region privately. All wrappers remain move-only and expose commitments only, with no pixel, observation, selected-semantic, coordinate, input, or event-entry conversion.

`bind_opaque_duel_control_to_competitive_action_plan_v1` joins that Windows-retained control to the separately checked League or Challenge gameplay scope and calibrated visible postcondition plan. It requires exact equality for the decision, model selection, control resolution, perception admission, deployment, selected semantic, control identity, source frame and sequence, serialized capture manifest, raw frame hash, output identity, and client geometry. The result retains both move-only inputs and exposes commitments only. It still has no coordinate, input, purchase, or event-entry conversion. This closes the copied-commitment substitution gap before the future immediate-recapture actuator boundary.
