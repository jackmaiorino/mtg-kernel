...
`build_pregame_action_plan_v3` consumes that opaque selection and binds it to the reviewed 1550 by 925 Solitaire control layout, the exact source capture, and the observed pixels of the selected control region. The target rectangle and point remain private. A planned Mulligan requires the next current ladder prompt on a strictly newer changed frame with the same process, match-window, and output identity. A seven-card Keep can be confirmed by the exact Turn 1 first-main profile shared by two reviewed games with different visible hand sizes. A one-card Keep can be confirmed by `measure_mtgo_dxgi_bottom_six_initial_candidate_v3`, which now uses the current binary-control state gate and requires selected count zero. Keeps at prospective sizes two through six remain blocked until their required bottom counts are separately calibrated. Zero-card Mulligan is also rejected until its prompt is calibrated.
