...
`verify_duel_perception_runtime_v1` and `perceive_admitted_duel_frame_v1` continue that opaque chain through an exact profile-pinned classifier executable. The runtime hashes the classifier binary, classifier-assets manifest, and card-database profile before and after each bounded child-process invocation. Pixels cross only the private stdin protocol. The returned observed decision must match the exact frame identity and geometry, every root pixel region is recomputed from the retained BGRA8 bytes, accessibility and manual-annotation evidence are rejected, every legal action belongs to a measured profile family, and the existing full decision validator passes. `score_and_select_opaque_admitted_duel_perception_v1` then retains that exact source and decision through the existing model scorer. All three wrappers remain move-only and expose commitments only, with no pixel, observation, selected-semantic, coordinate, input, or event-entry conversion.
