...
Production authorization ratification is deliberately `None`. Runtime booleans and arbitrary permission hashes cannot enable the actuator. The current actuator remains private-match-only. On 2026-08-10, the account owner reported equivalent Daybreak approval for League and Challenge play on the approved main account under the same visible-only and no-cheating condition. A later reviewed authorization token may therefore cover those two modes only when it binds the exact account alias and exact private correspondence bytes. That approval does not itself enter an event, and the current code has no queue-entry or event-entry path. Other prize-event modes remain disabled unless separately confirmed. No input was sent while implementing or testing this path.
