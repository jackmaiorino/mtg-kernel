...
`prepare_opaque_competitive_duel_pass_actuation_v1` implements the first part of that immediate-recapture boundary for priority Pass only. It acquires a strictly newer opaque DXGI frame, reruns the exact admitted classifier, requires the semantic decision payload to be unchanged, requires the same unique Pass control with identical geometry and pixels, and requires the same process incarnation, window, client geometry, and output. It retains the fresh frame and private target point but exposes commitments only and cannot send input. The preparation now carries a commitment to the complete authorization document and exactly one selected League or Challenge mode. A separate competitive Pass ratifier requires the exact visible account alias, correspondence hash, selected mode, and priority-Pass-only gesture scope. `bind_prepared_competitive_duel_pass_authorization_v1` consumes both opaque values and rejects any mode-commitment mismatch, but its result still has no input conversion. Other action families remain blocked until their exact click, double-click, drag, or multi-stage gesture is separately calibrated.

This is wiring, not a usable live classifier. The production perception-profile ratification root remains empty, no reviewed classifier executable implementing the protocol has been installed, and no live input path consumes these types.
...
Production authorization ratification is deliberately `None` for both the private-match actuator and the new competitive Pass boundary. Runtime booleans and arbitrary permission hashes cannot enable either path. On 2026-08-10, the account owner reported equivalent Daybreak approval for League and Challenge play on the approved main account under the same visible-only and no-cheating condition. A later reviewed ratification may cover either League or Challenge priority Pass, one mode at a time, only when it binds the exact account alias and exact private correspondence bytes. Combined League-plus-Challenge scopes, private or open-play authority, shadow authority, and other prize-event authority are rejected at this ratification boundary. That approval does not itself enter an event, and the current code has no queue-entry or event-entry path. Other prize-event modes remain disabled unless separately confirmed. No input was sent while implementing or testing this path.
