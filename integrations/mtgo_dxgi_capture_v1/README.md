...
The preferred pregame path now preserves that profile without using the downgrade. Four opaque stages carry the exact profile commitment through prompt measurement, complete seven-card identity measurement, card-aware scoring, and coordinate-private action planning. Each stage consumes the previous unforgeable value, rechecks the compiled profile commitment, retains pixels and templates privately, and has no generic-selection, actuator, coordinate, purchase, or queue-entry conversion. This is a source-provenance chain, not semantic admission: every evidence, observation, trusted-policy, and live-input flag remains false.
