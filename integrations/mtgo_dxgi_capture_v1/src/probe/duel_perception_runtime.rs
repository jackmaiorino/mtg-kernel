...
use mtgo_blackbox_v1::{
    duel_action_family_v1, model_deployment_commitment_v1, score_and_select_external_model_v1,
    validate_observed_decision_v1, visible_frame_region_content_sha256_v1,
    AdmittedMtgoDuelPerceptionProfileV1, CheckedUntrustedMtgoModelSelectionV1,
    MtgoEvidenceSourceV1, MtgoExpectedModelDeploymentV1, MtgoExternalObservationScorerV1,
    MtgoObservedDecisionV1, MtgoSizePxV1, ValidatedMtgoObservedDecisionV1,
};
use serde::Serialize;
...
pub fn perceive_admitted_duel_frame_v1(
    ...
    verify_runtime_identity_now_v1(runtime)?;
    let record: MtgoObservedDecisionV1 = serde_json::from_slice(&response).map_err(|error| {
        format!("perception response is not one strict observed-decision JSON value: {error}")
    })?;
