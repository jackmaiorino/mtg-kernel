...
compile_error!("qualify_mtgo_direct_visible_source_v1 is Windows-only");

use mtgo_dxgi_capture_v1::{
    qualify_attested_direct_visible_source_current_duel_v1, verify_direct_visible_source_runtime_v1,
};
use serde::Serialize;
use std::path::Path;
...
fn main() {
    if let Err(error) = run() {
        eprintln!("MTGO_DIRECT_VISIBLE_SOURCE_QUALIFICATION_REJECTED:{error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let status = if abstention_reason.is_some() {
        ...
        "visible_decision_candidate_retained_privately"
    };
    let summary = QualificationSummaryV1 {
        schema: "mtgo-direct-visible-source-qualification-summary/v1",
        status,
        abstention_reason,
