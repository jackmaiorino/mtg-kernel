...
The classifier binary and runtime now implement an exact three-state deck-gate protocol. A target is structurally eligible only when its asset manifest carries one reviewed profile for each state. State profiles bind event OCR, exact missing-deck prompt state, exact enabled Select Deck or Change Deck pixels, and exact Open Entry Review available or unavailable pixels. The selected deck name wraps across two lines, and the fixed-label offline OCR probe found zero exact deck-name matches in the compatible-deck frame. Selected states therefore bind a separately reviewed exact deck-label pixel region to the target deck identity. An opt-in regression rehashes the retained natural League missing-deck and compatible-deck manifests, PNGs, and decoded BGRA8 frames, runs real Windows OCR, and requires exactly one match for each of the first two states. Ambiguous, incomplete, substituted, or region-drifted profiles reject. The Open Entry Review state remains uncaptured because reaching it requires an entry-option choice, so the reviewed League and Challenge three-state deck-gate corpus is still missing and production deck-gate classifier readiness remains false.

The same pinned binary and runtime now implement a distinct exact two-state deck-chooser protocol for the dialog reached from that missing-deck gate. A fresh Modern League transaction on MTGO 3.4.158.4691 established the natural state rule. The fresh `Modern Decks` dialog had an unselected exact deck tile, empty detail pane, and visibly disabled Submit. Selecting the exact deck changed the tile, populated `MAIN DECK (60)`, and visibly enabled Submit. The fixed-label offline OCR probe found the exact title but could not reliably recognize the wrapped two-line deck name. The protocol therefore binds the original gate, account, event, expected deck hash, complete newer frame, process/window continuity, exact title OCR, a reviewed exact deck-label pixel region inside the row, exact selected or unselected row pixels, the corresponding populated or empty detail pane, and enabled or disabled Submit pixels. The checked-in review receipt binds the exact natural League pair, a restored Challenge selected corroboration frame with identical selected-region commitments, and a stale artificial Challenge negative whose enabled Submit makes it match neither state. It emits separate League and Challenge profile pairs from the same event-independent dialog references; the prior deck gate, not the dialog, binds event identity. The opt-in external-frame test rehashes all source pixels, runs real Windows OCR, requires the natural League frames and restored Challenge selected frame to match exactly one state, and requires the stale frame to match none. The result is move-only and non-authorizing. The dormant actuator consumes that ownership in three stages: `Select Deck`, exact deck row, and `Submit`. It withholds all further MTGO input after each click until the exact newer visible state is confirmed. The corpus grants only classifier-reference generation, and the compile-pinned actuator authority root remains empty.

The external League pending-review capture SHA-256s are `1865154A3B84EC0DBB97720D0133DE6C21FA110D4BA48076562EA467B1D9EBC0` for the missing-deck main page, `B8109095B9239AA949E7E8FA2CF5A0A4E2D701BEB85DDF347E312B33F828C3C8` for the fresh unselected chooser, `B339E16B46B47B56ED1060C7F7975CE720FCEB55527697EC054FA3155D0C5C41` for the selected chooser, and `156B0687EA48A749AFDD748C9307B029ED38942B5AAF73680F1DF45850B09CE6` for the compatible-deck main page after Submit. They remain outside the repository and are not admitted classifier assets. Ignored opt-in tests over the exact main-page and chooser pairs passed on 2026-08-14. The real Windows OCR path found the bounded event and chooser titles, the corrected exact-region classifiers matched exactly one natural state per frame, and the selected main page succeeded despite zero exact deck-name OCR matches. This confirms the wrapped-label correction against the retained frames but does not promote the frames or enable either actuator. The ticket and Play Point options remained untouched, and no fee, Open Entry Review, purchase, event entry, or match was invoked.

A later signed-in read-only probe confirmed that visible Windows accessibility is a substantially better source for this static pre-entry surface than the built-in OCR engine. The exact query returned one visible match each for `Change Deck`, `Choose Entry Option:`, `100 Play Points`, `mtgo-kernel-modern-basics-v1`, and `10 Event Tickets`; two for `Modern League`; and zero for `Open Entry Review`. Raw accessibility names, rectangles, control metadata, and unmatched values were not emitted. The stronger two-frame composed-pixel bracket failed closed when Desktop Duplication timed out on the static frame. The probe was then tightened so every exact match must be the same topmost UI Automation element returned by a center-point hit test inside the admitted foreground client. The live result remained identical with report commitment `ac9406307b9e97b4a950a64ca8476ab855f53e0c1b99b6bcd04e5f0b4375c070`. A narrow reviewed classifier may now consume only those fixed visible entry labels and export one bounded deck-gate state. It still exposes no arbitrary text, control state, coordinates, policy input, entry authority, spending authority, or input method. The generic accessibility catalog remains diagnostic and still requires pixel corroboration for broader semantic use.
