...
A later signed-in read-only probe confirmed that visible Windows accessibility is a substantially better source for this static pre-entry surface than the built-in OCR engine. The exact query returned one visible match each for `Change Deck`, `Choose Entry Option:`, `100 Play Points`, `mtgo-kernel-modern-basics-v1`, and `10 Event Tickets`; two for `Modern League`; and zero for `Open Entry Review`. Raw accessibility names, rectangles, control metadata, and unmatched values were not emitted. The stronger two-frame composed-pixel bracket failed closed when Desktop Duplication timed out on the static frame, so these counts remain diagnostic only and are not semantic evidence, policy input, or control authority. This supports a future visible-accessibility acquisition path, but only after same-frame visible-pixel corroboration or an equivalently reviewed visible projection closes that source gap.
