...
The same pinned binary and runtime now implement a distinct exact two-state deck-chooser protocol for the dialog reached from that missing-deck gate. A later live check on MTGO 3.4.158.4691 corrected the state rule: the visible Submit control is present in both states and does not distinguish them. The protocol binds the original gate, account, event, expected deck, complete newer frame, process/window continuity, exact `Modern Decks` title and deck-label OCR, exact selected or unselected row pixels, the corresponding populated or empty detail pane, and shared visible Submit pixels. Both reviewed state profiles are required and exactly one must match. The result is move-only and non-authorizing. The dormant actuator consumes that ownership in three stages: `Select Deck`, exact deck row, and `Submit`. It withholds all further MTGO input after each click until the exact newer visible state is confirmed. No reviewed League or Challenge chooser corpus exists, and the compile-pinned actuator authority root is empty.

The external pending-review captures for this correction are SHA-256 `1FDA5D118DA4E3CD34758402E696A0D1F86735C740D5146B9C68C36B601B1892` for the true unselected state and `8661B4BA4C592F87F58D32DF8634FF70768C1CD9DA1897A6F3F5228CF745E146` for the true selected state. They remain outside the repository and are not admitted classifier assets. Invoking the exact visible Submit returned to the same pre-entry event page with the expected deck selected. No entry choice, fee, Open Entry Review, purchase, event entry, or match was invoked.
