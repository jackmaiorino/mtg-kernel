...
| Match lifecycle | Spectator and Solitaire windows were identified and Solitaire setup was navigated under supervision. | Early calibration | Deck selection, queue entry, match acceptance, game transitions, sideboarding, result handling, reconnect recovery, and safe exit. |
| League and Challenge lifecycle | Runtime modes and separate authorization flags exist. | Not implemented | Version-specific visible navigation state machines, entry confirmation, event status, pairing, match launch, standings, and terminal event handling. No purchase or entry action may be inferred from gameplay authorization. |
| Verification | The isolated adapter suite passes 81 tests, including one composed shadow loop from validated decision through scoring, visible-control resolution, offline submission, and postcondition confirmation, plus compile-fail checks that prevent pixel, context, score, coordinate, and input authority escapes. | Green for current offline scope | Live shadow-match corpus, perception accuracy gates, full semantic coverage, fault injection, timer tests, reconnect tests, and private-match end-to-end rehearsal before any prize mode. |
