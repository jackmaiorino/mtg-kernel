...
| Verification | The isolated adapter suite passes 80 tests, including compile-fail checks that prevent pixel, context, score, coordinate, and input authority escapes. | Green for current offline scope | Live shadow-match corpus, perception accuracy gates, full semantic coverage, fault injection, timer tests, reconnect tests, and private-match end-to-end rehearsal before any prize mode. |
