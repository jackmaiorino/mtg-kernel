...
| Visible capture | The standalone DXGI Desktop Duplication backend acquired a real foreground MTGO client crop, rejected pointer-only updates, verified a current desktop presentation and no protected-content masking, removed staging padding, and emitted separately hashed BGRA8, PNG, and manifest files. The adapter strictly checks the complete artifact and proves PNG-to-BGRA identity. | Candidate implemented, not runtime-ready | Ratified profile and anchors, trusted opaque live-frame admission, and closure or explicit operational treatment of the transient-occluder race. |
...
| Verification | The isolated adapter suite passes 99 tests, including strict DXGI artifact tamper tests, composed shadow gameplay, a complete structural League lifecycle, Challenge authorization separation, exact entry-resource gating, reconnect handling, and compile-fail checks that prevent pixel, context, score, coordinate, lifecycle, and input authority escapes. The separate capture crate passes five tests and clean clippy. | Green for current offline and checked-untrusted capture scope | Live shadow-match corpus, perception accuracy gates, full semantic coverage, fault injection, timer tests, reconnect tests, and private-match end-to-end rehearsal before any prize mode. |
