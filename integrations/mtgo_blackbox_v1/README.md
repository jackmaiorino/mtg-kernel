...
The sibling `integrations/mtgo_visible_duel_producer_v1` assembly supplies the in-process producer seam. Version 1.16 locates exactly one public WPF `DuelScene`, requires its `DataContext` to be the exact duel view-model type, and verifies the complete 77-getter public allowlist plus thirty-two private visible-source-bound joins against the pinned loaded presentation assemblies. Its sanitized success slice covers bounded empty-stack noncombat priority decisions during upkeep, draw, Main 1, Main 2, and the end step. It exports visible turn, active and priority roles, life, mana, hand and library counts, battlefield, graveyards, player-relative exile panels, attachments, own hand, and simple source-bound actions. The turn is parsed only from the rendered English-client `GameTurnText`; an unexpected locale or format abstains. A face-down exiled card name remains withheld. The producer derives an Initiative-holder candidate only from the rendered per-player Shields zone and each rendered card's public `CardFrameID`; any non-null, ambiguous, or unknown visible emblem still forces abstention until an exact live UI corpus qualifies the player association. Visible poison, energy, experience, or radiation badges, Companion, pile and card selectors, trigger panels, wish flow, storm display, temporary-zone windows, Commander, Planechase, dungeon, Ring-temptation, and speed presentation all force the same generic abstention until represented. Static inspection of `Card_View.ProcessMouseUp` confirms the visible card menu transforms alternate-menu, grouped, action-choice, mode-choice, submenu, and attack-hover entries. Static inspection of `PromptBoxViewModel` confirms that ordinary local priority sets the prompt box active and produces a default `OK` action, while numeric, mana-payment, and target flows expose additional visible prompt state. Version 1.16 therefore admits Pass only when the sole visible enabled standard control is the default `OK` action with the exact `OK` flag, numeric entry is disabled, and no mana button is present. It uses action flags and the related target, X, confirmation, sideboard, fake-action, and mode-count values only as one-way guards. Any unsupported value causes the same generic abstention. It never exports the private values, client objects, or internal identifiers. It never enumerates either library, an opponent's hidden hand or action collection, or a closed revealed zone, and never reads a face-down exiled card name. The synthetic WPF fixture validates the original opening, upkeep, draw, and end-step priority, plus a Turn 4 Main 2 state with changed life, mana, hand and library counts and populated battlefield, graveyard, and exile zones, while throwing getters prove prohibited paths remain untouched. It separately proves an inactive prompt box, an additional enabled choice, a renamed default control, a numeric-prompt action flag, enabled numeric entry, and visible mana-payment control all abstain. The emitted decisions pass the strict Rust producer-result validator. The pinned live broker has loaded only the prior 46-getter version 1.5 build outside a duel. Version 1.16 has not been loaded into MTGO, the Pass control has not been confirmed in a real duel, and combat, stack items, revealed windows, Initiative, and modal decisions remain unsupported. Complete live semantic evidence, model scoring, and input therefore remain absent.
