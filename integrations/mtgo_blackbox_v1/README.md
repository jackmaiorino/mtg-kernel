...
`evaluate_untrusted_duel_perception_profile_v1` consumes that checked corpus directly. Its spec must bind the canonical corpus-manifest hash, the profile format must equal the visible corpus format, and the case list must contain exactly one case per corpus sample in canonical order. Each case ID, source manifest commitment, and visible BGRA commitment must match the corresponding corpus sample before any semantic comparison occurs. The reconstruction-local frame sequence remains separately bound to the expected decision and prediction and is not confused with the corpus-local ordering index. A substituted corpus hash, omitted or extra case, or crossed source frame now rejects before an accuracy result can be computed.
