...
The sibling `integrations/mtgo_visible_duel_producer_v1` assembly supplies the in-process producer seam. Version 1.19 locates exactly one public WPF `DuelScene`, requires its `DataContext` to be the exact duel view-model type, and verifies the complete 80-getter public allowlist plus thirty-three private visible-source-bound joins against the pinned loaded presentation assemblies. Its sanitized success slice covers bounded noncombat priority decisions during upkeep, draw, Main 1, Main 2, and the end step. The stack may be empty or contain only face-up, non-copy spells controlled by the seated player with an empty rendered association collection. Stack order is the rendered collection order and the last item is the visible top. Opponent-controlled items, targeted items, abilities, copies, and face-down items force the same generic abstention. It exports visible turn, active and priority roles, life, mana, hand and library counts, battlefield, graveyards, player-relative exile panels, the narrow stack slice, attachments, own hand, simple source-bound actions, and the separately typed narrow attacker-selection moment. The turn is parsed only from the rendered English-client `GameTurnText`; an unexpected locale or format abstains. A face-down exiled card name remains withheld. The producer derives an Initiative-holder candidate only from the rendered per-player Shields zone and each rendered card's public `CardFrameID`; any non-null, ambiguous, or unknown visible emblem still forces abstention until an exact live UI corpus qualifies the player association. Visible poison, energy, experience, or radiation badges, Companion, pile and card selectors, trigger panels, wish flow, storm display, temporary-zone windows, Commander, Planechase, dungeon, Ring-temptation, and speed presentation all force the same generic abstention until represented. Static inspection of `Card_View.ProcessMouseUp` confirms the visible card menu transforms alternate-menu, grouped, action-choice, mode-choice, submenu, and attack-hover entries. Static inspection of `PromptBoxViewModel` confirms that ordinary local priority sets the prompt box active and produces a default `OK` action, while numeric, mana-payment, and target flows expose additional visible prompt state. Version 1.19 therefore admits Pass only when the sole visible enabled standard control is the default `OK` action with the exact `OK` flag, numeric entry is disabled, and no mana button is present. It uses action flags, empty stack associations, and the related target, X, confirmation, sideboard, fake-action, mode-count, attacker victim, and backing identity values only as transient one-way guards and joins. Any unsupported value causes the same generic abstention. It never exports the private values, client objects, association objects, or internal identifiers. It never enumerates either library, an opponent's hidden hand or action collection, or a closed revealed zone, and never reads a face-down card name. The synthetic WPF fixture validates the original opening, upkeep, draw, and end-step priority, a two-spell stack and its fail-closed variants, a complete attacker observation and sealed step, plus a Turn 4 Main 2 state with changed life, mana, hand and library counts and populated public zones. The emitted decisions pass the strict Rust producer-result validator. Version 1.17 remains loaded in the current MTGO broker process. Version 1.19 has not been loaded into MTGO, the Pass, stack, and attacker mappings have not been confirmed in a real duel, and general stack items, blockers, revealed windows, Initiative, and modal decisions remain unsupported. Complete live semantic evidence, model scoring, and input authorization therefore remain absent.
...
first remaining toggle or `Done`. The version 1.19 producer is the sealed
