...
The sibling `integrations/mtgo_visible_duel_producer_v1` assembly supplies the in-process producer seam. Version 1.3 locates exactly one public WPF `DuelScene`, requires its `DataContext` to be the exact duel view-model type, verifies the complete 46-getter public allowlist plus nine private action-join getters against the pinned loaded presentation assemblies, and invokes exact getters for player-visible chrome, player panels, mana, the current prompt, visible standard buttons, public zones, battlefield and stack cards, attachments, visible counters, and visible legal-action classification. Private action joins originate only from visible enabled prompt controls or the seated player's visible cards. Values and raw objects are type checked, bounded, discarded locally, and never serialized. The producer never enumerates either library, an opponent's hidden hand or action collection, or a closed revealed zone, and does not read a hidden face-down name. It also never reads internal action IDs, targets, flags, timestamps, or backing card/player objects. A synthetic WPF fixture uses throwing getters to prove those prohibited paths remain untouched while every intended route executes and the channel still receives only `projection_incomplete`. The producer contains no unrestricted property enumeration, recursive object reflection, raw-value output, logging, files, network, child processes, client action invocation, model scoring, or input. `mtgo_visible_duel_viewmodel_producer_audit_v1` separately binds the exported visible getter surface and the sealed visible-source-bound private join surface. The complete player-visible projection, version 1.3 live execution attestation, semantic evidence, scoring, and input remain absent.
