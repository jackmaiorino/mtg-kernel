...
The sibling `integrations/mtgo_visible_duel_producer_v1` assembly supplies the in-process producer seam. Version 1.2 locates exactly one public WPF `DuelScene`, requires its `DataContext` to be the exact duel view-model type, verifies the complete 46-getter compile-time allowlist against the pinned loaded presentation assemblies, and invokes exact public getters for player-visible chrome, player panels, mana, the current prompt, visible standard buttons, public zones, battlefield and stack cards, attachments, and visible counters. Values are type checked, bounded, discarded locally, and never serialized. The producer never enumerates either library, an opponent's hidden hand, or a closed revealed zone, and does not read a hidden face-down name. A synthetic WPF fixture uses throwing getters to prove those prohibited paths remain untouched while every intended visible route executes and the channel still receives only `projection_incomplete`. The producer contains no unrestricted property enumeration, recursive object reflection, raw-value output, logging, files, network, child processes, client action invocation, model scoring, or input. `mtgo_visible_duel_viewmodel_producer_audit_v1` binds that root, getter surface, and partial getter layers to the existing candidate-surface commitment. The complete player-visible projection, version 1.2 live execution attestation, semantic evidence, scoring, and input remain absent.
