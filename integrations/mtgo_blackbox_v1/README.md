...
`COMPETITIVE_SIDEBOARD_MODEL_INTERFACE_2026-08-11.md` records the remaining sideboard checkpoint gap. The kernel now has reusable deterministic best-of-three state and exact registered-deck configuration validation, but it still lacks a player-visible sideboard observation, sequential checkpoint action head, and production scorer. Its existing opponent-deck-ID matchup policy is simulator-only because that identifier is hidden in MTGO. The adapter's bounded local transfer-and-submit deliberation emits no UI input while deciding, requires an explicit model-owned Submit for changed or unchanged targets, trains only from terminal match win or loss, and must be the sole production source of a competitive target configuration.
...
The attended navigation helper `scripts/invoke_supervised_mtgo_click_v1.ps1` emits exactly one left-click after checking the exact process ID and start time, executable and signer identity, DPI, title, client size, foreground ownership, client-to-screen transform, visible hit-test root, and cursor position. An explicitly empty expected title is accepted only for an exact MTGO-owned window because MTGO uses untitled top-level windows for some visible dialogs and no-activate popups. `ForegroundOwnedWindow` requires the exact owned window to be foreground. `VisibleOwnedPopup` instead requires the verified main client to remain foreground, enumerates exactly one visible same-process root owned by that main window, and requires the hit-tested point to resolve to that popup. It has no keyboard or text-input path. It does not identify the semantic control at the point, does not verify any postcondition, and is explicitly unsafe for autonomous input, purchases, or queue entry. Every use must be bracketed by manually inspected visible previews and must stop on an unexpected state.
...
sized model output. Its execution plan is bound to the exact source bytes and
desired visible-blocker mask. Each operation is derived from a fresh sanitized
selection or rendered execution-state result. The producer may add only a
desired unassigned blocker and will not submit another input until the prior
assignment is visible. It presses `Done` only after exact reconciliation and
refuses any unexpected blocker because removal is not qualified. Hidden client
objects are used only transaction-locally to resolve the already-visible
`Block` or `Done` control, then discarded. The runtime contract remains marked
unsafe for live input and has no event-entry or spending authority.
...
The loaded deployment's code-derived competitive capability commitment separately records the player-visible duel interface, player-visible history importer, pregame interface and trained head, generic sideboard interface, required sequential sideboard interface, trained sideboard head, and changed and unchanged support. Duel readiness requires the visible-only interface plus history import. Pregame readiness also requires history import. Sideboard readiness additionally requires the sequential interface. A legacy whole-target sideboard scorer therefore cannot satisfy the loaded-checkpoint capability gate.
