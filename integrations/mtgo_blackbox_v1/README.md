...
The sibling `integrations/mtgo_visible_duel_producer_v1` assembly supplies the in-process producer seam. Version 1.14 locates exactly one public WPF `DuelScene`, requires its `DataContext` to be the exact duel view-model type, and verifies the complete 74-getter public allowlist plus thirty-one private visible-source-bound joins against the pinned loaded presentation assemblies. Its synthetic success slice exports only the strict player-visible decision schema for an untouched 60-card Turn 1 Main 1 opening with seated-player priority, base life, seven-card hands, 53-card libraries, zero mana, and empty public zones, stack, combat, active prompt, and revealed zone. The turn is parsed only from the rendered English-client `GameTurnText`; an unexpected locale or format abstains. Visible exiled objects retain the player-relative rendered exile panel that contains them, while a face-down card name remains withheld. That restriction makes null Initiative complete before any game action has occurred. The producer now derives an Initiative-holder candidate only from the rendered per-player Shields zone and each rendered card's public `CardFrameID`; any non-null, ambiguous, or unknown visible emblem still forces abstention until an exact live UI corpus qualifies the player association. Visible poison, energy, experience, or radiation counter badges also force abstention because their amounts are not yet represented. A visible Companion panel likewise forces abstention because its public card is not yet represented. Visible pile and card selectors, trigger panels, wish flow, storm display, and temporary-zone windows also force abstention until their public contents and choices are represented. Commander and Planechase layouts, plus visible dungeon, Ring-temptation, and speed presentation, force the same generic abstention. Static inspection of the pinned `Card_View.ProcessMouseUp` path confirms the visible card menu is derived from the seated card's `Actions` collection but transforms alternate-menu, grouped, action-choice, mode-choice, submenu, and attack-hover entries. Static inspection of `PromptBoxViewModel.Execute` confirms target-bearing and confirmation-bearing actions transition into subsequent visible UI rather than immediately executing. Version 1.14 uses those and the related X, sideboard, fake-action, and mode-count values only as one-way guards and generically abstains on every non-default value until that visible transition is represented exactly. It supports source-bound Pass and simple opening-hand card actions and never exports client objects or internal identifiers. It never enumerates either library, an opponent's hidden hand or action collection, or a closed revealed zone, and it never reads a face-down exiled card name for either player. A synthetic WPF fixture uses throwing getters to prove those prohibited paths remain untouched and verifies these special-state, card-menu, and action-modal closures before passing the bounded opening JSON through the strict Rust producer-result validator. The pinned live broker has loaded the prior 46-getter version 1.5 build in a clean 3.4.158.4691 process and received only `duel_surface_unavailable` outside a duel. That lobby-only result exercises no data-bearing success path. Version 1.14 has not been loaded into MTGO, the synthetic Pass control has not yet been confirmed in a real duel, and later special visible game state remains unsupported, so the complete player-visible projection, live duel execution attestation, semantic evidence, scoring, and input remain absent.
