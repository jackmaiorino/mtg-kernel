...
`evaluate_untrusted_duel_perception_profile_v1` is the offline accuracy gate for that profile identity. It validates each human-expected decision, binds any prediction to the same source frame and profile, recomputes exact observation, ordered legal-action, object-binding, and whole-payload matches, counts abstentions, and verifies the declared action-family coverage. Semantic accuracy is fail-closed at exact agreement for every non-abstained prediction; the declared prediction-coverage threshold must be at least 95 percent. Corpus, annotation protocol, evaluator binary, cases, predictions, and outcomes all enter one evaluation commitment.
