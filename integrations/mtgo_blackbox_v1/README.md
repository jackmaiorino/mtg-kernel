...
`score_and_select_profile_bound_duel_candidate_v1` closes the offline scoring side of that later gate. It consumes one source-bound duel decision candidate, requires the exact admitted perception-profile commitment, calls the existing external-observation checkpoint seam with the candidate's private validated decision, and binds source, profile admission, deployment, response, and deterministic selection in one move-only result. The result exposes the selected semantic for later visible-control resolution but still cannot create input or enter an event. Because the candidate remains checked-untrusted rather than an opaque in-process capture proof, the Windows-side live bridge must separately preserve the opaque DXGI frame through perception before this can become actionable.
