...
The sibling `integrations/mtgo_visible_duel_producer_v1` assembly supplies the in-process producer seam. Version 1.20 locates exactly one public WPF `DuelScene`, requires its `DataContext` to be the exact duel view-model type, and verifies the complete 83-getter public allowlist plus thirty-three private visible-source-bound joins against the pinned loaded presentation assemblies. Its sanitized success slice covers bounded noncombat priority decisions during upkeep, draw, Main 1, Main 2, and the end step. The stack may be empty or contain only face-up, non-copy spells controlled by the seated player with an empty rendered association collection. It also exports separately typed narrow attacker and single-attacker blocker selection moments. The blocker result uses rendered `VisuallyAttacking`, `VisuallyBlocking`, and visible `Block` action state. It exports no action object or target identifier and has no dispatcher. Private backing values are used only as transient one-way guards and joins and are never exported. It never enumerates either library, an opponent's hidden hand or action collection, or a closed revealed zone, and never reads a face-down card name. The synthetic WPF fixture validates ordinary priority, the narrow stack, attacker observation and sealed execution, single-attacker blocker observation with zero execution authority, and populated public zones. The emitted results pass the strict Rust validator. Version 1.17 remains loaded in the current MTGO broker process. Version 1.20 has not been loaded into MTGO, and the live Pass, stack, attacker, and blocker mappings remain unconfirmed. Complete live semantic evidence, model scoring, and general input authorization therefore remain absent.
...
first remaining toggle or `Done`. The version 1.20 producer is the sealed
