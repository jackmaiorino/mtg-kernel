...
The sibling `integrations/mtgo_visible_duel_producer_v1` assembly supplies the in-process producer seam. Version 1.5 locates exactly one public WPF `DuelScene`, requires its `DataContext` to be the exact duel view-model type, and verifies the complete 46-getter public allowlist plus fifteen private visible-source-bound joins against the pinned loaded presentation assemblies. Its synthetic success slice exports only the strict player-visible decision schema for an untouched 60-card Turn 1 Main 1 opening with seated-player priority, base life, seven-card hands, 53-card libraries, zero mana, and empty public zones, stack, combat, active prompt, and revealed zone. That restriction makes null Initiative complete before any game action has occurred. It supports source-bound Pass and opening-hand card actions and never exports client objects or internal identifiers. It never enumerates either library, an opponent's hidden hand or action collection, or a closed revealed zone, and it never reads a face-down exiled card name for either player. A synthetic WPF fixture uses throwing getters to prove those prohibited paths remain untouched, then passes the bounded JSON through the strict Rust producer-result validator. The pinned live broker has loaded version 1.5 in a clean 3.4.158.4691 process and received only `duel_surface_unavailable` outside a duel. That lobby-only result exercises no data-bearing success path. The synthetic Pass control has not yet been confirmed in a real duel, and later special visible game state remains unsupported, so the complete player-visible projection, live duel execution attestation, semantic evidence, scoring, and input remain absent.
