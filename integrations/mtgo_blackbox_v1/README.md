...
The sibling `integrations/mtgo_visible_duel_producer_v1` assembly supplies the in-process producer seam. Version 1.1 locates exactly one public WPF `DuelScene`, requires its `DataContext` to be the exact duel view-model type, verifies the complete 46-getter compile-time allowlist against the pinned loaded presentation assemblies, and invokes only 19 exact public getters for player-visible chrome, the two player panels, mana, the current prompt, and visible standard buttons. Values are type checked, bounded, discarded locally, and never serialized. A synthetic WPF fixture proves all 19 intended getter paths execute while the channel still receives only `projection_incomplete`. The producer contains no unrestricted property enumeration, recursive object reflection, raw-value output, logging, files, network, child processes, client action invocation, model scoring, or input. `mtgo_visible_duel_viewmodel_producer_audit_v1` binds that root, getter surface, and partial getter layer to the existing candidate-surface commitment. The complete player-visible projection, version 1.1 live execution attestation, semantic evidence, scoring, and input remain absent.
