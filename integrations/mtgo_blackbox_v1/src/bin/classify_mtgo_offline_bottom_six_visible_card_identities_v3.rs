...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let artifact_directory = PathBuf::from(env::args().nth(1).ok_or(
        "usage: classify_mtgo_offline_bottom_six_visible_card_identities_v3 <artifact-directory> <profile-json>",
    )?);
    ...
    let candidate = classify_untrusted_offline_bottom_six_visible_card_identities_v3(
