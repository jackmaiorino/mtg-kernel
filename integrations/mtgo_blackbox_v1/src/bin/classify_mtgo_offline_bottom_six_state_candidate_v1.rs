...
fn main() {
    if let Err(error) = run() {
        eprintln!("MTGO_OFFLINE_BOTTOM_SIX_STATE_CANDIDATE_REJECTED:{error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let directory = PathBuf::from(arguments.next().ok_or(
        "usage: classify_mtgo_offline_bottom_six_state_candidate_v1 <artifact-directory>",
    )?);
    ...
    let candidate = classify_untrusted_offline_bottom_six_state_candidate_v1(&checked, &canonical)
        .map_err(|error| error.to_string())?;
