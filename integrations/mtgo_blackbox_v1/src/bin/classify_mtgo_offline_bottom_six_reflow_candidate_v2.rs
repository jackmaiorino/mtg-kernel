...
fn main() {
    if let Err(error) = run() {
        eprintln!("MTGO_OFFLINE_BOTTOM_SIX_REFLOW_CANDIDATE_V2_REJECTED:{error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let before_directory = PathBuf::from(arguments.next().ok_or(
        "usage: classify_mtgo_offline_bottom_six_reflow_candidate_v2 <before-directory> <after-directory>",
    )?);
    ...
    let candidate = classify_untrusted_offline_bottom_six_reflow_candidate_v2(
        &before_checked,
        &before.canonical,
        &after_checked,
        &after.canonical,
