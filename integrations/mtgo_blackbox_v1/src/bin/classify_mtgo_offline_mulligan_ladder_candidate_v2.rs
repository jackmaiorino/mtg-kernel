...
fn main() {
    if let Err(error) = run() {
        eprintln!("MTGO_OFFLINE_MULLIGAN_LADDER_CANDIDATE_V2_REJECTED:{error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let directory =
        PathBuf::from(arguments.next().ok_or(
            "usage: classify_mtgo_offline_mulligan_ladder_candidate_v2 <artifact-directory>",
        )?);
    ...
    let candidate = classify_untrusted_offline_mulligan_ladder_candidate_v2(&checked, &canonical)
        .map_err(|error| error.to_string())?;
