...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let directory = PathBuf::from(env::args().nth(1).ok_or(
        "usage: classify_mtgo_offline_bottom_six_state_candidate_v3 <artifact-directory>",
    )?);
    ...
    let candidate = classify_untrusted_offline_bottom_six_state_candidate_v3(&checked, &canonical)?;
    println!(
        "{}",
        serde_json::to_string_pretty(&json!({
            ...
            "predecessor_candidate_commitment_sha256": candidate.predecessor_candidate_commitment_sha256(),
            "observed_control_ink_sha256": candidate.observed_control_ink_sha256(),
            "prompt_matches": candidate.prompt_matches(),
            "turn_one_matches": candidate.turn_one_matches(),
            "control_matches": candidate.control_matches(),
