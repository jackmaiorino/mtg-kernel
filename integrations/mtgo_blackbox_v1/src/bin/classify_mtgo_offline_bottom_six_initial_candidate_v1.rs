...
fn main() {
    if let Err(error) = run() {
        eprintln!("MTGO_OFFLINE_BOTTOM_SIX_INITIAL_CANDIDATE_REJECTED:{error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let directory = PathBuf::from(arguments.next().ok_or(
        "usage: classify_mtgo_offline_bottom_six_initial_candidate_v1 <artifact-directory>",
    )?);
    ...
    let candidate =
        classify_untrusted_offline_bottom_six_initial_candidate_v1(&checked, &canonical)
            .map_err(|error| error.to_string())?;
    ...
    println!(
        ...
        serde_json::to_string_pretty(&json!({
            ...
            "matched_exact_region_count": candidate.matched_exact_region_count(),
            "exact_region_count": candidate.exact_region_count(),
            "bright_occupancy_pixel_count": candidate.bright_occupancy_pixel_count(),
            "occupancy_matches": candidate.occupancy_matches(),
