use mtgo_blackbox_v1::{
    check_untrusted_dxgi_capture_artifact_v1, classify_untrusted_offline_opening_hand_candidate_v1,
};
use serde_json::json;
use std::env;
use std::fs;
...
fn main() {
    if let Err(error) = run() {
        eprintln!("MTGO_OFFLINE_OPENING_HAND_CANDIDATE_REJECTED:{error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let directory =
        PathBuf::from(arguments.next().ok_or(
            "usage: classify_mtgo_offline_opening_hand_candidate_v1 <artifact-directory>",
        )?);
    ...
    let candidate = classify_untrusted_offline_opening_hand_candidate_v1(&checked, &canonical)
        .map_err(|error| error.to_string())?;
