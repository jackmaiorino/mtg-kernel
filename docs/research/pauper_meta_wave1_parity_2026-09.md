...
`0x64c82a261e078f1a` (the pre-wave-1 hash, Task 5's starting value), and
...
- `diagnostic_state_hash`, `environment_hash`, `observation_projection_hash`
  — state hashes; per the Addendum, `GameState` gained
  `monarch: Option<PlayerId>` this wave, which is folded into these hashes,
  so every pinned/computed state hash moved even when the underlying game
  state (for a monarch-free matchup like Burn mirror) did not.
- `observation.visible_projection_hash` (nested inside each decision's
  `observation`) — the same state-hash family; `rl.rs`'s
  `visible_projection_hash_v5` hashes an `ObservationHashInput` that
  explicitly includes `card_db_hash`, so this field moves purely because
  the registry's card count changed, not because of any observed content
  difference (confirmed below: the `projection`, `own_hand`,
  `known_library_cards`, and `known_hand_cards` fields it is computed over
  are byte-identical).
- `card_db_hash` (wherever nested: episode header, each decision's
  `observation`, and the manifest) — `KERNEL_CARDDB_HASH` moved with the
  registry (Addendum cause 2): branch `16022053761346434045`
  (`0xde59c501e943f3fd`) versus base `7262100742335860506`
  (`0x64c82a261e078f1a`).
