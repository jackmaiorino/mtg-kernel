...
Release build, exact formerly failing-match execution and the complete fresh engineering pilot passed under the separately recorded identity above. The later successor-loader, explicit-registration and CLI integration was compiled at `63de51911386f5fde7e8a40e4e1fc4016c619681`. Its separate engineering-006 verification completed two revision-3 collection episodes, one CPU update, exact checkpoint/embedding reload and a changed-75 BO3 with byte-identical replay; stale checkpoint and head inputs rejected before games. Those changes were not in this measured Ward binary. See the [successor integration result](learned_sideboarding_engineering_006_20260912.md).
...
The Ward implementation, focused verification and replacement engineering pilot are complete as described above. No paid allocation or GPU run was started. Successor-player loading and explicit registered-75 CLI integration subsequently passed separate engineering checks at source `63de5191`; a freshly successor-bound head was subsequently fit, evaluated on the explicitly transferred known grouped split and exercised in a two-game learned/keep BO3 with the registered 75s conserved. Full production integration still needs broader training and evaluation plus CPU/GPU/store/search work in the [brewing plan](learned_sideboard_production_brewing_plan_v1.md).
