...
3. On accept, cast that card from exile for no mana as an ordinary `CastMethodV4::Normal` cast on a dedicated new route, `SpellCastRouteV4::CascadeExile`. No cast method is added, so the frozen encoder is untouched (Global Constraints) and a cascaded spell encodes exactly like any other normal cast.
   ...
   *The five engine sites that become route-aware, all in engine.rs.* (a) 13224-13245, `begin_cast_ex`'s `cast_route` match: add `CastMethodV4::Normal if origin_zone == Zone::Exile && is_cascade_exile => SpellCastRouteV4::CascadeExile` immediately before the `ExilePermission` arm, mirroring the `is_adventure_exile` arm at 13240-13242, with the `is_cascade_exile` binding beside `is_adventure_exile` at 13209-13215. (b) 13260-13263, the cast-mode staging: stage `Some(CastMode::Normal)` unconditionally on the cascade route, so a cascaded card that happens to carry an `alt_cost` does not open a `ChooseCastMode` decision for a free cast. (c) 5998-6008, `cast_cost_is_payable`'s Normal arm: the cascade route is payable without consulting `effective_normal_cast_cost`. The `def.additional_cost` check that follows the match (engine.rs 6036-6039) still applies, which is rules-correct: "without paying its mana cost" waives the mana cost only. (d) 13467-13545, the payment match's Normal arm: pay no mana on the cascade route, and still pay any additional cost, matching (c). (e) 6767-6786, the pending-cast validator's Normal arm: add `has_cascade_exile_permission = pending.origin_zone == Zone::Exile && source.v4.cascade_grant` beside `has_adventure_exile_permission`. Nothing outside engine.rs and state.rs changes, and no `CastMethodV4` match gains an arm.
...
- Modify: `mtg-kernel/src/engine.rs` (the five route-aware sites design note B lists, plus the cascade restamp)
...
- [ ] **Step 3: Route, marker, and the five route-aware arms.** Append `SpellCastRouteV4::CascadeExile` and `ObjectStateV4::cascade_grant` in `state.rs`, then make the five engine.rs sites design note B lists route-aware: the `cast_route` match and its `is_cascade_exile` binding (13209-13215, 13224-13245), the cast-mode staging (13260-13263), the restamp across the Exile to Stack move (13286-13300), `cast_cost_is_payable`'s Normal arm (5998-6008), the payment match's Normal arm (13467-13545), and the pending-cast validator's Normal arm (6767-6786). `forced_cast_method` stays `None` and no `CastMethodV4` match gains an arm; confirm that in the task report by showing `git diff --stat` contains neither flat module. Build with `cargo test --locked -p mtg-kernel --no-run` before writing the effect.

- [ ] **Step 4: Implement the effect** per design note B: the `cast_self:cascade` trigger, `EffectOp::CascadeFromSource`'s exile loop, the `CascadeFreeCast` boolean, the cast through the ordinary cast entry point on the cascade route, and the bottoming through `shuffle_slice_in_place_v2` under the new `ShufflePurposeV2::CascadeBottomOrder`.

- [ ] **Step 5: RL surface.** Add the `CascadeFreeCast` arm to `rl.rs`'s `Decision::ChooseEffectBoolean` handling (2388) and to both `rl_session.rs` validators, then a test that drives the accept and decline branches through `flat_build_action_cache_v2`, and one that encodes the cascaded spell's own cast action through the same path. Without this, the first live AffinityV2-style abort repeats for Urzatron.
...
uvx uv@0.11.29 run --no-sync python python/tools/check_wave_branch_coverage_v1.py data/wave_branch_manifests/pauper_meta_w2.json
```
Expected: exit 0 over the cards landed so far. Then commit: `git commit -m "mechanic: cascade (shared machinery) with Maelstrom Colossus"` and push. Record in the body: the reused cast method and the new route, the five route-aware sites, the evidence that neither flat module nor `features.py` was touched, and the shuffle purpose used for the random bottoming.

**Effort: 10 hours** (revision 3: revision 2's 12 included a new cast method with ten forced match arms, an eight-site hand audit, two flat mappings, two version bumps, a Python enum and a golden migration; those are gone, replaced by five route-aware arms and the encoder-invariance test).
...
| 6 | Cascade and Maelstrom Colossus | 10 |
