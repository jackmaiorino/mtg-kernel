...
**Follow-ups for W6** (added by the harness final review, not blocking this plan's own tasks):
- Release-build P-core recalibration before `n_per_cell` is derived: the section 2 calibration run this plan's `n_derivation.calibration_mean_seconds_per_game` depends on needs to be re-taken on a release build pinned to the P-cores before W6 derives a real `n_per_cell` from it.
- The Rust versus Python paired-bootstrap cross-check test: an independent Python re-implementation of the paired-bootstrap CI (resampling method, sidedness) cross-checked against the Rust `paired_bootstrap_ci_v1` result on the same seeded input, so W6's acceptance rule is not trusting one single implementation.
- The Task C scoping ratification (mechanics-based versus target_spec-based): whether the requires-target/is-counterspell tag file's scope stays mechanics-tag-derived (as generated) or moves to a `target_spec`-derived definition is still an open ratification, not yet decided.
- Owner rulings still open: the `game_index` 4-and-above carry-forward behavior (section 4, "known gap against the parent design's section 6.1"); and whether the 2026-09-09 sequencing exception covers this search campaign's own measurements.
