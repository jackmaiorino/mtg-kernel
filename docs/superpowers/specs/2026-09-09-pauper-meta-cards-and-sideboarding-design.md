...
Date: 2026-09-09. Author: LEAD (Fable). Revision 2 (after Codex review round 1, gpt-6-astra xhigh, CHANGES REQUIRED; all ten findings adopted, see the countersignature under mtg-kernel-gae-lane). Status: DRAFT for Jack's rulings (section 9). Branch: `lead/pauper-meta-cards-v1` (from the main tree head 75406ffe). Nothing here changes a training run, a pre-registered constant, or the main tree.
...
A. Drift refresh only: register current versions of the nine decks under revisioned ids and add the cards that complete them. Cheapest; raises sampled coverage of 63 percent of the field to near 100 percent; adds no archetype the model has never seen.
...
- Utopia Sprawl, Wild Growth: aura on a land with an as-enters color choice (Sprawl), Forest attachment legality, and triggered mana delivered immediately to the land's controller.
...
- Crypt Rats, Pestilence: color-restricted X payment and simultaneous global damage; Pestilence adds an end-step intervening-if sacrifice trigger.
...
2. Registry: append `cards_v1.json` entries derived from the XMage Java source (cost, types, subtypes, power, toughness, mana production, mechanics tags, `java_file`, faces and tokens), with `engine_capability` full only after step 4 passes.
...
1. Approve W1 as revisioned registrations of the nine decks (historical registrations retained, active set switched explicitly), including the sampler re-pin in the vendored copy and the Mage fork.
2. Approve pool expansion with new registrations in the stated provisional order (W2 to W6), understanding that the trained checkpoint gains competence on them only through a later training cycle over the expanded active set, which is a separate science-lane decision.
3. Ratify the sideboard plan table (6.1) as pre-registered constants when it is authored, and again after each registration change.
4. Ratify the classifier constants (6.2) as a disclosed non-model component, and confirm the Unknown-preserves-current rule (6.3).
5. Confirm the landing rule: the card lane merges to the main tree only at a campaign boundary under a ruling; a deployment-branch merge requalifies the deployment identity.
6. Approve the W7 oracle and toolchain upgrade of the Mage fork with a CP7 baseline requalification, and any schema migration it needs.
7. Rule on the Bo3 sequencing: which accepted BO1 evidence applies to the revisioned registrations, or grant a sequencing exception (6.4).
