...
`phase1_bo3_learning_v1::update_bo3_gameplay_v1` performs one explicitly requested CPU gameplay operation. It consumes the actual private groups produced by BO3 preparation and calls the separate weighted primitive once for an eligible batch. The public CLI provides a thin one-operation entry; there is no training loop, GPU path, registry expansion or auxiliary-policy learning. Legacy checkpoint DTOs, old loss arithmetic, and ordinary update dispatch remain unchanged.
...
`phase1_bo3_trainable_v1 update --request ABSOLUTE_REQUEST.json` reads at most 1 MiB, uses the strict `Bo3GameplayUpdateRequestV1` parser, and calls the existing updater exactly once. The library owns `request.output_directory`, checkpoint/progress publication and recovery. The CLI accepts no separate `--output` for update and writes only the bounded JSON result (at most 4 MiB plus newline) to stdout. A stdout failure returns a nonzero exit even if the durable library operation completed; inspect its progress and use the same request for recovery. Existing `collect|prepare --request ABS --output ABS` contracts and artifact publication are unchanged.

All three CLI operations run on one named worker thread with a fixed 16 MiB stack. A real debug collector loading the new BO3 source overflowed the original Windows main stack (`0xC00000FD`); the main thread now starts and joins this explicitly sized worker. Worker spawn failure, returned error, panic or abort does not become success. There is no mutable stack-size override or additional parallel collection. The stack remains within the existing 8 GiB process reservation; heap/model/serialization memory still requires the execution wrapper's process cap.
...
Two ignored tests require root-supplied real pinned manifests through `MTG_BO3_CONTINUATION_REQUEST`: an update from either explicit input kind against a direct weighted oracle with injected checkpoint-only recovery, and all-incomplete `NoUpdate` preservation/recovery for either input kind. The update test derives expected update, batch and attempt counts from the validated parent/prior progress, so a fresh BO3-source second update receives the same checks. Test-only counters distinguish repeated preparation/Adam from merely equal final states. Root ran all six regular checks successfully in 5.37 test seconds after fixing a missing test-only trait import, then reported the first actual ordinary-to-BO3 update/oracle/checkpoint-only recovery passed at Adam483 to484. Fresh next-checkpoint collection hit the main-stack overflow described above. The new worker-stack/public-update CLI changes have not yet been built or executed by their author; their parser tests are source only until root runs them. Public update dispatch, corrected recollection, a second fresh update and real `NoUpdate` qualification remain pending.
