...
- Verified on 2026-09-08: both Rust crates compile and pass 539 offline tests; the producer builds with dotnet; the merge onto the main tree conflicts only in `data/flat_policy_v2/goldens_v2.json`, which regenerates cleanly; the merged kernel compiles.
...
- Branch `lead/mtgo-integration-v1` from the main tree head (lead/cycle4-refresh-manifest-v1 at 75406ffe), merging `codex/mtgo-black-box-adapter-v1` at c661ed3e as a merge commit; goldens regenerated with `python/tools/generate_flat_policy_v2_goldens.py` through `uvx uv@0.11.29 run --no-sync`; the auto-merged feature inventory verified by the generator's `--check`.
- Fix `validate_external_fixed_action_relation_v1` so `ActivateManaAbility.cost_target`, when present, must be a battlefield permanent controlled by the actor. Documenting the omission is not acceptable for live use.
...
- Codex diff review (gpt-6-astra, xhigh) before the merge lands on the main tree.
...
- `cost_target` validation fixed and merged.
