...
- A deterministic Burn-mirror engine with a schema-v5 public RL protocol, session, observation, legal-action, and policy-surface boundary. The engine-facing schema-v4 observation/action identities remain frozen, H2 remains `surface_version: 2`, and its aggregate attacker/blocker semantics are unchanged.
...
- The synchronized Python hard boundary is feature schema `actor-relative-v5-python-4`, feature registry `rust-observation-v5-action-v5-registry-4`, encoding contract `actor-relative-node-graph-10`, model-config schema 5, and model contract `kernel-policy-value-net-6`. Runner artifacts are schema 4. Training is run schema `kernel_rl_train_run/v14`, episode-summary schema `kernel_rl_train_episode_summary/v4`, and summary schema `kernel_rl_train_summary/v3`; checkpoints are `kernel_rl_train_checkpoint/v4` with sidecar v2, update-record v4, and latest-pointer v2. Cross-boundary resume or publication fails closed rather than upgrading an older artifact in place.
...
- Card-database provenance remains `kernel_carddb/v5` and is now frozen at `0x3e82c5ede6465ba9`; it binds every generated gameplay selector, including keywords, casting variants, activated abilities, secondary modes, final spell/mana programs, flashback/effect recipes, the existing reducer, Lorien's structured Draw3/Islandcycling recipes, and each Blast's checked color plus targeting-versus-resolution filter timing.
...
| Terror | 60/60 | 48/60 | 15/15 | 5/15 | Mental Note, Thought Scour, Ponder, Brainstorm, Cryptic Serpent, Deep Analysis, Lorien Revealed, and both blue Blasts work; remaining threats and spells stay fail-closed |
...
- Before phase promotion, run at least 16 paired seeds against every previously accepted deck under the separately versioned multi-deck seed contract described below. The current policy-schema-v5 Burn-mirror schedule with the Sampled V3 selector is not that contract.
