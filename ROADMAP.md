...
- **Performance go/no-go:** the historical 4.784 games/second XMage planning figure is withdrawn as a comparison denominator. It sums rounded Spy, Wildfire, Rally, and Affinity rows from one 96-runner local PBT artifact rather than measuring a matched Rally mirror, lacks a complete source/config/exit record and proof that killed games were excluded, and derives throughput from maximum assigned episode IDs rather than certified completed natural terminals. No Rust/XMage multiplier may use it. The impact target remains at least three orders of magnitude over a fresh same-host matched XMage Rally end-to-end baseline; its absolute threshold will be frozen only after that baseline is captured. Before shared actor/learner work, the Rally H2 engine-plus-`HarnessSurfaceV2` upper bound at N=1/4/8/16 must provisionally reach 956.8 natural-terminal games/second with zero safety-cap truncations, engine halts, apply errors, or seed-partition exhaustion in warmup and measurement. That number is now only an internal capacity continuation floor, not an XMage-derived speedup threshold. H2 excludes production `PolicySurfaceV5` scalarization/transactional cloning and the RL-session observation, integrity, JSONL, Python, model, and persistence stack. Falling below that floor routes the H2/kernel boundary to redesign rather than proving the core engine physics insufficient; clearing it authorizes the `kernel_rl_three_lane_ceiling/v1` core-engine, H2, and production `RlEpisodeSessionV1`/`PolicySurfaceV5` in-process diagnostic, followed separately by unprofiled end-to-end measurement. The three-lane diagnostic excludes JSONL, IPC, Python, model inference, learning, and persistence, pairs H2/V5 seed prefixes and aggregate combat semantics, explicitly declares raw/H2 trajectories non-equivalent, and is not a training-speed or XMage-speedup claim. Any multiplier requires the fresh matched same-host XMage Rally rerun.
...
- The synchronized Python hard boundary is feature schema `actor-relative-v5-python-4`, feature registry `rust-observation-v5-action-v5-registry-4`, encoding contract `actor-relative-node-graph-12`, model-config schema 5, and model contract `kernel-policy-value-net-8`. Runner artifacts are schema 5: neural seats are bound to one exact validated post-update training head, canonical episode data is published before hash-bound authoritative metadata, and a side-effect-free validator re-derives the receipt. Training is run schema `kernel_rl_train_run/v14`, episode-summary schema `kernel_rl_train_episode_summary/v4`, and summary schema `kernel_rl_train_summary/v3`; checkpoints are `kernel_rl_train_checkpoint/v4` with sidecar v2, update-record v4, and latest-pointer v2. Cross-boundary resume or publication fails closed rather than upgrading an older artifact in place.
- `docs/reports/RUNTIME_DECKS_V1_VALIDATION.md` remains an immutable record of the earlier node-graph-11/net-7 run. Its trainer/runner/evaluator validation must be rerun under node-graph-12/net-8 before new science; old evidence is not relabeled across the encoding boundary.
...
- Ordered physical-seat deck IDs and resolved deck hashes now travel through reset responses, rollouts, training runs, greedy evaluation, sampled evaluation, manifests, and every episode/game/pair record. Resume and publication reject requested or resolved deck-identity drift before mutation. Session reset and the uniform runner admit all four ordered Burn/Rally pairings; checkpoint-backed greedy or sampled runner policies exactly inherit the validated training run's environment and deck contract. Training, greedy evaluation, and sampled evaluation admit only exact mirrors (`Burn`/`Burn` or `Rally`/`Rally`) and reject mixed pairings before artifact mutation pending the separately versioned multi-deck contract.
...
- Opt-in phase profiling is now a separate, non-authoritative boundary: the Rust environment aggregates fixed disjoint `Instant` counters and writes one strict stderr record only after graceful EOF, while Python records IPC, feature/model/sample/hash, learner update, and publication phases with `perf_counter_ns`. Profile-off protocol bytes, a fake-environment full-update store, and a conditional real-kernel update-0 bootstrap store are regression-checked for profile-on/off byte identity; the latter also requires exactly one strict kernel profile record. `kernel_rl_training_benchmark/v1` separates a primary uninstrumented throughput mode from diagnostic `phase_v1` attribution and binds per-trial rates to commit claim, binary digest, authoritative run contract, model, feature contract, effective hardware, workload, and an explicit cross-process overlap contract without absolute paths. A designated-hardware Rally ceiling, production-boundary lane, matched XMage rerun, and end-to-end benchmark have not yet been accepted, so no Rust-over-XMage training speedup is claimed.
- Privileged audit artifacts use audit schema 11, model-visible policy artifacts use schema 5, and rollout manifests use schema 9. The audit stream retains the versioned cross-platform diagnostic game-state hash `fnv1a64-serde-json-game-state-envelope-v6` plus the privileged policy-environment binding; the public session wire and model-visible policy stream expose neither `environment_hash` nor `environment_hash_algorithm`, diagnostic state hashes, nor hidden state. Audit and policy readers reject legacy, mixed, missing, unknown, misgrouped, and cross-stream-inconsistent contracts. Internal stack integrity now binds immutable spell/copy, Madness-offer, and Escape source incarnations, validates staged cast/activation/discard actions before mutation, and routes every spell departure through one provenance-aware path without changing the public observation, action, policy, or session schemas.
...
- Sleep of the Dead is engine-rules complete as `{U}` Sorcery, target creature, tap it, and skip that exact battlefield incarnation's controller's next untap, with reusable Escape `{2}{U}` plus exile three other owned graveyard cards. Escape has an explicit graveyard cast route distinct from Flashback, validates the complete owner/index/incarnation-bound candidate set, stages one public object choice at a time, preflights mana and all objects before one exile batch, canonicalizes payment provenance independently of hidden pick order, and returns the physical spell to the graveyard after resolution or countering. Focused coverage pins insufficient resources, opponent/source/ceased-token exclusion, invalid and duplicate nonmutation, real JSON snapshot/restore, stable actions, continuation loss with no partial payment, stale leave-and-return targets, countering, exact paid-cost refs, repeat Escape, and the normal already-tapped target case. The rules reference is XMage commit `0723fc0c2be922af47b0ef0539f28114cc23b998` with exact source blobs `SleepOfTheDead.java` `0a736726d6204a91ee08793588c6b933a03f29e2`, `EscapeAbility.java` `d841f1724223a80565127cc60bdf8d7cfdaea5da`, `ExileFromGraveCost.java` `6565f9e164da4832ed6d09aac22876cad6a8638b`, `DontUntapInControllersNextUntapStepTargetEffect.java` `e0ca981deb84d077a5c5b6ac644aa287e02f7881`, and `TapTargetEffect.java` `329ce20cf6377cff0dde80a0e3a82c04c6c9dde2`. The current schema-v5 policy path deliberately fails closed once an Escape cast is pending: its frozen `sacrifice_chosen`/`PendingCastSacrificed` features cannot be reinterpreted, and its value head does not pool legal candidates, so a versioned generic object-cost observation/flat-policy successor plus the post-carddb-v5 Store successor is required before Terror can become an authorized training runtime.
...
- Card-database provenance is `kernel_carddb/v6` and is frozen at `0xcf2b0b92dfacdad2`; it binds every generated gameplay selector, including keywords, targeting contracts, casting variants, activated abilities, secondary modes, final spell/mana programs, Flashback/Escape/effect recipes, the generic cost reducers, Sleep of the Dead's ordered Escape cost and tap/skip-next-untap recipe, Deem Inferior's owner-library-placement recipe, Lorien's structured Draw3/Islandcycling recipes, each Blast's checked color plus targeting-versus-resolution filter timing, Murmuring Mystic's full-capability promotion and canonical subtype correction, and the appended Bird Illusion token definition. Trigger-to-token and target-history runtime validation remain source-revision-gated and behavior-tested rather than duplicated into the generated-selector hash grammar. Frozen native Store RunV2 authority remains bound to carddb-v5; current-carddb science execution requires its explicit versioned successor rather than reinterpretation.
...
| Terror | 60/60 | 56/60 | 15/15 | 5/15 | Mental Note, Thought Scour, Ponder, Brainstorm, Cryptic Serpent, Deem Inferior, Deep Analysis, Lorien Revealed, Murmuring Mystic, Sleep of the Dead, and both blue Blasts work; Tolarian Terror remains fail-closed |
...
Pauper meta wave 1 (`lead/pauper-meta-cards-v1`, Tasks 1-13, 2026-09) added 21 cards plus one token on top of the state above, growing `data/cards_v1.json` from 162 to 184 definitions and `KERNEL_CARDDB_HASH` from `0x64c8_2a26_1e07_8f1a` to `0xde59_c501_e943_f3fd`. It also registered nine new V2 decks into `REGISTRATION_SPECS` (Wildfire, Rally, Affinity, Elves, Spy, Burn, Terror, CawGates, and Faeries each gain a V2 sibling), doubling `pool_decks` from 9 to 18 and the pool from 150 to 171 unique cards, all still `engine_capability: full`. Registrations and the runtime-active set remain distinct: the nine historical decks in `RUNTIME_DECK_IDS`/`EXPECTED_RUNTIME_DECK_HASHES` are unchanged and `data/runtime_decks_v1.json` stays byte-identical, so none of the V2 decks or wave-1 cards are part of the trained/science-loop-admitted catalog yet -- they exist as registry and pool entries only. Full card list, new primitives, substitutions, and recomputed mtgtop8 coverage are in `docs/research/pauper_meta_wave1_record_2026-09.md`. **This wave is kernel-covered, not XMage-shadow-certified**: Task 14a (kernel-side directed branch coverage, spec 5.3 step 4) ran and passed, 23 of 23 cards kernel-covered with 0 unexercised branches and a matching one-seed bit-identical rerun. Full XMage-shadow certification is gated on Task 14b, which has not yet run.
...
The repository cutover is complete now: the path-filtered history and exact source/standalone checkpoint mapping are recorded in `docs/archive/EXTRACTION_PROVENANCE.md`, and new work continues in this repository. Cutover is an ownership and dependency boundary, not the final science-ready certificate.
