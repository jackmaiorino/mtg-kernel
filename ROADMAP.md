...
- Privileged audit artifacts use audit schema 7 and the versioned cross-platform diagnostic state-hash contract `fnv1a64-serde-json-game-state-envelope-v3`, bumped together after `RedSpellOnStack` became reachable in serialized pending-cast state. The faster derived hash remains an internal hot-path check. Audit readers reject legacy, mixed, missing, and unknown hash contracts; policy artifacts contain neither full-state hashes nor this privileged metadata.
