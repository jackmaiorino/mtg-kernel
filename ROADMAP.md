...
The first post-migration validation slice is Winding Way: a resolution-time Creature/Land choice followed by a reusable public `reveal top N and partition by card type` primitive. It exercises the new continuation and hand-knowledge boundary, immediately generalizes to Lead the Stampede, and requires no further public schema change. The next cluster then returns to the Terror path with Brainstorm, Ponder, scry, draw/mill, and graveyard-count costs.
