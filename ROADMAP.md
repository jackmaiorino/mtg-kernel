...
- Source-level card behavior for the complete Burn and Rally mainboards. Chain Lightning now has explicit payment, retarget, target, repeat-copy, counter/fizzle, and copy-departure behavior; the RL observation, action, and session contracts are schema v4.
- A synchronized Rust/Python schema-v4 feature boundary: perspective-scoped library and revealed-hand knowledge; typed counters, costs, choices, relations, continuous-effect durations, cast methods, object state, dungeon/initiative state, and fixed action identities. Reserved variants fail closed until their engine mechanics are implemented.
- Ordered physical-seat deck IDs and resolved deck hashes now travel through reset responses, rollouts, training runs, greedy evaluation, sampled evaluation, manifests, and every episode/game/pair record. Resume and publication reject requested or resolved deck-identity drift before mutation; the runtime currently accepts only the implemented `Burn`/`Burn` pair.
- Privileged audit artifacts use the versioned cross-platform diagnostic state-hash contract `fnv1a64-serde-json-game-state-envelope-v1`, while the faster derived hash remains an internal hot-path check. Audit readers reject legacy, mixed, missing, and unknown hash contracts; policy artifacts contain neither full-state hashes nor this privileged metadata.
...
| Terror | 60/60 | 44/60 | 15/15 | 0/15 | Mental Note, Thought Scour, Ponder, Brainstorm, Cryptic Serpent, and Deep Analysis work; remaining threats and spells stay fail-closed |
| CawGates | 60/60 | 13/60 | 15/15 | 5/15 | Island, Counterspell, Brainstorm, and Preordain work; Gates and remaining spells stay fail-closed |
...
- Preserve Chain Lightning's implemented copy, retarget, repeat-copy, counter/fizzle, schema-v4 RL, snapshot, and trace-parser regressions.
