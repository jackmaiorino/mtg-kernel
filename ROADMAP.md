...
- The support manifest currently classifies 33 unique cards as `full`, zero as `partial`, and 117 as `no_effect`. Rally and Burn are both 60/60 `full` at the mainboard card-behavior layer, and both designated content-locked, fixed-provenance reference corpora replay 40/40 with zero divergence. Phase 1 now includes metadata-derived intrinsic mana for every registered basic land, a shared fail-closed engine-capability/preflight contract, and Winding Way's schema-neutral library primitive.
