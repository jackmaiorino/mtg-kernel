...
- Privileged audit artifacts use audit schema 6 and the versioned cross-platform diagnostic state-hash contract `fnv1a64-serde-json-game-state-envelope-v2`, bumped together after the serialized pending-activation/discard state changed. The faster derived hash remains an internal hot-path check. Audit readers reject legacy, mixed, missing, and unknown hash contracts; policy artifacts contain neither full-state hashes nor this privileged metadata.
...
- Card-database provenance is now `kernel_carddb/v5` (`0x1da0a6ad2ded9e61`) and binds every generated gameplay selector, including keywords, casting variants, activated abilities, secondary modes, final spell/mana programs, flashback/effect recipes, the existing reducer, and Lorien's structured Draw3/Islandcycling recipes.
...
| Terror | 60/60 | 48/60 | 15/15 | 0/15 | Mental Note, Thought Scour, Ponder, Brainstorm, Cryptic Serpent, Deep Analysis, and Lorien Revealed work; remaining threats and spells stay fail-closed |
| CawGates | 60/60 | 16/60 | 15/15 | 5/15 | Island, Counterspell, Brainstorm, Preordain, and Lorien Revealed work; Gates and remaining spells stay fail-closed |
