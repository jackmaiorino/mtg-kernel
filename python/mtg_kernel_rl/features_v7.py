"""Version 7 actor-relative feature authority. Frozen features.py and
features_v6.py stay unchanged; this is the Phase 1 fresh-lineage generation.

The V6 successor added scoped private search sets, declared object-cost
prefixes, and public historical source bindings. This fork additively adds a
third historical-source context arm, "pending_trigger", for a same-controller
pending trigger whose live object is not yet revealed, alongside the existing
"stack" and "pending_effect" arms. Shape-compatible Net8 reuse is an explicit
feature-transfer experiment, not the frozen V5 model contract.
"""
...
FEATURE_SCHEMA_VERSION = "actor-relative-v7-python-1"
FEATURE_REGISTRY_VERSION = "rust-observation-v7-action-v5-registry-1"
ENCODING_CONTRACT_VERSION = "actor-relative-node-graph-16"
MODEL_CONTRACT_VERSION = "kernel-policy-value-net-9"
...
COST_KINDS = COST_ONE_HOT_KINDS_V7 + ["ChooseCreatureOrRevealCreature"]
...
EXTENSION_CONTEXT_SUBROLES_V7 = [
    ("extensions", "pending_cast_object_cost", "source"),
    ("extensions", "pending_cast_object_cost", "selected", "[]"),
    ("extensions", "decision_local_library", "cards", "[]", "stable"),
    ("extensions", "historical_public_sources", "[]", "source"),
]
CONTEXT_SUBROLE_IDS.update({path: 32 + i for i, path in enumerate(EXTENSION_CONTEXT_SUBROLES_V7)})
EXTENSION_ENCODING_CONTRACT_V7 = {
    "empty_extensions": "always-include-required-null-and-empty-fields-in-v7-state-hash",
    "search_cards": "actor-only-unordered-visible-multiset-no-library-position-no-operational-id-order",
    "search_copy_roles": "within-public-class-selected-prefix-index-before-unselected-no-id-derived-role-order",
    ...
    "historical_sources": "exact-stack-pending-effect-or-pending-trigger-context-binding-not-json-provenance-authentication",
...
PENDING_CAST_OBJECT_COST_V7 = ObjectSpec({
    "source": CARD_STABLE_REF,
    "controller": Seat(),
    "cast_method": E(CAST_METHODS),
    "cost_kind": E(COST_KINDS),
    "required_count": I(MODEL_INPUT, maximum=U32),
    "selected": ListSpec(CARD_STABLE_REF),
    "remaining_count": I(MODEL_INPUT, maximum=U32),
})
DECISION_LOCAL_LIBRARY_V7 = ObjectSpec({
    "chooser": Seat(),
    "library_owner": Seat(),
    "cards": ListSpec(CARD_PRIVATE),
})
HISTORICAL_SOURCE_CONTEXT_V7 = VariantSpec("kind", {
    "stack": ObjectSpec({"kind": E(["stack"]), "stack_index": I(MODEL_INPUT, maximum=U32)}),
    "pending_effect": ObjectSpec({"kind": E(["pending_effect"])}),
    "pending_trigger": ObjectSpec({"kind": E(["pending_trigger"]), "position": I(MODEL_INPUT, maximum=U32)}),
}, MODEL_INPUT)
HISTORICAL_PUBLIC_SOURCE_V7 = ObjectSpec({
    "context": HISTORICAL_SOURCE_CONTEXT_V7,
    "source": CARD_STABLE_REF,
    "stack_item_kind": E(STACK_KINDS),
})
PENDING_CHOSEN_CREATURE_COST_V7 = ObjectSpec({
    "source": CARD_STABLE_REF,
    "controller": Seat(),
    "selected_zone": E(["Battlefield", "Hand"]),
})
FINALIZED_CHOSEN_CREATURE_COST_V7 = ObjectSpec({
    "stack_index": I(MODEL_INPUT, maximum=U32),
    "source": CARD_STABLE_REF,
    "chosen": CARD_STABLE_REF,
    "power_lki": I(MODEL_INPUT, minimum=I32_MIN, maximum=I32_MAX),
})
WARD_PAYMENT_V7 = ObjectSpec({
    "targeting_stack_index": I(MODEL_INPUT, maximum=U32),
    "payer": Seat(),
    "generic": I(MODEL_INPUT, maximum=U8),
})
QUEUED_WARD_PAYMENT_V7 = ObjectSpec({
    "stack_index": I(MODEL_INPUT, maximum=U32),
    "payment": WARD_PAYMENT_V7,
})
OBSERVATION_EXTENSIONS_V7 = ObjectSpec({
    "pending_cast_object_cost": Opt(PENDING_CAST_OBJECT_COST_V7),
    "decision_local_library": Opt(DECISION_LOCAL_LIBRARY_V7),
    "historical_public_sources": ListSpec(HISTORICAL_PUBLIC_SOURCE_V7),
    "pending_chosen_creature_cost": Opt(PENDING_CHOSEN_CREATURE_COST_V7),
    "finalized_chosen_creature_costs": ListSpec(FINALIZED_CHOSEN_CREATURE_COST_V7),
    "pending_ward_payment": WARD_PAYMENT_V7,
    "queued_ward_payments": ListSpec(QUEUED_WARD_PAYMENT_V7, min_length=1),
...
OBSERVATION_SPEC = ObjectSpec(
    {
        ...
        "extensions": OBSERVATION_EXTENSIONS_V7,
...
def _encoding_payload() -> dict[str, Any]:
    return {
        ...
        "extension_context_subroles_v7": [".".join(path) for path in EXTENSION_CONTEXT_SUBROLES_V7],
        "extension_encoding_contract_v7": EXTENSION_ENCODING_CONTRACT_V7,
        "semantic_cost_kinds_v7": COST_KINDS,
        "cost_one_hot_kinds_v7": COST_ONE_HOT_KINDS_V7,
    }
...
def canonical_observation_v7(observation: dict[str, Any]) -> dict[str, Any]:
    """Canonical model JSON shared with native FlatV3, without operational IDs.

    This function assumes validated input. Required empty extension fields remain
    present, intentionally giving even common states a new V7 hash identity.
    """
    return _canonical_model_value(
        observation, OBSERVATION_SPEC, ("observation",),
        _CanonicalContext(observation["acting_player"], observation),
    )
...
def _state_features(obs: dict[str, Any]) -> list[float]:
    ...
    canonical = canonical_observation_v7(obs)
...
def _historical_source_key_v7(ref: dict[str, Any]) -> tuple:
...
class _NodeRegistry:
    ...
    def __post_init__(self) -> None:
        ...
        self._historical_source_nodes_v7: dict[tuple, int] = {}
    ...
    def resolve_context_ref_node(self, ref: dict[str, Any], group: str, order: int, source_kind: str, path: tuple[str, ...]) -> int:
        if path == ("pending_effect", "source"):
            return self.resolve_historical_source_v7(ref)
        key = self.validate_ref(ref)
    ...
    def add_historical_source_v7(self, ref: dict[str, Any], order: int) -> int:
        ...
        """
        full_key = _historical_source_key_v7(ref)
        if full_key in self._historical_source_nodes_v7:
            return self._historical_source_nodes_v7[full_key]
        key = _stable_key(ref)
        identity = _stable_identity(ref)
        previous = self._identity_by_key.get(key)
        ...
        self._historical_source_nodes_v7[full_key] = node
        return node

    def resolve_historical_source_v7(self, ref: dict[str, Any]) -> int:
        try:
            return self._historical_source_nodes_v7[_historical_source_key_v7(ref)]
        except KeyError as exc:
...
def _context_refs_with_paths_v7(value: Any, spec: Spec, path: tuple[str, ...]):
    if value is None:
        return
    if spec is CARD_STABLE_REF:
        yield path, value
    elif isinstance(spec, OptionalSpec):
        yield from _context_refs_with_paths_v7(value, spec.item, path)
    elif isinstance(spec, ListSpec):
        for child in value:
            yield from _context_refs_with_paths_v7(child, spec.item, path + ("[]",))
    elif isinstance(spec, TupleSpec):
        for index, (child, child_spec) in enumerate(zip(value, spec.items)):
            yield from _context_refs_with_paths_v7(child, child_spec, path + (str(index),))
    elif isinstance(spec, ObjectSpec):
        for key, child_spec in spec.fields.items():
            if key in value:
                yield from _context_refs_with_paths_v7(value[key], child_spec, path + (key,))
    elif isinstance(spec, VariantSpec):
        yield from _context_refs_with_paths_v7(value, spec.variants[value[spec.tag]], path)
...
def _objects(obs: dict[str, Any]) -> tuple[_NodeRegistry, list[list[float]], list[int], list[int], list[int], list[list[float]], list[int], list[int]]:
    ...
    # Ordinary detached sources keep their V5 context-derived ordinal and
    # precede new V7 rows, even when a historical ability also remains stacked.
    # Register only the existing narrow allowlist here; search/history receive
    # their separate validated authority below before context edges resolve.
    pending_ref_order = 0
    for key, spec, path in PENDING_CONTEXT_ROOTS:
        value = p["engine_context"][key]
        for ref_path, ref in _context_refs_with_paths_v7(value, spec, path):
            if ref_path in DETACHED_CONTEXT_REF_ALLOWLIST:
                registry.resolve_context_ref_node(ref, "pending_context", pending_ref_order, "pending", ref_path)
            pending_ref_order += 1

    extensions = obs["extensions"]
    search = extensions["decision_local_library"]
    search_orders: list[int] = []
    if search is not None:
        class_order = -1
        previous_class = None
        for card in search["cards"]:
            current_class = visible_library_card_key_v7(card["stable"])
            if current_class != previous_class:
                class_order += 1
                previous_class = current_class
            search_orders.append(class_order)
            registry.add_ref_node(card["stable"], "private_context", class_order, "private")
    for record in extensions["historical_public_sources"]:
        ref = record["source"]
        order = record["context"].get("stack_index", len(p["stack"]))
        registry.add_historical_source_v7(ref, order)
    ...
    for i, item in enumerate(p["stack"]):
        source = (registry.resolve(item["source"]) if item["stack_item_kind"] == "spell"
                  else registry.resolve_historical_source_v7(item["source"]))
    ...
    cost = extensions["pending_cast_object_cost"]
    extension_refs: list[tuple[dict[str, Any], str, int, int]] = []
    if cost is not None:
        extension_refs.append((cost["source"], "pending_context", 0, 32))
        extension_refs.extend((ref, "pending_context", index, 33) for index, ref in enumerate(cost["selected"]))
    if search is not None:
        extension_refs.extend((card["stable"], "private_context", order, 34)
                              for card, order in zip(search["cards"], search_orders))
    ...
    for ref, role, order, subrole in extension_refs:
        node = registry.resolve_historical_source_v7(ref) if subrole == 35 else registry.resolve(ref)
        _append_edge(edge_rows, edge_sources, edge_targets, node, node, role, order, subrole)
    ...
    def ward_edge(payment: dict[str, Any], source: dict[str, Any], subrole: int, trigger_index: int) -> None:
        ...
        source_node = registry.resolve_historical_source_v7(source)
        target_node = (registry.resolve(targeter["source"]) if targeter["stack_item_kind"] == "spell"
                       else registry.resolve_historical_source_v7(targeter["source"]))
...
def _action_card_refs(semantic: dict[str, Any], registry: _NodeRegistry) -> list[tuple[str, int, dict[str, Any], int, int]]:
    ...
    for role in ("source", "candidate", "card", "attacker", "blocker"):
        if role in semantic:
            ...
            ) and _historical_source_key_v7(semantic[role]) in registry._historical_source_nodes_v7
            node = (registry.resolve_historical_source_v7(semantic[role]) if historical_effect
...
def _action_features(action: dict[str, Any], actor: str, registry: _NodeRegistry) -> tuple[list[float], list[list[float]], list[int], list[int]]:
    ...
    cost_kind = semantic.get("cost_kind", COST_ONE_HOT_KINDS_V7[0])
    features += ([0.0] * len(COST_ONE_HOT_KINDS_V7) if cost_kind == "ChooseCreatureOrRevealCreature"
                 else _one_hot(cost_kind, COST_ONE_HOT_KINDS_V7))
...
def visible_library_card_key_v7(ref: dict[str, Any]) -> tuple[int, str, str]:
    """Observable search class, never the library position or arena identifier."""
    return ref["card_db_id"], ref["owner"], ref["controller"]


def _historical_pending_source_v7(extensions: dict[str, Any]) -> dict[str, Any] | None:
    return next((record for record in extensions["historical_public_sources"]
                 if record["context"]["kind"] == "pending_effect"), None)


def _validate_extensions_v7(observation: dict[str, Any]) -> None:
    """Check JSON consistency; only the Rust producer authenticates history."""
    if observation["schema_version"] != 6:
        raise FeatureSchemaError("observation schema mismatch")
    ext = observation["extensions"]
    ...
    if pending_cast is not None:
        ...
        if matching_spells and ((matching_spells[-1]["cast_method"] == "escape") != (cost is not None)):
            raise FeatureSchemaError("Escape extension must agree with any matching public spell cast method")
    if cost is not None:
        if pending_cast is None or engine["current_stage"] != "pending_cast":
            raise FeatureSchemaError("object cost requires an active pending cast")
        if cost["cast_method"] != "escape" or cost["cost_kind"] != "ExileFromGraveyard":
            raise FeatureSchemaError("V7 object cost currently declares only Escape graveyard exile")
        if cost["source"] != pending_cast["source"] or cost["controller"] != pending_cast["controller"]:
            raise FeatureSchemaError("object cost must bind the pending cast source and controller")
        if cost["controller"] != actor or pending_cast["origin_zone"] != "Graveyard":
            raise FeatureSchemaError("Escape cost must belong to the actor and originate in Graveyard")
        if pending_cast["is_flashback"] or pending_cast["sacrifice_chosen"]:
            raise FeatureSchemaError("Escape cannot reinterpret legacy flashback or sacrifice fields")
        if cost["source"]["zone"] != "Stack" or cost["source"]["owner"] != actor:
            raise FeatureSchemaError("announced Escape source must be actor-owned on Stack")
        if cost["required_count"] == 0 or cost["required_count"] != len(cost["selected"]) + cost["remaining_count"]:
            raise FeatureSchemaError("Escape required count must equal selected plus remaining")
        own_graveyard = {_stable_key(card["stable"]): card["stable"]
                         for card in projection["graveyards"][SEATS.index(actor)]}
        selected_keys = [_stable_key(ref) for ref in cost["selected"]]
        if len(selected_keys) != len(set(selected_keys)):
            raise FeatureSchemaError("Escape selection prefix must not repeat cards")
        for ref in cost["selected"]:
            if (ref["arena_id"] == cost["source"]["arena_id"] or ref["owner"] != actor
                    or ref["zone"] != "Graveyard" or own_graveyard.get(_stable_key(ref)) != ref):
                raise FeatureSchemaError("Escape selected cost must be another visible own graveyard card")

    records = ext["historical_public_sources"]
    contexts: list[int] = []
    for record in records:
        context, source = record["context"], record["source"]
        kind = record["stack_item_kind"]
        if source["zone"] == "Library":
            raise FeatureSchemaError("historical public source cannot grant hidden library authority")
        if source["zone"] == "Hand" and kind != "activated_ability":
            raise FeatureSchemaError("historical Hand source requires announced activated ability provenance")
        if kind == "spell" and source["zone"] != "Stack":
            raise FeatureSchemaError("historical spell source must retain Stack provenance")
        if context["kind"] == "stack":
            index = context["stack_index"]
            if index >= len(projection["stack"]):
                raise FeatureSchemaError("historical source stack context is out of bounds")
            item = projection["stack"][index]
            if kind == "spell" or source != item["source"] or kind != item["stack_item_kind"]:
                raise FeatureSchemaError("historical source must match its exact nonspell public stack item")
            if source["controller"] != item["controller"]:
                raise FeatureSchemaError("historical source controller must match its stack item")
            contexts.append(index)
        else:
            pending = engine["pending_effect"]
            if pending is None or source != pending["source"] or source["controller"] != pending["controller"]:
                raise FeatureSchemaError("historical pending source must match its exact resolving context")
            contexts.append(len(projection["stack"]))
    if contexts != sorted(set(contexts)):
        raise FeatureSchemaError("historical public source contexts must be ordered and unique")
    expected_stack = [index for index, item in enumerate(projection["stack"])
                      if item["stack_item_kind"] != "spell"]
    expected = expected_stack + ([len(projection["stack"])] if engine["pending_effect"] is not None else [])
    if contexts != expected:
        raise FeatureSchemaError("V7 requires each nonspell stack and pending effect historical source record")

    search = ext["decision_local_library"]
    pending = engine["pending_effect"]
    choice = None if pending is None else pending["choice"]
    is_search = choice is not None and choice["choice_kind"] == "targets" and choice["purpose"] == "search_result"
    if is_search != (search is not None):
        raise FeatureSchemaError("decision-local library must exist exactly for an active search-result choice")
    if search is not None:
        if search["chooser"] != actor or choice["player"] != actor:
            raise FeatureSchemaError("only the active search chooser receives decision-local library cards")
        if search["library_owner"] != actor:
            raise FeatureSchemaError("the current V7 search contract supports only the chooser's own library")
        refs = [card["stable"] for card in search["cards"]]
        keys = [_stable_key(ref) for ref in refs]
        if len(keys) != len(set(keys)):
            raise FeatureSchemaError("decision-local library repeats a physical card")
        by_key = dict(zip(keys, refs))
        targets = choice["selected_targets"] + choice["legal_targets"]
        if any(target["target_kind"] != "object" for target in targets):
            raise FeatureSchemaError("search-result targets must be library objects")
        target_refs = [target["object"] for target in targets]
        if by_key != {_stable_key(ref): ref for ref in target_refs}:
            raise FeatureSchemaError("decision-local library must equal the selected and legal target union")
        if any(ref["zone"] != "Library" or ref["owner"] != search["library_owner"] for ref in refs):
            raise FeatureSchemaError("decision-local cards must belong to the searched library")
        if len(refs) > projection["library_counts"][SEATS.index(search["library_owner"])]:
            raise FeatureSchemaError("decision-local library exceeds visible library count")
        classes = [visible_library_card_key_v7(ref) for ref in refs]
        legal_classes = [visible_library_card_key_v7(target["object"]) for target in choice["legal_targets"]]
        if classes != sorted(classes) or legal_classes != sorted(legal_classes):
            raise FeatureSchemaError("V7 search cards and legal targets require canonical visible class order")
        selected_rank = {_stable_key(target["object"]): index for index, target in enumerate(choice["selected_targets"])}
        class_roles = [(visible_library_card_key_v7(ref), selected_rank.get(_stable_key(ref), U64)) for ref in refs]
        if class_roles != sorted(class_roles):
            raise FeatureSchemaError("V7 search copies require selected prefix order before unselected copies within each class")


def _validate_engine_context(
    engine: dict[str, Any], actor: str, stack: list[dict[str, Any]], extensions: dict[str, Any]
) -> None:
    pending_keys = (
        "pending_cast",
        "pending_activation",
    ...
    if pending_activation is not None:
        source = _require_ref_zone(pending_activation["source"], "pending_activation.source", ("Battlefield", "Hand"))
        _require_ref_controller(source, pending_activation["controller"], "pending_activation.source")
        if source["zone"] == "Hand" and (source["owner"] != actor or pending_activation["controller"] != actor):
            raise FeatureSchemaError("pending Hand activation source must be visible in the actor's own hand")
    ...
    if pending_effect is not None:
        ...
        if _historical_pending_source_v7(extensions) is None:
            raise FeatureSchemaError("pending_effect.source requires its exact V7 historical context")
...
def _validate_observation_semantics(observation: dict[str, Any]) -> None:
    ...
    historical_source_refs_v7 = []
    for record in observation["extensions"]["historical_public_sources"]:
        historical_source_refs_v7.append(record["source"])
    ...
    for ref in _iter_card_refs_by_schema(observation, OBSERVATION_SPEC):
        if id(ref) in historical_stack_target_ids or id(ref) in historical_source_ids_v7:
            continue
        key = _stable_key(ref)
        identity = _stable_identity(ref)
    ...
    for ref in historical_source_refs_v7:
        key = _stable_key(ref)
        previous = identity_by_key.get(key)
        if previous is not None and (previous[0], previous[1], previous[3]) != _historical_stack_target_identity(ref):
            raise FeatureSchemaError("historical source changes immutable facts of the same incarnation")
    ...
    _validate_engine_context(p["engine_context"], observation["acting_player"], p["stack"], observation["extensions"])
...
def assert_observation_classified(observation: dict[str, Any]) -> None:
    ...
    _validate_extensions_v7(observation)
...
def validate_decision_contract(
    ...
    for action in legal_actions:
        ...
        if semantic["action_kind"] != "choose_cost_target" or semantic["cost_kind"] != "ExileFromGraveyard":
            continue
        if cost is None:
            raise FeatureSchemaError("graveyard exile cost action requires its V7 pending object cost")
        if semantic["source"] != cost["source"] or semantic["remaining"] != cost["remaining_count"]:
            raise FeatureSchemaError("Escape action must match the declared source and remaining cost")
        candidate = semantic["candidate"]
        own_graveyard = [card["stable"] for card in observation["projection"]["graveyards"][SEATS.index(observation["acting_player"])]]
        if (candidate not in own_graveyard or candidate in cost["selected"]
                or candidate["arena_id"] == cost["source"]["arena_id"]):
            raise FeatureSchemaError("Escape action candidate must be an unselected own graveyard card")
