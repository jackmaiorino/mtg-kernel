"""Version 6 actor-relative feature authority. Frozen features.py stays unchanged.

The successor adds scoped private search sets, declared object-cost prefixes,
and public historical source bindings. Shape-compatible Net8 reuse is an
explicit feature-transfer experiment, not the frozen V5 model contract.
"""
...
CONTEXT_SUBROLE_IDS.update({path: 32 + i for i, path in enumerate(EXTENSION_CONTEXT_SUBROLES_V6)})
EXTENSION_ENCODING_CONTRACT_V6 = {
    "empty_extensions": "always-include-required-null-and-empty-fields-in-v6-state-hash",
...
PENDING_CAST_OBJECT_COST_V6 = ObjectSpec({
    "source": CARD_STABLE_REF,
    "controller": Seat(),
...
DECISION_LOCAL_LIBRARY_V6 = ObjectSpec({
    "chooser": Seat(),
    "library_owner": Seat(),
    "cards": ListSpec(CARD_PRIVATE),
})
HISTORICAL_SOURCE_CONTEXT_V6 = VariantSpec("kind", {
    "stack": ObjectSpec({"kind": E(["stack"]), "stack_index": I(MODEL_INPUT, maximum=U32)}),
    "pending_effect": ObjectSpec({"kind": E(["pending_effect"])}),
}, MODEL_INPUT)
HISTORICAL_PUBLIC_SOURCE_V6 = ObjectSpec({
    "context": HISTORICAL_SOURCE_CONTEXT_V6,
    "source": CARD_STABLE_REF,
    "stack_item_kind": E(STACK_KINDS),
})
...
OBSERVATION_SPEC = ObjectSpec(
    {
        ...
        "extensions": OBSERVATION_EXTENSIONS_V6,
...
def canonical_observation_v6(observation: dict[str, Any]) -> dict[str, Any]:
    ...
    present, intentionally giving even common states a new V6 hash identity.
    """
...
def _state_features(obs: dict[str, Any]) -> list[float]:
    ...
    canonical = canonical_observation_v6(obs)
...
def _context_refs_with_paths_v6(value: Any, spec: Spec, path: tuple[str, ...]):
    if value is None:
        return
    if spec is CARD_STABLE_REF:
        yield path, value
    elif isinstance(spec, OptionalSpec):
        yield from _context_refs_with_paths_v6(value, spec.item, path)
    elif isinstance(spec, ListSpec):
        for child in value:
            yield from _context_refs_with_paths_v6(child, spec.item, path + ("[]",))
...
def _objects(obs: dict[str, Any]) -> tuple[_NodeRegistry, list[list[float]], list[int], list[int], list[int], list[list[float]], list[int], list[int]]:
    ...
    # precede new V6 rows, even when a historical ability also remains stacked.
    ...
    for key, spec, path in PENDING_CONTEXT_ROOTS:
        ...
        for ref_path, ref in _context_refs_with_paths_v6(value, spec, path):
    ...
    if search is not None:
        ...
        for card in search["cards"]:
            current_class = visible_library_card_key_v6(card["stable"])
...
def _validate_extensions_v6(observation: dict[str, Any]) -> None:
    ...
    if cost is not None:
        ...
        if cost["cast_method"] != "escape" or cost["cost_kind"] != "ExileFromGraveyard":
            raise FeatureSchemaError("V6 object cost currently declares only Escape graveyard exile")
    ...
    if contexts != expected:
        raise FeatureSchemaError("V6 requires each nonspell stack and pending effect historical source record")
    ...
    if search is not None:
        ...
        if search["library_owner"] != actor:
            raise FeatureSchemaError("the current V6 search contract supports only the chooser's own library")
        ...
        classes = [visible_library_card_key_v6(ref) for ref in refs]
        legal_classes = [visible_library_card_key_v6(target["object"]) for target in choice["legal_targets"]]
        if classes != sorted(classes) or legal_classes != sorted(legal_classes):
            raise FeatureSchemaError("V6 search cards and legal targets require canonical visible class order")
        selected_rank = {_stable_key(target["object"]): index for index, target in enumerate(choice["selected_targets"])}
        class_roles = [(visible_library_card_key_v6(ref), selected_rank.get(_stable_key(ref), U64)) for ref in refs]
        if class_roles != sorted(class_roles):
            raise FeatureSchemaError("V6 search copies require selected prefix order before unselected copies within each class")
...
def _validate_engine_context(
    ...
    if pending_effect is not None:
        ...
        if _historical_pending_source_v6(extensions) is None:
            raise FeatureSchemaError("pending_effect.source requires its exact V6 historical context")
...
def assert_observation_classified(observation: dict[str, Any]) -> None:
    ...
    _validate_extensions_v6(observation)
...
def validate_decision_contract(
    ...
    for action in legal_actions:
        ...
        if semantic["action_kind"] != "choose_cost_target" or semantic["cost_kind"] != "ExileFromGraveyard":
            continue
        if cost is None:
            raise FeatureSchemaError("graveyard exile cost action requires its V6 pending object cost")
