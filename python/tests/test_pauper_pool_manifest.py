...
class PauperPoolManifestTest(unittest.TestCase):
    ...
    def test_support_is_complete_hashed_and_pins_copy_totals(self) -> None:
        ...
        cryptic_serpent = next(
            row for row in self.support["cards"] if row["name"] == "Cryptic Serpent"
        )
        self.assertEqual(
        ...
        self.assertEqual(deem_inferior["blockers"], [])
        deep_analysis = next(
            row for row in self.support["cards"] if row["name"] == "Deep Analysis"
        )
        self.assertEqual(
        ...
        self.assertEqual(deep_analysis["blockers"], [])
        lorien = next(row for row in self.support["cards"] if row["name"] == "Lorien Revealed")
        self.assertEqual(
            lorien["mainboard"],
            [
                {"deck_id": "Terror", "copies": 4},
