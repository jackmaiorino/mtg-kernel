"""Generate Torch-authoritative CPU forward goldens for the wide-net
(kernel-policy-value-net-8w128) native reference.

Capacity Experiment Contract (Stage 3), Section 3: "wide forward goldens
generated from the wide runner-fixed construction and pinned (same pattern as
the existing goldens test)". This is the wide-net sibling of
``generate_native_policy_value_net_v1_goldens.py``; it does not touch that
script, its output artifact, or the frozen ``kernel-policy-value-net-8``
constants.

Uses the same ``runner-fixed-v1`` deterministic initializer as the frozen
goldens (not the trainer-seeded snapshot authority): this is an
inference-parity fixture, not a training/throughput/cross-libm claim, and not
evidence for the capacity experiment itself.
"""
...
from mtg_kernel_rl.model import INITIALIZER_RUNNER_FIXED_V1, KernelPolicyValueNet  # noqa: E402
from mtg_kernel_rl.wide_model_snapshot_v1 import WideModelConfigV1  # noqa: E402


SCHEMA = "native-policy-value-net-wide-v1-torch-goldens-v1"
...
OUTPUT = ROOT / "data" / "native_policy_value_net_wide_v1" / "runner_fixed_forward_goldens_wide_v1.json"
...
NUMERICAL_CLAIM = (
    ...
    "tolerances; no cross-libm or bit-parity claim. Diagnostic, non-evidence "
    "wide-net (kernel-policy-value-net-8w128) capacity-experiment fixture."
)
...
def _strict_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise RuntimeError(f"duplicate JSON key in wide native model fixture: {key}")
        result[key] = value
    return result


def _reject_json_constant(value: str) -> None:
    raise RuntimeError(f"non-finite JSON constant in wide native model fixture: {value}")
...
def _schema(config: WideModelConfigV1) -> FeatureSchema:
...
def _encoded(case: dict[str, Any], config: WideModelConfigV1) -> EncodedDecision:
...
def _assert_portable_runtime() -> None:
    ...
    if actual != expected:
        raise RuntimeError(
            f"wide native model portable checker runtime mismatch: expected={expected} actual={actual}"
        )
...
def _assert_exact_authority_environment() -> None:
    ...
    if actual != expected:
        raise RuntimeError(
            f"wide native model exact authority environment mismatch: expected={expected} actual={actual}"
        )
...
def _portable_check() -> None:
    ...
    if not OUTPUT.is_file():
        raise RuntimeError(f"checked wide native model fixture is missing: {OUTPUT}")
    ...
    if _encoded_payload(checked) != checked_bytes:
        raise RuntimeError("checked wide native model fixture is not canonical pretty JSON")
    if set(checked) != {
        "schema",
        "authority",
        "model_config",
        ...
        raise RuntimeError("checked wide native model fixture top-level schema drifted")
    authority_sha = _sha256(MODEL_AUTHORITY)
    if authority_sha != EXPECTED_MODEL_AUTHORITY_SHA256:
        raise RuntimeError(
            "python model.py authority drifted: "
    ...
    for key, expected in expected_static.items():
        if checked.get(key) != expected:
            raise RuntimeError(f"checked wide native model fixture static contract drift: {key}")
    manifest = checked["parameter_manifest"]
    if manifest["count"] != 2_750_754 or len(manifest["ordered"]) != 33:
        raise RuntimeError("checked wide native model parameter manifest count drifted")
...
def main() -> int:
    ...
    mode.add_argument("--authority-check", action="store_true")
    ...
    if args.authority_check:
        ...
        if actual != expected:
            raise SystemExit(f"stale wide native policy/value authority golden: {OUTPUT}")
