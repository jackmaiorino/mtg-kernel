...
#[path = "../src/native_flat_cpu_reference_v1.rs"]
mod native_flat_cpu_reference_v1;
use native_flat_cpu_reference_v1::{
    checked_i32, checked_product, checked_sum, cpu_adam_update, max_abs, mix64, validate_adam,
    AdamConfig, CpuActivations, CpuOutputGradients, CpuStepResult, DenseBatchViewV1, HostModel,
    HostParameter, LossSummary, ACTION_DIM, ADAM_BETA1, ADAM_BETA2, ADAM_EPSILON, HIDDEN,
    LEARNING_RATE, PARAMETER_COUNT, STATE_DIM, VALUE_COEFFICIENT, WEIGHT_SEED,
};
#[cfg(test)]
use native_flat_cpu_reference_v1::{linear_no_bias, matmul_nt, matmul_tn};
...
impl SyntheticBatch {
    ...
    fn dense_view(&self) -> DenseBatchViewV1<'_> {
        DenseBatchViewV1::from_slices_unvalidated(
            &self.offsets,
            &self.action_owner,
            &self.states,
            &self.actions,
...
fn cpu_forward(
    ...
    let view = batch.dense_view();
    native_flat_cpu_reference_v1::cpu_forward(model, &view)
}
...
fn cpu_terminal_loss_and_output_gradients(
    ...
    let view = batch.dense_view();
    native_flat_cpu_reference_v1::cpu_terminal_loss_and_output_gradients(
        logits,
        values,
        &view,
        value_coefficient,
...
fn cpu_train_step(
    ...
    let view = batch.dense_view();
    native_flat_cpu_reference_v1::cpu_train_step(model, &view, value_coefficient, adam)
}
...
fn cpu_detached_surrogate_loss(
    ...
    let view = batch.dense_view();
    native_flat_cpu_reference_v1::cpu_detached_surrogate_loss(model, &view, detached_advantages)
}
...
mod tests {
    ...
    #[test]
    fn compatibility_view_preserves_model_batch_and_adam_error_precedence() {
        let mut invalid_batch = SyntheticBatch::small_golden().unwrap();
        invalid_batch.offsets[1] = -1;

        let mut invalid_model = HostModel::deterministic().unwrap();
        invalid_model.state_w1.gradient.pop();
        let model_first = cpu_forward(&invalid_model, &invalid_batch)
            .unwrap_err()
            .to_string();
        assert!(model_first.starts_with("host parameter state_w1 shape mismatch:"));

        let mut valid_model = HostModel::deterministic().unwrap();
        let invalid_adam = AdamConfig {
            epsilon: 0.0,
            ..AdamConfig::default()
        };
        let batch_before_adam = cpu_train_step(
            &mut valid_model,
            &invalid_batch,
            VALUE_COEFFICIENT,
            invalid_adam,
        ...
        .unwrap_err()
        .to_string();
        assert_eq!(batch_before_adam, "negative end offset at decision 0");
        assert_eq!(valid_model.adam_step, 0);
    }
