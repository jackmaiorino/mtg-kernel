...
fn ready_game_without_permanents() -> GameState {
    let p0_library = [card_id("Island")];
    let p1_library = [card_id("Snow-Covered Forest")];
    let mut state =
        GameState::new_from_libraries(&p0_library, &p1_library, card_name, 0x424C_4153_5454);
    state.step = Step::Main1;
    state.active_player = PlayerId::P0;
    state.priority_player = PlayerId::P0;
...
#[test]
fn blue_blasts_with_zero_viable_modes_are_not_offered_or_castable() {
    for blast_name in ["Blue Elemental Blast", "Hydroblast"] {
        let mut state = ready_game_without_permanents();
        ...
        assert!(matches!(
            ...
            Decision::CastSpellOrPass {
                ...
                ref castable_spells,
                ..
            } if !castable_spells.contains(&blast)
        ));
        assert_eq!(state, before, "{blast_name}");

        let error = engine::step(&mut state, Action::CastSpell(blast))
            .expect_err("a spell with no viable mode must fail closed");
        assert!(error.contains("not castable"), "{blast_name}: {error}");
        assert_eq!(state, before, "{blast_name}");
    }
}
...
fn blue_pending_cast_is_frozen_into_diagnostic_hash_v3() {
    ...
    assert_eq!(
        state.engine.pending_cast.as_ref().unwrap().target_spec,
    ...
    assert!(matches!(
        ...
        Decision::ChooseTargets {
            ref legal_targets,
            ..
        } if legal_targets.contains(&Target::Object(red_permanent))
    ));
    assert_eq!(
        state.engine.pending_cast.as_ref().unwrap().target_spec,
        TargetSpec::RedSpellOnStack,
        "the serialized primary spec stays fixed while mode 1 uses its own target spec"
    );
}
