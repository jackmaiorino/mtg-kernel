...
/// CI guard (not ignored, seconds): replays the first recorded search-wrapped
/// match, root actions only and no search, through the production fill, and
/// checks every decision's fingerprint against the pinned search golden's
/// line for that decision.
#[test]
fn tensorize_cost_search_replay_prefix_matches_golden_v1() {
    let mut replay: SearchReplayFileV1 =
        serde_json::from_slice(&std::fs::read(search_replay_path_v1()).unwrap()).unwrap();
    replay.matches.truncate(1);
    let (corpus, seats) = replay_search_corpus_v1(&replay);
    assert!(corpus.len() >= 50 && seats[0] > 0 && seats[1] > 0);
    let golden = std::fs::read_to_string(default_search_golden_path_v1())
        .unwrap()
        .replace("\r\n", "\n");
    ...
    for (index, owned) in corpus.iter().enumerate() {
        ...
        assert_eq!(fields[0], index.to_string(), "golden line order");
        let fingerprint = decision_fingerprint_v1(&mut tensorizer, owned.view());
        assert_eq!(
            hex(&fingerprint),
            fields[3],
            "search replay decision {index}"
        );
    }
}
