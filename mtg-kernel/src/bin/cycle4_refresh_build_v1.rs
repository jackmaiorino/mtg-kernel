...
//! plain-file inputs a wrapper (or a human operator) already produced, run
//! the pure builder, and write the resulting manifest's exact canonical
//! bytes to `--output`. No environment variables; every input is a strict
//! flag. Follows the `checkpoint_shadow_stdio_v1.rs` parsing shape.
//!
...
//! is well-formed -- a file that cannot be read or written, or the pure
//! builder rejecting the assembled content (contract rejection). The
//! governing contract (`docs/native_cycle4_arm_launcher_v1.md` section 5)
//! only names 0/2/3 for this bin, so I/O failure is folded into 3 alongside
//! content rejection rather than adding an undocumented fourth code.

use mtg_kernel::native_population_refresh_builder_cycle4_v1::{
    build_cycle4_genesis_refresh_v1, build_cycle4_next_refresh_v1, Cycle4RefreshBuildResultV1,
};
...
fn usage_v1() -> ! {
    eprintln!(
        "usage: cycle4_refresh_build_v1 --trainee-run-sha256 HEX --trainee-base-seed N --slot-identities PATH --output PATH (--genesis | --previous-manifest PATH --panel PATH --next-generation N)"
    );
    std::process::exit(2);
}

/// The two ways a boundary can be built, matching the builder module's own
/// two entry points exactly: genesis takes no previous manifest, no panel,
...
fn parse_args_v1(raw: Vec<OsString>) -> Result<ParsedArgsV1, ()> {
    ...
    let mut previous_manifest: Option<PathBuf> = None;
    ...
    while index < raw.len() {
        ...
        match flag {
            "--previous-manifest" if previous_manifest.is_none() => {
                previous_manifest = Some(PathBuf::from(value));
            }
    ...
    let mode = match (genesis, previous_manifest, panel, next_generation) {
        (true, None, None, None) => RefreshModeV1::Genesis,
        (false, Some(previous_manifest), Some(panel), Some(next_generation)) => {
...
fn main() {
    ...
    let build_result: Result<Cycle4RefreshBuildResultV1, _> = match &args.mode {
        ...
        RefreshModeV1::NextRefresh {
            ...
            let previous_manifest_bytes =
                read_file_or_exit_v1(previous_manifest, "--previous-manifest");
    ...
    if let Err(error) = std::fs::write(&args.output, &result.canonical_bytes) {
        eprintln!(
