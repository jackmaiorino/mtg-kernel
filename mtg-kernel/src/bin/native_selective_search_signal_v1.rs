use mtg_kernel::native_rollout_teacher_v1::{
    native_selective_search_report_bytes_v1, run_native_selective_search_signal_v1,
};
...
fn usage_v1() -> ! {
    eprintln!(
        "usage: native_selective_search_signal_v1 --source-outcome-root PATH --output-json PATH"
    );
    std::process::exit(2);
}
...
fn main() {
    ...
    let envelope = run_native_selective_search_signal_v1(&source).unwrap_or_else(|error| {
        eprintln!("native selective-search signal screen failed: {error}");
        std::process::exit(1);
    });
    let file = OpenOptions::new()
        ...
        .unwrap_or_else(|error| {
            eprintln!("native selective-search output create failed: {error}");
            std::process::exit(1);
        });
    let mut writer = BufWriter::new(file);
    let report_bytes =
        native_selective_search_report_bytes_v1(&envelope.report).unwrap_or_else(|error| {
            eprintln!("native selective-search output encode failed: {error}");
            std::process::exit(1);
        });
    writer.write_all(&report_bytes).unwrap_or_else(|error| {
        eprintln!("native selective-search output write failed: {error}");
        std::process::exit(1);
    });
    writer.flush().unwrap_or_else(|error| {
        eprintln!("native selective-search output flush failed: {error}");
        std::process::exit(1);
    });
    println!(
        "{} {} {}",
