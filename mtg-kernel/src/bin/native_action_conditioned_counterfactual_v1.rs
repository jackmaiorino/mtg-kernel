use mtg_kernel::native_rollout_teacher_v1::{
    native_action_conditioned_counterfactual_report_bytes_v1,
    run_native_action_conditioned_counterfactual_v1,
};
...
fn usage_v1() -> ! {
    eprintln!(
        "usage: native_action_conditioned_counterfactual_v1 --source-outcome-root PATH --output-json PATH"
    );
    std::process::exit(2);
}
...
fn main() {
    ...
    let envelope =
        run_native_action_conditioned_counterfactual_v1(&source).unwrap_or_else(|error| {
            eprintln!("native action-conditioned counterfactual corpus failed: {error}");
            std::process::exit(1);
        });
    let file = OpenOptions::new()
        ...
        .unwrap_or_else(|error| {
            eprintln!("counterfactual corpus output create failed: {error}");
            std::process::exit(1);
        });
    let mut writer = BufWriter::new(file);
    let bytes = native_action_conditioned_counterfactual_report_bytes_v1(&envelope.report)
        .unwrap_or_else(|error| {
            eprintln!("counterfactual corpus output encode failed: {error}");
            std::process::exit(1);
        });
    writer.write_all(&bytes).unwrap_or_else(|error| {
        eprintln!("counterfactual corpus output write failed: {error}");
        std::process::exit(1);
    });
    writer.flush().unwrap_or_else(|error| {
        eprintln!("counterfactual corpus output flush failed: {error}");
        std::process::exit(1);
    });
    println!(
        "{} {} {}",
