use mtg_kernel::native_checkpoint_shadow_stdio_v1::{
    run_checkpoint_shadow_stdio_with_selector_v1, ShadowCandidateSelectorV1,
...
fn usage_v1() -> ! {
    eprintln!(
        "usage: checkpoint_shadow_history_value_depth8_stdio_v1 --xmage-cp7-outcome-root PATH"
    );
    std::process::exit(2);
}
...
fn main() {
    ...
    if let Err(error) = run_checkpoint_shadow_stdio_with_selector_v1(
        ...
        eprintln!("checkpoint depth-8 history-value shadow scorer failed: {error}");
        std::process::exit(1);
    }
}
...
mod tests {
    ...
    #[test]
    fn accepts_only_one_explicit_history_candidate_root_v1() {
        assert_eq!(
            parse_args_v1(vec!["--xmage-cp7-outcome-root".into(), "candidate".into()]).unwrap(),
            PathBuf::from("candidate")
        );
        assert!(parse_args_v1(vec!["--original-store-root".into(), "store".into()]).is_err());
    }
}
