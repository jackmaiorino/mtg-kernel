//! Cycle-5 arm run-record CLI (`docs/native_cycle5_arm_launcher_v1.md`
...
//! Store, and the compiled cycle-5 literals; there are no defaults, no
...
//!
//! Strict flag parsing follows `cycle5_refresh_build_v1.rs`: order-
...
use mtg_kernel::native_cycle5_arm_v1::Cycle5ArmKindV1;
use mtg_kernel::native_cycle5_run_record_v1::{
    build_cycle5_arm_run_record_v1, Cycle5RunRecordRequestV1,
};
...
fn publish_output_or_exit_v1(output: &Path, bytes: &[u8], replacing: bool) {
    ...
    let final_name = output.file_name().unwrap_or_else(|| {
        eprintln!(
            "cycle5_run_record_v1: --output names no file: {}",
            output.display()
        );
        std::process::exit(3);
    });
    ...
    let fail = |detail: String| -> ! {
        eprintln!(
            "cycle5_run_record_v1: failed writing --output ({}): {detail}",
            output.display()
        );
        std::process::exit(3);
    };
...
fn main() {
    ...
    .unwrap_or_else(|error| {
        eprintln!("cycle5_run_record_v1: {error}");
        std::process::exit(3);
    });
    ...
    match overwrite_decision_v1(existing.as_deref(), &outcome.canonical_bytes, args.force) {
        ...
        None => {
            eprintln!(
                "cycle5_run_record_v1: {} already holds a DIFFERENT run record; refusing to re-key a campaign identity (pass --force only when no Store, chain or manifest binds the existing one)",
                args.output.display()
            );
            std::process::exit(3);
        }
...
mod tests {
    ...
    fn parses_every_required_flag_v1() {
        ...
        assert_eq!(
            ...
            PathBuf::from("D:\\release\\cycle5_arm_v1.exe")
        );
    ...
    fn a_forced_replacement_replaces_a_differing_file_v1() {
        ...
        let root = std::env::temp_dir().join(format!(
            "mtg-kernel-cycle5-run-record-force-{}-{nonce}",
            std::process::id()
        ));
