//! Thin CLI wrapping `native_population_refresh_builder_cycle5_v1`: read the
...
//! `native_population_refresh_builder_cycle5_v1` documents), the new payoff
...
//! contract (`docs/native_cycle5_arm_launcher_v1.md` section 5) only names
...
use mtg_kernel::native_population_refresh_builder_cycle5_v1::{
    build_cycle5_genesis_refresh_v1, build_cycle5_next_refresh_v1,
    cycle5_chain_manifest_filename_v1, cycle5_chain_panel_filename_v1, Cycle5ChainLinkV1,
    Cycle5RefreshBuildResultV1,
};
...
/// `docs/native_population_refresh_manifest_cycle5_v1.md`); bounds the
...
fn usage_v1() -> ! {
    eprintln!(
        "usage: cycle5_refresh_build_v1 --trainee-run-sha256 HEX --trainee-base-seed N --slot-identities PATH --output PATH (--genesis | --chain-dir PATH --panel PATH --next-generation N)"
    );
    std::process::exit(2);
}
...
fn read_file_or_exit_v1(path: &Path, flag: &str) -> Vec<u8> {
    std::fs::read(path).unwrap_or_else(|error| {
        eprintln!(
            "cycle5_refresh_build_v1: failed reading {flag} ({}): {error}",
            path.display()
        );
        std::process::exit(3);
    })
...
fn read_chain_dir_or_exit_v1(chain_dir: &Path) -> Vec<Cycle5ChainLinkV1> {
    let mut chain = Vec::new();
    for refresh_index in 0..=CHAIN_WALK_MAX_REFRESH_INDEX_V1 {
        let manifest_path = chain_dir.join(cycle5_chain_manifest_filename_v1(refresh_index));
...
fn write_output_atomically_or_exit_v1(output: &Path, bytes: &[u8]) {
    ...
    let file_name = output.file_name().map_or_else(
        || "cycle5-refresh-manifest".to_owned(),
    ...
    if let Err(error) = write_result {
        ...
        eprintln!(
            "cycle5_refresh_build_v1: failed writing --output ({}): {error}",
            output.display()
        );
        std::process::exit(3);
    }
...
fn main() {
    ...
    let build_result: Result<Cycle5RefreshBuildResultV1, _> = match &args.mode {
        RefreshModeV1::Genesis => build_cycle5_genesis_refresh_v1(
    ...
    let result = build_result.unwrap_or_else(|error| {
        eprintln!("cycle5_refresh_build_v1: manifest rejected: {error}");
        std::process::exit(3);
    });
...
mod tests {
    ...
    fn fresh_temp_dir_v1(label: &str) -> PathBuf {
        ...
        let root = std::env::temp_dir().join(format!(
            "mtg-kernel-cycle5-refresh-build-{label}-{}-{nonce}",
            std::process::id()
        ));
