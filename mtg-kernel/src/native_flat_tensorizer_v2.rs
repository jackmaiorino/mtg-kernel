...
// ratified this literal as the CURRENT-profile pin (collab CLAUDE #239).
...
// Rebound together with the source-hash constant above: this is the live
// golden's own whole-file hash, regenerated by
// `python/tools/generate_python_full_features_v2_goldens.py` against the
// fixed features.py (`data/flat_policy_v2/python_full_features_v2.json`).
// The historical values are preserved forever as
// `FROZEN_TENSORIZER_FIXTURE_SHA256_V2`/`_PAYLOAD_SHA256_V2` in
