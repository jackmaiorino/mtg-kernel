//! Thin, pure builder for the cycle-5 population refresh manifest chain.
//!
//! Wraps `native_population_refresh_manifest_cycle5_v1` (contract:
//! `docs/native_population_refresh_manifest_cycle5_v1.md`) for the one-shot
//! CLI use case in `src/bin/cycle5_refresh_build_v1.rs`: read the whole
...
//! assemble the eight `Cycle5RefreshSlotV1` records, and build the next
...
//! directory into `Cycle5ChainLinkV1` values is the bin's job).
//!
...
//!
//! `cycle5_chain_manifest_filename_v1` and `cycle5_chain_panel_filename_v1`
//! are the single source of truth for these names on the Rust side; the
//! Python payoff-panel runner
//! (`scripts/experiments/population_v2_cycle5_v1/run_payoff_panel_v1.py`)
...
use crate::native_population_refresh_manifest_cycle5_v1::{
    build_cycle5_refresh_manifest_v1, decode_cycle5_refresh_manifest_v1, mw_update_cycle5_v1,
    panel_score_fraction_cycle5_v1, Cycle5RefreshManifestErrorV1, Cycle5RefreshManifestV1,
    Cycle5RefreshSlotV1, CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1, CYCLE5_PANEL_GAMES_PER_MATCHUP_V1,
    CYCLE5_PANEL_GAMES_PER_POLICY_V1, CYCLE5_SLOT_COUNT_V1,
};
...
pub const CYCLE5_SLOT_IDENTITIES_SCHEMA_V1: &str = "mtg-kernel-cycle5-slot-identities/v1";
/// Schema this builder expects of the payoff panel document (Python panel
/// runner's `panel.json` / `refresh-NN.panel.json`, schema
/// `mtg-kernel-cycle5-payoff-panel/v1`). This module reads the
...
pub const CYCLE5_PANEL_DOC_SCHEMA_V1: &str = "mtg-kernel-cycle5-payoff-panel/v1";

const EXPECTED_ROLES_V1: [&str; CYCLE5_SLOT_COUNT_V1] = [
    "anchor-0",
    "anchor-1",
    "historical-0",
    "historical-1",
...
/// Number of round-robin matchups over `CYCLE5_SLOT_COUNT_V1` slots:
/// `C(8, 2) = 28`.
const CYCLE5_MATCHUP_COUNT_V1: usize = CYCLE5_SLOT_COUNT_V1 * (CYCLE5_SLOT_COUNT_V1 - 1) / 2;

/// Fixed on-disk filename for one refresh index's manifest, per the chain
/// directory naming scheme documented on this module.
#[must_use]
pub fn cycle5_chain_manifest_filename_v1(refresh_index: u64) -> String {
...
/// module.
#[must_use]
pub fn cycle5_chain_panel_filename_v1(refresh_index: u64) -> String {
...
pub struct Cycle5SlotIdentityInputV1 {
    pub slot_index: u64,
...
#[derive(Clone, Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct Cycle5SlotIdentitiesFileV1 {
    schema: String,
    slots: Vec<Cycle5SlotIdentityInputV1>,
}

#[derive(Clone, Debug, Deserialize)]
struct Cycle5PanelRankSumEntryV1 {
    slot_index: u64,
    u_i: i64,
}
...
#[derive(Clone, Debug, Deserialize)]
struct Cycle5PanelMatchupRowV1 {
...
#[derive(Clone, Debug, Deserialize)]
struct Cycle5PanelDocForBuildV1 {
    schema: String,
    manifest_sha256: String,
    rank_sums: Vec<Cycle5PanelRankSumEntryV1>,
    matchups: Vec<Cycle5PanelMatchupRowV1>,
}
...
/// summary for logging.
#[derive(Clone, Debug)]
pub struct Cycle5RefreshBuildResultV1 {
    pub canonical_bytes: Vec<u8>,
    pub manifest_sha256: String,
    pub refresh_index: u64,
    pub trainee_local_generation: u64,
    pub weight_units: [u64; CYCLE5_SLOT_COUNT_V1],
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum Cycle5RefreshBuildErrorKindV1 {
    ...
    /// `Cycle5RefreshManifestErrorKindV1` (a crate-private type this
    /// module's public interface cannot name directly).
    ManifestRejected(String),
}

#[derive(Clone, Debug)]
pub struct Cycle5RefreshBuildErrorV1 {
    kind: Cycle5RefreshBuildErrorKindV1,
}

impl Cycle5RefreshBuildErrorV1 {
    const fn new(kind: Cycle5RefreshBuildErrorKindV1) -> Self {
        Self { kind }
    }

    pub const fn kind_v1(&self) -> &Cycle5RefreshBuildErrorKindV1 {
        &self.kind
    }
}

impl Display for Cycle5RefreshBuildErrorV1 {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "{:?}", self.kind)
    }
}

impl Error for Cycle5RefreshBuildErrorV1 {}

impl From<Cycle5RefreshManifestErrorV1> for Cycle5RefreshBuildErrorV1 {
    fn from(error: Cycle5RefreshManifestErrorV1) -> Self {
        Self::new(Cycle5RefreshBuildErrorKindV1::ManifestRejected(format!(
            "{:?}",
            error.kind_v1()
        )))
    }
...
type Result<T> = std::result::Result<T, Cycle5RefreshBuildErrorV1>;

fn invalid_slot_identities_v1() -> Cycle5RefreshBuildErrorV1 {
    Cycle5RefreshBuildErrorV1::new(Cycle5RefreshBuildErrorKindV1::InvalidSlotIdentitiesDocument)
}

fn invalid_panel_v1() -> Cycle5RefreshBuildErrorV1 {
    Cycle5RefreshBuildErrorV1::new(Cycle5RefreshBuildErrorKindV1::InvalidPanelDocument)
}

fn panel_manifest_mismatch_v1() -> Cycle5RefreshBuildErrorV1 {
    Cycle5RefreshBuildErrorV1::new(Cycle5RefreshBuildErrorKindV1::PanelManifestMismatch)
}

fn empty_chain_v1() -> Cycle5RefreshBuildErrorV1 {
    Cycle5RefreshBuildErrorV1::new(Cycle5RefreshBuildErrorKindV1::EmptyChain)
}

fn malformed_chain_v1() -> Cycle5RefreshBuildErrorV1 {
    Cycle5RefreshBuildErrorV1::new(Cycle5RefreshBuildErrorKindV1::MalformedChain)
}
...
fn ordered_slot_identities_v1(
    json_bytes: &[u8],
) -> Result<[Cycle5SlotIdentityInputV1; CYCLE5_SLOT_COUNT_V1]> {
    let file: Cycle5SlotIdentitiesFileV1 =
        serde_json::from_slice(json_bytes).map_err(|_| invalid_slot_identities_v1())?;
    if file.schema != CYCLE5_SLOT_IDENTITIES_SCHEMA_V1 || file.slots.len() != CYCLE5_SLOT_COUNT_V1 {
        return Err(invalid_slot_identities_v1());
    }
    let mut ordered: [Option<Cycle5SlotIdentityInputV1>; CYCLE5_SLOT_COUNT_V1] =
        [None, None, None, None, None, None, None, None];
    for entry in file.slots {
        let index = usize::try_from(entry.slot_index).map_err(|_| invalid_slot_identities_v1())?;
        if index >= CYCLE5_SLOT_COUNT_V1 || ordered[index].is_some() {
            return Err(invalid_slot_identities_v1());
        }
...
fn ordered_declared_rank_sums_v1(
    entries: &[Cycle5PanelRankSumEntryV1],
) -> Result<[i64; CYCLE5_SLOT_COUNT_V1]> {
    if entries.len() != CYCLE5_SLOT_COUNT_V1 {
        return Err(invalid_panel_v1());
    }
    let mut ordered: [Option<i64>; CYCLE5_SLOT_COUNT_V1] = [None; CYCLE5_SLOT_COUNT_V1];
    ...
    let mut result = [0_i64; CYCLE5_SLOT_COUNT_V1];
...
/// `CYCLE5_MATCHUP_COUNT_V1`-row matchup ledger: win `+1`, draw `0`, loss
...
/// losses` equal to `CYCLE5_PANEL_GAMES_PER_MATCHUP_V1`, the two recorded
...
/// `CYCLE5_PANEL_GAMES_PER_POLICY_V1` (`7 * G`). Fails closed on any
/// disagreement.
fn recomputed_rank_sums_v1(
    matchups: &[Cycle5PanelMatchupRowV1],
) -> Result<[i64; CYCLE5_SLOT_COUNT_V1]> {
    if matchups.len() != CYCLE5_MATCHUP_COUNT_V1 {
        return Err(invalid_panel_v1());
    }
    let games_per_matchup =
        i64::try_from(CYCLE5_PANEL_GAMES_PER_MATCHUP_V1).map_err(|_| invalid_panel_v1())?;
    let expected_total =
        i64::try_from(CYCLE5_PANEL_GAMES_PER_POLICY_V1).map_err(|_| invalid_panel_v1())?;
    let mut totals = [0_i64; CYCLE5_SLOT_COUNT_V1];
    let mut rank_sums = [0_i64; CYCLE5_SLOT_COUNT_V1];
    ...
    for row in matchups {
        ...
        if lower >= CYCLE5_SLOT_COUNT_V1 || higher >= CYCLE5_SLOT_COUNT_V1 || lower >= higher {
            return Err(invalid_panel_v1());
        }
        if !seen_pairs.insert((lower, higher)) {
            return Err(invalid_panel_v1());
        }
        if row.game_count != CYCLE5_PANEL_GAMES_PER_MATCHUP_V1
            || row.lower_wins < 0
            || row.lower_draws < 0
            || row.lower_losses < 0
            || row.higher_wins < 0
...
/// passing its raw bytes through to `build_cycle5_refresh_manifest_v1`
/// unparsed.
fn validated_panel_rank_sums_v1(
    panel_bytes: &[u8],
    chain_tip_manifest_sha256_hex: &str,
) -> Result<[i64; CYCLE5_SLOT_COUNT_V1]> {
    let doc: Cycle5PanelDocForBuildV1 =
        serde_json::from_slice(panel_bytes).map_err(|_| invalid_panel_v1())?;
    if doc.schema != CYCLE5_PANEL_DOC_SCHEMA_V1 {
        return Err(invalid_panel_v1());
    }
...
fn build_slot_records_v1(
    identities: &[Cycle5SlotIdentityInputV1; CYCLE5_SLOT_COUNT_V1],
    weight_units: &[u64; CYCLE5_SLOT_COUNT_V1],
) -> Vec<Cycle5RefreshSlotV1> {
    identities
        .iter()
        .enumerate()
        .map(|(index, identity)| Cycle5RefreshSlotV1 {
...
pub fn build_cycle5_genesis_refresh_v1(
    trainee_run_sha256: &str,
    trainee_base_seed: u64,
    slot_identities_json: &[u8],
) -> Result<Cycle5RefreshBuildResultV1> {
    let identities = ordered_slot_identities_v1(slot_identities_json)?;
    let weight_units = [CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1; CYCLE5_SLOT_COUNT_V1];
...
/// `build_cycle5_next_refresh_v1`) and `Some` for every later link;
/// violating either fails closed with `MalformedChain` before any link's
/// bytes are decoded.
#[derive(Clone, Debug)]
pub struct Cycle5ChainLinkV1 {
...
/// `decode_cycle5_refresh_manifest_v1` -- so a chain directory produced by
...
/// validated tip.
fn decode_and_validate_chain_v1(chain: &[Cycle5ChainLinkV1]) -> Result<Cycle5RefreshManifestV1> {
    let mut previous: Option<Cycle5RefreshManifestV1> = None;
...
/// (`chain`, per `Cycle5ChainLinkV1`'s doc -- this replaces trusting a
...
pub fn build_cycle5_next_refresh_v1(
    chain: &[Cycle5ChainLinkV1],
    panel_bytes: &[u8],
    ...
    let mut panel_score_fractions = [0.0_f64; CYCLE5_SLOT_COUNT_V1];
    for (index, rank_sum) in rank_sums.into_iter().enumerate() {
        panel_score_fractions[index] = panel_score_fraction_cycle5_v1(rank_sum)?;
    }
    let mut prior_weight_units = [0_u64; CYCLE5_SLOT_COUNT_V1];
    for (index, slot) in tip.slots_v1().iter().enumerate() {
        prior_weight_units[index] = slot.weight_units;
    }
    let weight_units = mw_update_cycle5_v1(&prior_weight_units, &panel_score_fractions)?;
    let identities = ordered_slot_identities_v1(slot_identities_json)?;
    let slots = build_slot_records_v1(&identities, &weight_units);
    let manifest = build_cycle5_refresh_manifest_v1(
        next_refresh_index,
...
mod tests {
    ...
    use super::*;
    use crate::native_population_refresh_manifest_cycle5_v1::{
        FrozenOccupantIdentityCycle5V1, CYCLE5_ANCHOR_0_V1, CYCLE5_ANCHOR_1_V1,
        CYCLE5_CURRENT_0_V1, CYCLE5_CYCLE3_LINEAGE_BASE_SEED_V1,
        CYCLE5_CYCLE3_LINEAGE_RUN_SHA256_V1, CYCLE5_EXPLOITER_0_V1, CYCLE5_EXPLOITER_1_V1,
        CYCLE5_HISTORICAL_1_ROTATION_V1, CYCLE5_HISTORICAL_LAG_V1, CYCLE5_ROLE_FLOOR_UNITS_V1,
        CYCLE5_TRAINEE_START_LOCAL_GENERATION_V1, CYCLE5_WEIGHT_TOTAL_UNITS_V1,
    };
    ...
    fn slot_identities_json_v1(
        ...
        let local_generation = CYCLE5_TRAINEE_START_LOCAL_GENERATION_V1 + refresh_index * 128;
        let historical_0_generation = local_generation - CYCLE5_HISTORICAL_LAG_V1;
        ...
        let doc = serde_json::json!({
            "schema": CYCLE5_SLOT_IDENTITIES_SCHEMA_V1,
            "slots": [
                frozen_identity_json_v1(0, &CYCLE5_ANCHOR_0_V1),
                frozen_identity_json_v1(1, &CYCLE5_ANCHOR_1_V1),
                historical_0,
                frozen_identity_json_v1(3, &CYCLE5_HISTORICAL_1_ROTATION_V1[rotation]),
                frozen_identity_json_v1(4, &CYCLE5_CURRENT_0_V1),
                trainee_bound_identity_json_v1(5, trainee_run, trainee_seed, local_generation, 32),
                frozen_identity_json_v1(6, &CYCLE5_EXPLOITER_0_V1),
                frozen_identity_json_v1(7, &CYCLE5_EXPLOITER_1_V1),
            ],
        });
    ...
    /// (`itertools.combinations`): exactly `CYCLE5_MATCHUP_COUNT_V1` pairs.
    fn all_pairs_v1() -> Vec<(usize, usize)> {
        let mut pairs = Vec::with_capacity(CYCLE5_MATCHUP_COUNT_V1);
        for lower in 0..CYCLE5_SLOT_COUNT_V1 {
            for higher in (lower + 1)..CYCLE5_SLOT_COUNT_V1 {
    ...
    fn matchup_row_v1(
        ...
        serde_json::json!({
            ...
            "game_count": CYCLE5_PANEL_GAMES_PER_MATCHUP_V1,
    ...
    /// Builds the full `CYCLE5_MATCHUP_COUNT_V1`-row matchup ledger, every
    /// pair drawn all `CYCLE5_PANEL_GAMES_PER_MATCHUP_V1` games by default,
    ...
    ) -> (Vec<serde_json::Value>, [i64; CYCLE5_SLOT_COUNT_V1]) {
        let games = i64::try_from(CYCLE5_PANEL_GAMES_PER_MATCHUP_V1).expect("256 fits i64");
        let mut rows = Vec::with_capacity(CYCLE5_MATCHUP_COUNT_V1);
        let mut rank_sums = [0_i64; CYCLE5_SLOT_COUNT_V1];
    ...
    fn panel_json_v1(
        manifest_sha256: &str,
        overrides: &[MatchupOverrideV1],
    ) -> (Vec<u8>, [i64; CYCLE5_SLOT_COUNT_V1]) {
        let (matchups, rank_sums) = matchup_ledger_v1(overrides);
        let doc = serde_json::json!({
            "schema": CYCLE5_PANEL_DOC_SCHEMA_V1,
    ...
    fn rank_sum_entries_v1(rank_sums: &[i64; CYCLE5_SLOT_COUNT_V1]) -> Vec<serde_json::Value> {
        rank_sums
            .iter()
            .enumerate()
    ...
    fn chain_link_v1(manifest_bytes: &[u8], panel_bytes: Option<&[u8]>) -> Cycle5ChainLinkV1 {
        Cycle5ChainLinkV1 {
    ...
    fn genesis_rejects_malformed_slot_identities_v1() {
        ...
        let error = build_cycle5_genesis_refresh_v1(TEST_TRAINEE_RUN, TEST_TRAINEE_SEED, &bad)
            .expect_err("malformed slot identities");
        assert_eq!(
            *error.kind_v1(),
            Cycle5RefreshBuildErrorKindV1::InvalidSlotIdentitiesDocument
        );
    }
    ...
    fn genesis_rejects_wrong_slot_identities_schema_v1() {
        ...
        let error = build_cycle5_genesis_refresh_v1(TEST_TRAINEE_RUN, TEST_TRAINEE_SEED, &bytes)
    ...
    fn next_refresh_moves_weight_toward_winners_within_constraints_v1() {
        ...
        let genesis = build_cycle5_genesis_refresh_v1(
            TEST_TRAINEE_RUN,
            TEST_TRAINEE_SEED,
            &genesis_identities,
        )
        ...
        let next = build_cycle5_next_refresh_v1(
            &chain,
            &panel,
            1,
            TEST_TRAINEE_RUN,
    ...
    fn next_refresh_chains_through_multiple_links_v1() {
        ...
        let refresh_one = build_cycle5_next_refresh_v1(
        ...
        let refresh_two = build_cycle5_next_refresh_v1(
            &chain_to_one,
            &panel_two,
            2,
            TEST_TRAINEE_RUN,
    ...
    fn next_refresh_rejects_empty_chain_v1() {
        ...
        let next_identities = slot_identities_json_v1(1, TEST_TRAINEE_RUN, TEST_TRAINEE_SEED);
        let error = build_cycle5_next_refresh_v1(
            &[],
            &panel,
            1,
            TEST_TRAINEE_RUN,
        ...
        assert_eq!(*error.kind_v1(), Cycle5RefreshBuildErrorKindV1::EmptyChain);
    }
    ...
    fn next_refresh_rejects_genesis_link_carrying_a_panel_v1() {
        ...
        assert_eq!(
            *error.kind_v1(),
            Cycle5RefreshBuildErrorKindV1::MalformedChain
        );
    }
    ...
    fn next_refresh_rejects_trainee_run_drift_against_the_chained_manifest_v1() {
        ...
        let genesis_b =
            build_cycle5_genesis_refresh_v1(&other_run, TEST_TRAINEE_SEED, &genesis_b_identities)
                .expect("genesis b");
        ...
        assert_eq!(
            *error.kind_v1(),
            Cycle5RefreshBuildErrorKindV1::ManifestRejected("InvalidChain".to_owned())
        );
    }
    ...
    fn next_refresh_rejects_panel_bound_to_a_different_manifest_v1() {
        ...
        let genesis_a = build_cycle5_genesis_refresh_v1(
            TEST_TRAINEE_RUN,
            TEST_TRAINEE_SEED,
        ...
        assert_eq!(
            *error.kind_v1(),
            Cycle5RefreshBuildErrorKindV1::PanelManifestMismatch
        );
    }
    ...
    fn next_refresh_rejects_malformed_panel_document_v1() {
        ...
        assert_eq!(
            *error.kind_v1(),
            Cycle5RefreshBuildErrorKindV1::InvalidPanelDocument
        );
    }
    ...
    fn next_refresh_rejects_rank_sums_disagreeing_with_the_matchup_ledger_v1() {
        ...
        assert_eq!(rank_sums, [0; CYCLE5_SLOT_COUNT_V1]);
    ...
    fn chain_filenames_follow_the_fixed_naming_scheme_v1() {
        assert_eq!(
            cycle5_chain_manifest_filename_v1(0),
            "refresh-00.manifest.json"
        );
        assert_eq!(
            cycle5_chain_manifest_filename_v1(16),
            "refresh-16.manifest.json"
        );
        assert_eq!(cycle5_chain_panel_filename_v1(1), "refresh-01.panel.json");
        assert_eq!(cycle5_chain_panel_filename_v1(16), "refresh-16.panel.json");
    }
}
