...
fn validate_tranche_refresh_authority_and_generation_v2(
    ...
    if wire.schema != expected_schema
        ...
        || wire.program_document_sha256_v2_proposed
...
mod tests {
    ...
    mod tranche_refresh_v2 {
        ...
        fn second_link_chains_onto_the_first() {
            let initial =
                decode_population_tranche_refresh_manifest_v2(&initial_bytes(), None).unwrap();
            let previous_sha = lower_hex_raw32_v1(initial.manifest_sha256_v2());
            let next_bytes =
                tranche_manifest_v2(1, 128, Some(previous_sha), Some(tranche_digest(2)), |_| {});
            let next =
                decode_population_tranche_refresh_manifest_v2(&next_bytes, Some(&initial)).unwrap();
        ...
        fn refresh_index_34_the_new_cap_itself_is_accepted() {
            let bytes = tranche_manifest_v2(
                34,
                34 * 128,
                Some(tranche_digest(9)),
        ...
        fn manifest_with_search_slot_v2(
            action_seed: u64,
            tier: KernelNativeSearchTierV1,
            weight_units: u64,
        ) -> Vec<u8> {
        ...
        fn archived_cycle2_refresh_003_decodes_through_the_ported_v2_path() {
            ...
            let decoded = decode_population_tranche_refresh_manifest_v2_current_only(&bytes)
                .expect(
                "the archived cycle-2 refresh-003 record must decode through the ported v2 path",
            );
        ...
        fn task7_preflight_all_16_cycle3_manifests_decode_and_chain() {
            ...
            for idx in 1_u64..=34 {
                ...
                let bytes = std::fs::read(&path).unwrap_or_else(|_| {
                    panic!("{path} must be present (run Task 6's authoring script first)")
                });
