//! Canonical refresh authority for the cycle-5 population campaign.
//!
//! Versioned successor of `native_population_refresh_manifest_v1`; it
//! reinterprets nothing retroactively. Contract:
//! `docs/native_population_refresh_manifest_cycle5_v1.md` under the ratified
//! pre-registration `OX_CYCLE5_PREREG_SKETCH_V2.md`. The cycle-3 lesson is
...
//! (`mw_update_cycle5_v1`) runs on one designated machine; cross-platform
...
/// The schema and pre-registration literals are the run-record module's:
/// one definition, so the manifest can never disagree with the record that
/// names it. `CYCLE5_PREREG_SHA256_V1` is an UNRATIFIED sentinel in
/// production builds (see that module).
pub(crate) use crate::native_training_store_run_v2::{
    CYCLE5_PREREG_SHA256_V1, CYCLE5_REFRESH_MANIFEST_SCHEMA_V1,
};
/// ASSUMPTION: carried from cycle 4 pending the sketch's per-arm scope ruling.
pub(crate) const CYCLE5_REFRESH_INTERVAL_V1: u64 = 128;
pub(crate) const CYCLE5_REFRESH_MAX_INDEX_V1: u64 = 16;
pub(crate) const CYCLE5_SLOT_COUNT_V1: usize = 8;
pub(crate) const CYCLE5_WEIGHT_TOTAL_UNITS_V1: u64 = 1_000_000;
pub(crate) const CYCLE5_ROLE_FLOOR_UNITS_V1: u64 = 200_000;
pub(crate) const CYCLE5_POLICY_CAP_UNITS_V1: u64 = 250_000;
pub(crate) const CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1: u64 = 125_000;
/// The routed parent generation (the cycle-3 lineage run at its own 2048),
/// which is also the trainee-local label of the arm's genesis.
pub(crate) const CYCLE5_TRAINEE_START_LOCAL_GENERATION_V1: u64 = 2_048;

/// Whether the frozen roster below is the ratified cycle-5 roster.
///
/// ASSUMPTION carried as the roster VALUES: the cycle-4 frozen eight-identity
/// genesis pool, which `OX_CYCLE5_PREREG_SKETCH_V1.md` open decision 3 offers
/// as the default option. Until the owner ratifies a roster this is `false`
/// in production builds and every manifest build or decode fails closed
/// with `RosterUnratified`; the `cfg(test)` twin lets the machinery be
/// tested against these values. Flip it only in the same commit that
/// replaces the values with the ratified ones.
#[cfg(not(test))]
pub(crate) const CYCLE5_ROSTER_RATIFIED_V1: bool = false;
#[cfg(test)]
pub(crate) const CYCLE5_ROSTER_RATIFIED_V1: bool = true;
pub(crate) const CYCLE5_HISTORICAL_LAG_V1: u64 = 512;
/// 28 matchups, `G = 256` games per matchup; each policy plays `7 * G` games.
pub(crate) const CYCLE5_PANEL_GAMES_PER_MATCHUP_V1: u64 = 256;
pub(crate) const CYCLE5_PANEL_GAMES_PER_POLICY_V1: u64 = 7 * CYCLE5_PANEL_GAMES_PER_MATCHUP_V1;
pub(crate) const CYCLE5_MW_ETA_V1: f64 = 0.10;
...
pub(crate) const CYCLE5_PLACEHOLDER_ZERO_PREFIX_V1: usize = 48;

/// Cycle-3 trainee lineage (historical-0's source at refresh indices 0..=3).
pub(crate) const CYCLE5_CYCLE3_LINEAGE_RUN_SHA256_V1: &str =
    "f25a63d0a2968016c2d44220b02d46b642fad5c4d524cd7ed82d699dbfda83a1";
pub(crate) const CYCLE5_CYCLE3_LINEAGE_BASE_SEED_V1: u64 = 977_002;

const EXPECTED_ROLES_CYCLE5_V1: [&str; CYCLE5_SLOT_COUNT_V1] = [
    "anchor-0",
    "anchor-1",
    "historical-0",
    "historical-1",
...
pub(crate) struct FrozenOccupantIdentityCycle5V1 {
...
pub(crate) const CYCLE5_ANCHOR_0_V1: FrozenOccupantIdentityCycle5V1 =
    FrozenOccupantIdentityCycle5V1 {
...
pub(crate) const CYCLE5_ANCHOR_1_V1: FrozenOccupantIdentityCycle5V1 =
    FrozenOccupantIdentityCycle5V1 {
...
pub(crate) const CYCLE5_HISTORICAL_1_ROTATION_V1: [FrozenOccupantIdentityCycle5V1; 3] = [
    FrozenOccupantIdentityCycle5V1 {
...
pub(crate) const CYCLE5_CURRENT_0_V1: FrozenOccupantIdentityCycle5V1 =
    FrozenOccupantIdentityCycle5V1 {
...
pub(crate) const CYCLE5_EXPLOITER_0_V1: FrozenOccupantIdentityCycle5V1 =
    FrozenOccupantIdentityCycle5V1 {
...
pub(crate) const CYCLE5_EXPLOITER_1_V1: FrozenOccupantIdentityCycle5V1 =
    FrozenOccupantIdentityCycle5V1 {
...
pub(crate) struct Cycle5RefreshSlotV1 {
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
struct Cycle5RefreshManifestWireV1 {
    schema: String,
    ...
    slots: Vec<Cycle5RefreshSlotV1>,
}

#[derive(Clone, Debug)]
pub(crate) struct Cycle5RefreshManifestV1 {
    wire: Cycle5RefreshManifestWireV1,
    canonical_bytes: Vec<u8>,
    manifest_sha256: [u8; 32],
}

impl Cycle5RefreshManifestV1 {
    ...
    pub(crate) fn slots_v1(&self) -> &[Cycle5RefreshSlotV1] {
        &self.wire.slots
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum Cycle5RefreshManifestErrorKindV1 {
    ...
    MwArithmetic,
    /// The compiled roster is not the ratified cycle-5 roster.
    RosterUnratified,
}

#[derive(Clone, Copy, Debug)]
pub(crate) struct Cycle5RefreshManifestErrorV1 {
    kind: Cycle5RefreshManifestErrorKindV1,
}

impl Cycle5RefreshManifestErrorV1 {
    const fn new(kind: Cycle5RefreshManifestErrorKindV1) -> Self {
        Self { kind }
    }

    pub(crate) const fn kind_v1(self) -> Cycle5RefreshManifestErrorKindV1 {
        self.kind
    }
}

impl From<CanonicalJsonErrorV1> for Cycle5RefreshManifestErrorV1 {
    fn from(error: CanonicalJsonErrorV1) -> Self {
        Self::new(Cycle5RefreshManifestErrorKindV1::CanonicalJson(
            error.kind(),
        ))
    }
}

impl Display for Cycle5RefreshManifestErrorV1 {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "{:?}", self.kind)
    }
}

impl Error for Cycle5RefreshManifestErrorV1 {}

type Result<T> = std::result::Result<T, Cycle5RefreshManifestErrorV1>;

fn is_sha256_cycle5_v1(value: &str) -> bool {
    value.len() == 64
        && value
            .bytes()
...
pub(crate) fn is_placeholder_hash_cycle5_v1(value: &str) -> bool {
    is_sha256_cycle5_v1(value)
        && value
            .bytes()
            .take(CYCLE5_PLACEHOLDER_ZERO_PREFIX_V1)
            .all(|byte| byte == b'0')
}

#[allow(clippy::too_many_arguments)]
pub(crate) fn build_cycle5_refresh_manifest_v1(
    refresh_index: u64,
    previous: Option<&Cycle5RefreshManifestV1>,
    payoff_panel_bytes: Option<&[u8]>,
    trainee_run_sha256: &str,
    trainee_base_seed: u64,
    slots: Vec<Cycle5RefreshSlotV1>,
) -> Result<Cycle5RefreshManifestV1> {
    require_roster_ratified_v1()?;
    let program_update = refresh_index
        .checked_mul(CYCLE5_REFRESH_INTERVAL_V1)
        .ok_or_else(|| {
            Cycle5RefreshManifestErrorV1::new(Cycle5RefreshManifestErrorKindV1::InvalidGeneration)
        })?;
    ...
    let wire = Cycle5RefreshManifestWireV1 {
        schema: CYCLE5_REFRESH_MANIFEST_SCHEMA_V1.to_owned(),
        ...
        trainee_base_seed,
        weight_total_units: CYCLE5_WEIGHT_TOTAL_UNITS_V1,
    ...
    decode_cycle5_refresh_manifest_v1(&bytes, previous, payoff_panel_bytes)
}
...
pub(crate) fn decode_cycle5_refresh_manifest_v1(
    bytes: &[u8],
    previous: Option<&Cycle5RefreshManifestV1>,
    payoff_panel_bytes: Option<&[u8]>,
) -> Result<Cycle5RefreshManifestV1> {
    ...
    if reencoded != bytes {
        return Err(Cycle5RefreshManifestErrorV1::new(
            Cycle5RefreshManifestErrorKindV1::InvalidAuthority,
        ));
    }
    validate_wire_cycle5_v1(&wire, previous, payoff_panel_bytes)?;
    Ok(Cycle5RefreshManifestV1 {
...
fn validate_wire_cycle5_v1(
    wire: &Cycle5RefreshManifestWireV1,
    previous: Option<&Cycle5RefreshManifestV1>,
    payoff_panel_bytes: Option<&[u8]>,
) -> Result<()> {
    if wire.schema != CYCLE5_REFRESH_MANIFEST_SCHEMA_V1
        || wire.prereg_sha256 != CYCLE5_PREREG_SHA256_V1
        || !is_sha256_cycle5_v1(&wire.trainee_run_sha256)
    {
        return Err(Cycle5RefreshManifestErrorV1::new(
            Cycle5RefreshManifestErrorKindV1::InvalidAuthority,
        ));
    ...
    let expected_local_generation = CYCLE5_TRAINEE_START_LOCAL_GENERATION_V1
        .checked_add(expected_program_update)
        .ok_or_else(|| {
            Cycle5RefreshManifestErrorV1::new(Cycle5RefreshManifestErrorKindV1::InvalidGeneration)
        })?;
    if wire.refresh_index > CYCLE5_REFRESH_MAX_INDEX_V1
        || wire.program_update != expected_program_update
        || wire.trainee_local_generation != expected_local_generation
    {
        return Err(Cycle5RefreshManifestErrorV1::new(
            Cycle5RefreshManifestErrorKindV1::InvalidGeneration,
        ));
    }
    match (wire.refresh_index, previous) {
        ...
        (0, _) => {
            return Err(Cycle5RefreshManifestErrorV1::new(
                Cycle5RefreshManifestErrorKindV1::InvalidChain,
            ));
        }
        (_, Some(previous_manifest)) => {
            ...
            let declared_panel = wire.payoff_panel_sha256.as_deref().ok_or_else(|| {
                Cycle5RefreshManifestErrorV1::new(Cycle5RefreshManifestErrorKindV1::InvalidChain)
            })?;
            if !is_sha256_cycle5_v1(declared_panel) {
                return Err(Cycle5RefreshManifestErrorV1::new(
                    Cycle5RefreshManifestErrorKindV1::InvalidChain,
                ));
            }
            if is_placeholder_hash_cycle5_v1(declared_panel) {
                return Err(Cycle5RefreshManifestErrorV1::new(
                    Cycle5RefreshManifestErrorKindV1::PlaceholderPanelHash,
                ));
            }
            let panel_bytes = payoff_panel_bytes.ok_or_else(|| {
                Cycle5RefreshManifestErrorV1::new(
                    Cycle5RefreshManifestErrorKindV1::MissingPanelBytes,
                )
            })?;
            if lower_hex_raw32_v1(sha256_v1(panel_bytes)) != declared_panel {
                return Err(Cycle5RefreshManifestErrorV1::new(
                    Cycle5RefreshManifestErrorKindV1::PanelContentMismatch,
                ));
            }
        }
        (_, None) => {
    ...
    validate_slots_cycle5_v1(wire)
}

fn slot_matches_frozen_v1(
    slot: &Cycle5RefreshSlotV1,
...
fn validate_slots_cycle5_v1(wire: &Cycle5RefreshManifestWireV1) -> Result<()> {
    if wire.slots.len() != CYCLE5_SLOT_COUNT_V1
        || wire.weight_total_units != CYCLE5_WEIGHT_TOTAL_UNITS_V1
    {
        return Err(Cycle5RefreshManifestErrorV1::new(
            Cycle5RefreshManifestErrorKindV1::InvalidSlots,
        ));
    }
    let invalid =
        || Cycle5RefreshManifestErrorV1::new(Cycle5RefreshManifestErrorKindV1::InvalidSlots);
    ...
    for (index, slot) in wire.slots.iter().enumerate() {
        ...
        if slot.slot_index != expected_index
            || slot.role != EXPECTED_ROLES_CYCLE5_V1[index]
            || slot.occupant_class != expected_class
            || !is_sha256_cycle5_v1(&slot.source_run_sha256)
            || !is_sha256_cycle5_v1(&slot.checkpoint_manifest_sha256)
            || !is_sha256_cycle5_v1(&slot.checkpoint_payload_sha256)
            || !is_sha256_cycle5_v1(&slot.model_parameter_sha256)
            || !is_sha256_cycle5_v1(&slot.train_state_sha256)
        {
            return Err(invalid());
        }
        validate_slot_assignment_cycle5_v1(wire, index, slot)?;
        if slot.weight_units == 0 || slot.weight_units > CYCLE5_POLICY_CAP_UNITS_V1 {
            return Err(Cycle5RefreshManifestErrorV1::new(
                Cycle5RefreshManifestErrorKindV1::InvalidWeight,
            ));
        }
        if wire.refresh_index == 0 && slot.weight_units != CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1 {
            return Err(Cycle5RefreshManifestErrorV1::new(
                Cycle5RefreshManifestErrorKindV1::InvalidWeight,
            ));
        }
        weight_sum = weight_sum.checked_add(slot.weight_units).ok_or_else(|| {
            Cycle5RefreshManifestErrorV1::new(Cycle5RefreshManifestErrorKindV1::InvalidWeight)
        })?;
    ...
    if weight_sum != CYCLE5_WEIGHT_TOTAL_UNITS_V1
        || role_weights
            .iter()
            .any(|weight| *weight < CYCLE5_ROLE_FLOOR_UNITS_V1)
    {
        return Err(Cycle5RefreshManifestErrorV1::new(
            Cycle5RefreshManifestErrorKindV1::InvalidWeight,
        ));
...
/// Every build and decode passes through here first: a production build
/// carrying an unratified roster can neither author nor accept a manifest.
fn require_roster_ratified_v1() -> Result<()> {
    if !CYCLE5_ROSTER_RATIFIED_V1 {
        return Err(Cycle5RefreshManifestErrorV1::new(
            Cycle5RefreshManifestErrorKindV1::RosterUnratified,
        ));
    }
    Ok(())
}

fn validate_slot_assignment_cycle5_v1(
    wire: &Cycle5RefreshManifestWireV1,
    index: usize,
    slot: &Cycle5RefreshSlotV1,
) -> Result<()> {
    ...
    match index {
        0 => {
            if !slot_matches_frozen_v1(slot, &CYCLE5_ANCHOR_0_V1) {
                return Err(invalid());
            }
        }
        1 => {
            if !slot_matches_frozen_v1(slot, &CYCLE5_ANCHOR_1_V1) {
                return Err(invalid());
            }
        }
        2 => {
            let lagged_generation = wire
                .trainee_local_generation
                .checked_sub(CYCLE5_HISTORICAL_LAG_V1)
                .ok_or_else(invalid)?;
        ...
        3 => {
            ...
            if !slot_matches_frozen_v1(slot, &CYCLE5_HISTORICAL_1_ROTATION_V1[rotation_index]) {
                return Err(invalid());
            }
        }
        4 => {
            if !slot_matches_frozen_v1(slot, &CYCLE5_CURRENT_0_V1) {
                return Err(invalid());
            }
        }
        5 => {
        ...
        6 => {
            if !slot_matches_frozen_v1(slot, &CYCLE5_EXPLOITER_0_V1) {
                return Err(invalid());
            }
        }
        7 => {
            if !slot_matches_frozen_v1(slot, &CYCLE5_EXPLOITER_1_V1) {
                return Err(invalid());
            }
        }
...
pub(crate) fn panel_score_fraction_cycle5_v1(rank_sum: i64) -> Result<f64> {
    let games = i64::try_from(CYCLE5_PANEL_GAMES_PER_POLICY_V1).map_err(|_| {
        Cycle5RefreshManifestErrorV1::new(Cycle5RefreshManifestErrorKindV1::MwArithmetic)
    })?;
    // Range comparison rather than `abs()`: `i64::MIN.abs()` would overflow.
    if !(-games..=games).contains(&rank_sum) {
        return Err(Cycle5RefreshManifestErrorV1::new(
            Cycle5RefreshManifestErrorKindV1::MwArithmetic,
        ));
    }
...
/// The v1 multiplicative-weights update with the cycle-5 panel size:
...
pub(crate) fn mw_update_cycle5_v1(
    prior_weight_units: &[u64; CYCLE5_SLOT_COUNT_V1],
    panel_score_fractions: &[f64; CYCLE5_SLOT_COUNT_V1],
) -> Result<[u64; CYCLE5_SLOT_COUNT_V1]> {
    let arithmetic =
        || Cycle5RefreshManifestErrorV1::new(Cycle5RefreshManifestErrorKindV1::MwArithmetic);
    let cap = 0.25_f64;
    let role_floor = 0.20_f64;
    let mut weights = [0.0_f64; CYCLE5_SLOT_COUNT_V1];
    let mut sum = 0.0_f64;
    for index in 0..CYCLE5_SLOT_COUNT_V1 {
        ...
        #[allow(clippy::cast_precision_loss)]
        let raw = prior_weight_units[index] as f64 * (CYCLE5_MW_ETA_V1 * fraction).exp();
    ...
    #[allow(clippy::cast_precision_loss)]
    let total = CYCLE5_WEIGHT_TOTAL_UNITS_V1 as f64;
    let mut units = [0_u64; CYCLE5_SLOT_COUNT_V1];
    let mut remainders: Vec<(usize, f64)> = Vec::with_capacity(CYCLE5_SLOT_COUNT_V1);
    ...
    let missing = CYCLE5_WEIGHT_TOTAL_UNITS_V1
        .checked_sub(assigned)
        .ok_or_else(arithmetic)?;
    let missing_count = usize::try_from(missing).map_err(|_| arithmetic())?;
    if missing_count > CYCLE5_SLOT_COUNT_V1 {
        return Err(arithmetic());
    }
    ...
    for deficient in 0..4 {
        while role_units[deficient] < CYCLE5_ROLE_FLOOR_UNITS_V1 {
            let deficit_slot =
            ...
            let donor_role = role_units
                ...
                .filter(|(role_index, role)| {
                    *role_index != deficient && **role > CYCLE5_ROLE_FLOOR_UNITS_V1
                })
    ...
    if final_sum != CYCLE5_WEIGHT_TOTAL_UNITS_V1
        || units
            .iter()
            .any(|unit| *unit > CYCLE5_POLICY_CAP_UNITS_V1 || *unit == 0)
    {
        return Err(arithmetic());
    }
    Ok(units)
...
mod tests {
    ...
    const TEST_TRAINEE_RUN: &str = CYCLE5_CYCLE3_LINEAGE_RUN_SHA256_V1;
    const TEST_TRAINEE_SEED: u64 = CYCLE5_CYCLE3_LINEAGE_BASE_SEED_V1;
    ...
    fn slots_for(refresh_index: u64, weight: u64) -> Vec<Cycle5RefreshSlotV1> {
        let local = CYCLE5_TRAINEE_START_LOCAL_GENERATION_V1 + refresh_index * 128;
        let rotation = usize::try_from(refresh_index % 3).expect("rotation");
        vec![
            frozen_slot(0, "anchor-0", &CYCLE5_ANCHOR_0_V1, weight),
            frozen_slot(1, "anchor-1", &CYCLE5_ANCHOR_1_V1, weight),
            trainee_slot(
                2,
                "historical-0",
                if refresh_index <= 3 {
                ...
                local - CYCLE5_HISTORICAL_LAG_V1,
                16,
                weight,
            ),
            frozen_slot(
                3,
                "historical-1",
                &CYCLE5_HISTORICAL_1_ROTATION_V1[rotation],
                weight,
            ),
            frozen_slot(4, "current-0", &CYCLE5_CURRENT_0_V1, weight),
            trainee_slot(
                5,
                "current-1",
                TEST_TRAINEE_RUN,
            ...
            frozen_slot(6, "exploiter-0", &CYCLE5_EXPLOITER_0_V1, weight),
            frozen_slot(7, "exploiter-1", &CYCLE5_EXPLOITER_1_V1, weight),
        ]
    }

    fn genesis() -> Cycle5RefreshManifestV1 {
        build_cycle5_refresh_manifest_v1(
            ...
            TEST_TRAINEE_RUN,
            TEST_TRAINEE_SEED,
            slots_for(0, CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1),
        )
    ...
    fn genesis_builds_and_round_trips_v1() {
        ...
        let decoded = decode_cycle5_refresh_manifest_v1(manifest.canonical_bytes_v1(), None, None)
            .expect("decode");
    ...
    fn refresh_one_requires_and_binds_panel_bytes_v1() {
        let genesis = genesis();
        let panel = b"cycle5-test-panel-bytes-v1".to_vec();
        let manifest = build_cycle5_refresh_manifest_v1(
            1,
            Some(&genesis),
            Some(&panel),
            TEST_TRAINEE_RUN,
            TEST_TRAINEE_SEED,
            slots_for(1, CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1),
        )
        .expect("refresh 1");
        // Decoding without the panel bytes must fail closed.
        let missing =
            decode_cycle5_refresh_manifest_v1(manifest.canonical_bytes_v1(), Some(&genesis), None)
        ...
        let mismatch = decode_cycle5_refresh_manifest_v1(
        ...
        // Exact bytes decode.
        let decoded = decode_cycle5_refresh_manifest_v1(
            manifest.canonical_bytes_v1(),
            Some(&genesis),
            Some(&panel),
        )
    ...
    fn placeholder_panel_hash_is_rejected_v1() {
        assert!(is_placeholder_hash_cycle5_v1(
        ...
        let error =
            decode_cycle5_refresh_manifest_v1(text.as_bytes(), Some(&genesis), Some(&panel))
    ...
    fn wrong_rotation_identity_is_rejected_v1() {
        let genesis = genesis();
        let panel = b"panel".to_vec();
        let mut slots = slots_for(1, CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1);
        // Refresh 1 expects rotation index 1 (seed 970002); install 970003.
        slots[3] = frozen_slot(
            3,
            "historical-1",
            &CYCLE5_HISTORICAL_1_ROTATION_V1[2],
            CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1,
        );
        let error = build_cycle5_refresh_manifest_v1(
            1,
            Some(&genesis),
            Some(&panel),
            TEST_TRAINEE_RUN,
    ...
    fn historical_zero_switches_source_at_index_four_v1() {
        ...
        for index in 1..=4_u64 {
            let manifest = build_cycle5_refresh_manifest_v1(
                ...
                TEST_TRAINEE_RUN,
                TEST_TRAINEE_SEED,
                slots_for(index, CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1),
            )
            .expect("chain");
            previous = manifest;
        }
    ...
    fn duplicate_model_hash_is_rejected_v1() {
        let mut slots = slots_for(0, CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1);
    ...
    fn mw_update_zero_scores_preserve_uniform_weights_v1() {
        let prior = [CYCLE5_GENESIS_SLOT_WEIGHT_UNITS_V1; 8];
        let scores = [0.0_f64; 8];
        let updated = mw_update_cycle5_v1(&prior, &scores).expect("mw");
    ...
    fn mw_update_moves_mass_toward_winners_within_constraints_v1() {
        ...
        assert_eq!(total, CYCLE5_WEIGHT_TOTAL_UNITS_V1);
        assert!(updated[0] > updated[1]);
        assert!(updated[2] > updated[3]);
        assert!(updated[6] > updated[7]);
        for pair in 0..4 {
            assert!(updated[2 * pair] + updated[2 * pair + 1] >= CYCLE5_ROLE_FLOOR_UNITS_V1);
        }
        for unit in updated {
            assert!(unit > 0 && unit <= CYCLE5_POLICY_CAP_UNITS_V1);
        }
    }

    #[test]
    fn panel_score_fraction_bounds_v1() {
        assert!(panel_score_fraction_cycle5_v1(1_793).is_err());
        assert!(panel_score_fraction_cycle5_v1(-1_793).is_err());
        // Regression: i64::MIN must fail closed, not overflow in `abs()`.
        assert!(panel_score_fraction_cycle5_v1(i64::MIN).is_err());
        let fraction = panel_score_fraction_cycle5_v1(896).expect("fraction");
    ...
    fn mw_projection_handles_multiple_deficient_roles_v1() {
        ...
        let updated = mw_update_cycle5_v1(&prior, &scores).expect("feasible projection");
        let total: u64 = updated.iter().sum();
        assert_eq!(total, CYCLE5_WEIGHT_TOTAL_UNITS_V1);
        for pair in 0..4 {
            assert!(updated[2 * pair] + updated[2 * pair + 1] >= CYCLE5_ROLE_FLOOR_UNITS_V1);
