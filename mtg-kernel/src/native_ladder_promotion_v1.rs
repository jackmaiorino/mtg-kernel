...
//! 4): the predeclared rung promotion test. No I/O, no run-record schema, no
...
//!
//! - Win-rate test: strictly greater than 55 percent over 1,024 seat-swapped
//!   CRN pairs against the rung's PRIMARY opponent, draws counted as losses
//!   (so `wins` excludes draws; anything not a strict win falls into
//!   `pairs - wins`). The contract states this as "the S1 estimator's
//!   convention at four times its sample size" (S1's own estimator ran at
//!   256 pairs; the ladder's gate runs at 1,024).
...
//!   over the panel's pair count ("no forgetting the base game").
//!
...
/// Predeclared pair count for the win-rate test (Section 4): "1,024
/// seat-swapped CRN pairs".
pub(crate) const PROMOTION_GATE_PAIR_COUNT_V1: u64 = 1_024;

/// Predeclared win-rate threshold (Section 4): "strictly greater than 55
/// percent". The test is `wins / pairs > 0.55`, not `>=`.
...
/// Win-rate sub-check: strictly greater than 55 percent of `pairs`
/// seat-swapped CRN pairs are strict wins (draws already folded into
/// `pairs - wins` by the caller, per the contract's "draws as losses"
/// convention). `pairs == 0` or `wins > pairs` fail closed.
pub(crate) fn promotion_gate_win_rate_passes_v1(wins: u64, pairs: u64) -> bool {
    if pairs == 0 || wins > pairs {
        return false;
    }
    (wins as f64 / pairs as f64) > PROMOTION_GATE_WIN_RATE_THRESHOLD_V1
}
...
/// The complete predeclared promotion gate (Section 4): both sub-checks must
/// pass. `None` propagates an invalid panel-regression input (fails closed:
/// the caller must treat `None` as "do not promote", never as "gate
/// inapplicable").
pub(crate) fn promotion_gate_passes_v1(
    ...
    let win_rate_ok = promotion_gate_win_rate_passes_v1(wins, pairs);
...
mod tests {
    ...
    // ------------------------------------------------------------------
    // Win-rate boundary fixtures (contract Section 7): 563/1024 fails,
    // 564/1024 passes.
    // ------------------------------------------------------------------

    #[test]
    fn win_rate_boundary_563_fails_564_passes() {
        assert!(!promotion_gate_win_rate_passes_v1(
            563,
            PROMOTION_GATE_PAIR_COUNT_V1
        ));
        assert!(promotion_gate_win_rate_passes_v1(
            564,
            PROMOTION_GATE_PAIR_COUNT_V1
        ));
        // Sanity: 0.55 * 1024 = 563.2, so 563 is exactly the highest
        // integer win count still at or below the threshold.
        assert!(
            563.0 / (PROMOTION_GATE_PAIR_COUNT_V1 as f64) < PROMOTION_GATE_WIN_RATE_THRESHOLD_V1
        );
        assert!(
            564.0 / (PROMOTION_GATE_PAIR_COUNT_V1 as f64) > PROMOTION_GATE_WIN_RATE_THRESHOLD_V1
        );
    }
    ...
    fn win_rate_degenerate_inputs_fail_closed() {
        ...
        assert!(!promotion_gate_win_rate_passes_v1(1_025, 1_024));
        assert!(!promotion_gate_win_rate_passes_v1(
            0,
            PROMOTION_GATE_PAIR_COUNT_V1
        ));
    ...
    fn combined_gate_requires_both_subchecks() {
        ...
        // win-rate outcome.
        assert_eq!(
            promotion_gate_passes_v1(564, PROMOTION_GATE_PAIR_COUNT_V1, previous_mean, 0.5, 0),
            None
        );
    }
}
