...
const ADMISSION_SCHEMA_V1: &str = "mtg-kernel-action-ingress-admission-envelope/v1";
const ADMISSION_PAYLOAD_SCHEMA_V1: &str = "mtg-kernel-action-ingress-admission-payload/v1";
const ADMISSION_TEST_IDENTITY_V1: &str = "native_checkpoint_inference_v1::checkpoint_reliance_probe_v1::action_ingress_admission_v1::official_action_ingress_admission_probe_v1";
...
const MODEL_ROLE_ENV_V1: &str = "ACTION_INGRESS_MODEL_ROLE";
const STORE_ROOT_ENV_V1: &str = "ACTION_INGRESS_STORE_ROOT";
const REPORT_MARKER_V1: &str = "ACTION_INGRESS_ADMISSION_JSON=";
...
#[test]
fn slot69_repair_and_fixed_digest_gates_are_exact_and_fail_closed_v1() {
...
#[test]
fn digest_zero_stress_and_non_digest_ingress_controls_are_exact_v1() {
...
#[test]
fn supplemental_player_target_cross_runtime_tensorization_is_bit_exact_v1() {
...
fn execute_model_admission_v1(
    ...
    AdmissionPayloadV1 {
        schema: ADMISSION_PAYLOAD_SCHEMA_V1,
        label: "ACTION-INGRESS-ADMISSION-DIAGNOSTIC-NON-EVIDENCE",
        test_identity: ADMISSION_TEST_IDENTITY_V1,
...
fn emit_payload_v1(payload: AdmissionPayloadV1) {
    ...
    println!("{REPORT_MARKER_V1}{report}");
}

fn run_raw_common_snapshot_v1() {
    assert!(
        std::env::var_os(STORE_ROOT_ENV_V1).is_none(),
        "{STORE_ROOT_ENV_V1} must be absent for raw-common-snapshot"
    );
    let initial_model =
        NativePolicyValueNetV1::runner_fixed_v1(NativePolicyValueModelConfigV1::contract_v1())
...
fn run_imported_store_g0_v1(role: ModelRoleV1) {
    ...
    let store_root = std::env::var_os(STORE_ROOT_ENV_V1)
        .map(PathBuf::from)
        .unwrap_or_else(|| panic!("{STORE_ROOT_ENV_V1} must name the exact canonical Store"));
    assert_eq!(
        store_root,
        PathBuf::from(role.expected_store_root().unwrap()),
        "{STORE_ROOT_ENV_V1} must equal the predeclared canonical Store path"
    );
    let root =
...
///
/// - `ACTION_INGRESS_MODEL_ROLE`: exactly `raw-common-snapshot`,
///   `imported-mirror-g0`, or `imported-diverged-g0`;
/// - `ACTION_INGRESS_STORE_ROOT`: absent for the raw role and exactly the
...
fn official_action_ingress_admission_probe_v1() {
    let role_value = std::env::var(MODEL_ROLE_ENV_V1)
        .unwrap_or_else(|_| panic!("{MODEL_ROLE_ENV_V1} must bind one exact model role"));
    let role = ModelRoleV1::parse_v1(&role_value)
        .unwrap_or_else(|| panic!("{MODEL_ROLE_ENV_V1} contains an unrecognized model role"));
    match role {
