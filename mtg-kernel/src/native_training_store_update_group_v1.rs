...
mod tests {
    ...
    fn searcher_active_scheduler_timeout_accepts_only_the_frozen_override_value() {
        ...
        validate_prepared_execution_config_v1(&run, &searcher_config)
            .expect("exactly the frozen override value must validate when searcher_active_v1 is true");
    ...
    fn non_searcher_launch_still_requires_exactly_the_frozen_recorded_timeout() {
        ...
        validate_prepared_execution_config_v1(&run, &config)
            .expect("the frozen recorded 30,000ms value must still validate for an all-neural launch");
