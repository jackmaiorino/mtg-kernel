#!/usr/bin/env python3
"""Seal measured native transport into a projected successor package."""
...
RESULT_SCHEMA = "mtg-kernel-structured-policy-terminal-trust-projection-transport/v1"
...
def finalize(args: argparse.Namespace) -> dict:
    ...
    if (
        candidate.get("schema") != projection.CANDIDATE_SCHEMA
        or report.get("schema") != projection.REPORT_SCHEMA
        ...
        _fail("projected package binding mismatch")
    ...
    if candidate_backup.exists() or report_backup.exists():
        _fail("projection transport backups already exist")
