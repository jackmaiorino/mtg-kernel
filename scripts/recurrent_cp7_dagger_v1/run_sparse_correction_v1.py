#!/usr/bin/env python3
"""Rapid sparse disagreement-correction screen on the CP7 selection split."""
...
SCHEMA = "mtg-kernel-sparse-recurrent-cp7-correction-screen/v1"
BETAS = (0.3, 1.0, 3.0, 10.0, 30.0)
EPOCHS = 8
BATCH_SIZE = 256
...
def _loss(
    ...
    disagreement = (packed.parent_logits.argmax(dim=1) != teacher).to(weights.dtype)
    nll = -candidate_logp.gather(1, teacher.unsqueeze(1)).squeeze(1)
    disagreement_mass = (weights * disagreement).sum().clamp_min(1.0e-12)
    disagreement_ce = (nll * weights * disagreement).sum() / disagreement_mass
    ...
    total = disagreement_ce + beta * preservation
    return total, {
        "disagreement_cross_entropy": float(disagreement_ce.detach()),
        "parent_kl": float(preservation.detach()),
        "disagreement_fraction": float(
            (weights * disagreement).sum() / weights.sum().clamp_min(1.0e-12)
        ),
    }


def _batches(
    decisions: list[Any], rng: random.Random
) -> Iterable[list[Any]]:
...
def _checkpoint_rank(metrics: dict[str, Any]) -> tuple[float, float, float]:
    ...
    if gate["pass"]:
        return (0.0, overall["mean_total_variation"], -overall["relative_nll_improvement"])
    substantive = min(
        overall["relative_nll_improvement"] / base.RELATIVE_NLL_MIN,
        overall["top1_delta"] / base.TOP1_DELTA_MIN,
        *(metrics["by_candidate_seat"][str(seat)]["relative_nll_improvement"] / base.RELATIVE_NLL_MIN for seat in (0, 1)),
    )
    movement_excess = max(
        overall["mean_total_variation"] / base.MEAN_TV_MAX,
        overall["p90_total_variation"] / base.P90_TV_MAX,
    )
    return (1.0, -substantive, movement_excess)
...
def _fit_arm(
    ...
    checkpoints: list[tuple[tuple[float, float, float], int, dict[str, Tensor], dict[str, Any]]] = []
    for epoch in range(1, EPOCHS + 1):
        model.train()
        started = time.perf_counter()
        sums = {"loss": 0.0, "disagreement_cross_entropy": 0.0, "parent_kl": 0.0}
        gradient_max = 0.0
        steps = 0
        for batch in _batches(train, rng):
            ...
            if not torch.isfinite(gradient):
                base._fail("non-finite sparse correction gradient")
            optimizer.step()
            sums["loss"] += float(loss.detach())
            sums["disagreement_cross_entropy"] += parts["disagreement_cross_entropy"]
        ...
        history.append(
            {
                ...
                "mean_disagreement_cross_entropy": sums["disagreement_cross_entropy"] / steps,
    ...
    selected_gate = base._gate(selected[3])
    return model, {
        "beta": beta,
...
def main() -> None:
    ...
    for beta in BETAS:
        arms.append(_fit_arm(train, selection, beta, device))
    ...
    result = {
        ...
        "config": {
            ...
            "log_ratio_budget": LOG_RATIO_BUDGET,
            "seed": base.SEED,
            "objective": "disagreement-only CP7 CE plus all-row parent KL",
        },
        "split": {
    ...
    base._write_new(args.output_dir / "manifest.json", manifest)
