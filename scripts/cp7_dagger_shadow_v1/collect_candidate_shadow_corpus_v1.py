...
def _validate_task_outputs(
    ...
    episodes_without_priority_labels = sorted(expected_episodes - observed_episodes)
    ...
    return {
        ...
        "episodes_with_priority_labels": len(observed_episodes),
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.reuse_completed_tasks:
        if not args.evidence_root.is_dir():
            _fail(f"evidence root does not exist: {args.evidence_root}")
        results = [
            _existing_task_result(args, worker, first_pair, pair_count)
            for worker, (first_pair, pair_count) in enumerate(tasks)
        ]
    else:
    ...
    elapsed = (
        args.collection_wall_seconds
        if args.reuse_completed_tasks and args.collection_wall_seconds is not None
    ...
    report = {
        ...
        "episodes_with_priority_labels": sum(result["episodes_with_priority_labels"] for result in results),
        "episodes_without_priority_labels": sum(
            len(result["episodes_without_priority_labels"]) for result in results
        ),
...
def main() -> int:
    ...
    if (
        ...
        or (args.collection_wall_seconds is not None and args.collection_wall_seconds <= 0.0)
        or (args.collection_wall_seconds is not None and not args.reuse_completed_tasks)
    ):
