#!/usr/bin/env python3
"""Run and receipt the complete-screen classifier as a separate process."""
...
try:
    from scripts.action_ingress_admission_v1 import classify_results, contract, run_probe
...
def run_classifier(
    *,
    ...
    classifier_source = (
        repo / "scripts" / "action_ingress_admission_v1" / "classify_results.py"
    )
    command = [
        sys.executable,
    ...
    receipt: dict[str, Any] = {
        ...
        "sources": [
            _source_record(repo / "scripts" / "action_ingress_admission_v1" / name)
            for name in (
                "classify_results.py",
                "contract.py",
                "run_classifier.py",
...
if __name__ == "__main__":
    try:
        ...
        print(f"ACTION_INGRESS_CLASSIFIER_RECEIPT_INVALID: {error}", file=sys.stderr)
        raise
