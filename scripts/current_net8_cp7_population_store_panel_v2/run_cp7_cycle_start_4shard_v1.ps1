# run_cp7_cycle_start_4shard_v1.ps1
#
# Sibling of run_cp7_cycle_end_4shard_v1.ps1 for the cycle-3 CYCLE-START CP7
# read (the third of the three registered reads: cycle-start, mid-cycle,
# cycle-end -- see CLAUDE-CYCLE3-READS-PHASE-NOTES-V1.md Section 7). Same
# structure, same acceptance/failure/void-cap machinery, unchanged from the
# cycle-end wrapper except:
#   - base_seed 2026082801 (the cycle-start seed), not cycle-end's 2026082803.
#   - focal is the store's GENERATION-0 checkpoint (population:0:...), not the
#     current head (population:2048:...). This makes the read double as a
#     panel self-consistency check: generation 0 of the focal lineage is its
#     own warm-start import point, expected to be parameter-identical to the
#     reference model (current-1-seed-975002-store at generation 2048, the
#     exact checkpoint that lineage was imported from). If the harness's own
#     checkpoint loading/decode/comparison machinery is sound, focal@0 and
#     reference@current-1@2048 should play IDENTICALLY against CP7 under the
#     shared CRN seeds -- zero discordant matched legs, not just a similar
#     win rate. A nonzero discordant count here would be a real harness
#     defect (bit-identical weights producing different game outcomes),
#     not the ordinary model-vs-model variance the cycle-end read measures.
#   - evidence-root naming: cycle-start-g0000-attempt-001 (cycle-end's own
#     equivalent: cycle-end-g2048-attempt-NNN).
#
# void-cap-mode: report, unchanged rationale from the cycle-end wrapper --
# this script is scoped to the registered reads, and Jack's ruling (relayed
# by the coordinator; no independently located written record in collab at
# implementation time) waives cap enforcement for all three under full
# disclosure. Accounting/reporting stay mandatory either way.
#
...
$parentEvidenceRoot = "E:\mtg-kernel-population-v2-cycle3-cp7-anchor-reads\cycle-start-g0000-attempt-001"
$baseSeed = "2026082801"
...
$focalModel = "focal=population:0:E:\mtg-kernel-population-v2-cycle3\lineage\real-attempt-003\run-0\store"
