#!/usr/bin/env python3
"""Validate a complete admission-v2 screen and emit three separate labels."""
...
try:
    from scripts.action_ingress_admission_v2 import contract, run_probe
...
def _validate_invocation(
    *,
    ...
    expected_projection_gate = (
        run_probe.verify_preserved_v1_raw_projection(parsed.payload)
        if spec.kind == "raw"
        else None
    )
    if receipt["preserved_v1_raw_projection_gate"] != expected_projection_gate:
        contract.fail(f"{spec.identity} preserved-v1 raw projection gate mismatch")
    ...
    *,
    ...
    if require_windows:
        ...
        preserved_v1_before = contract.verify_preserved_v1_evidence_record(
            completion["preserved_v1_evidence_before"],
            current=preserved_v1_current,
            where="completion.preserved_v1_evidence_before",
        )
        preserved_v1_after = contract.verify_preserved_v1_evidence_record(
            completion["preserved_v1_evidence_after"],
            current=preserved_v1_current,
            where="completion.preserved_v1_evidence_after",
        )
        if dict(preserved_v1_before) != dict(preserved_v1_after):
            contract.fail("preserved v1 evidence changed during recorded v2 execution")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"ACTION_INGRESS_ADMISSION_V2_CLASSIFICATION_INVALID: {error}",
            file=sys.stderr,
        )
        raise
