...
def payload_for(
    ...
    *,
    ...
    direct_sum = run_probe._ordered_f64_sum(
        row["direct_squared_norm"] for row in statistic_rows
    )
    digest_sum = run_probe._ordered_f64_sum(
        row["digest_squared_norm"] for row in statistic_rows
    )
    ingress_digest_rows = [
        {
    ...
    for row in contribution_rows:
        row["direct_contribution_rms"] = math.sqrt(
            row["direct_contribution_squared_norm"] / 64
        )
        row["digest_contribution_rms"] = math.sqrt(
            row["digest_contribution_squared_norm"] / 64
        )
    direct_contribution_rms = math.sqrt(
        run_probe._ordered_f64_sum(
            row["direct_contribution_squared_norm"] for row in contribution_rows
        )
        / (1_115 * 64)
    )
    digest_contribution_rms = math.sqrt(
        run_probe._ordered_f64_sum(
            row["digest_contribution_squared_norm"] for row in contribution_rows
        )
        / (1_115 * 64)
    )
    payload = {
