#!/usr/bin/env python3
"""Execute exactly three sequential action-ingress-v2 model reads fail closed."""
...
MARKER = b"ACTION_INGRESS_ADMISSION_V2_JSON="
HARNESS_PREFIX = (
    b"test " + contract.PROBE_TEST.encode("ascii") + b" ... " + MARKER
)
PRESERVED_V1_MARKER = b"ACTION_INGRESS_ADMISSION_JSON="
PRESERVED_V1_PROBE_TEST = (
    "native_checkpoint_inference_v1::checkpoint_reliance_probe_v1::"
    "action_ingress_admission_v1::official_action_ingress_admission_probe_v1"
)
PRESERVED_V1_ENVELOPE_SCHEMA = "mtg-kernel-action-ingress-admission-envelope/v1"
PRESERVED_V1_PAYLOAD_SCHEMA = "mtg-kernel-action-ingress-admission-payload/v1"
PRESERVED_V1_LABEL = "ACTION-INGRESS-ADMISSION-DIAGNOSTIC-NON-EVIDENCE"
PRESERVED_V1_RAW_STDOUT_RELATIVE_PATH = Path(
    "runs/01-raw-common-snapshot/probe.stdout.log"
)
PRESERVED_V1_RAW_STDERR_RELATIVE_PATH = Path(
    "runs/01-raw-common-snapshot/probe.stderr.log"
)
PRESERVED_V1_RAW_STDOUT_SHA256 = (
    "816cf78c980356cd3e1f956962122528d87d76d9fa807dbbdfa0774f7cd2c253"
)
PRESERVED_V1_RAW_PAYLOAD_SHA256 = (
    "507a8bdef0fe13e37808074d57e20d446958d8dc78cdfb434f272874cb2bef09"
)
RETRY_PROJECTION_IDENTITY = (
    "canonical-json-drop-top-schema-label-test-identity-and-v2-"
    "contribution-squared-norm-fields-v1"
)
RETRY_PROJECTION_BYTE_COUNT = 400_290
...
CONTRIBUTION_ROW_KEYS = {
    ...
    "digest_contribution_squared_norm",
    "direct_contribution_rms",
    "digest_contribution_rms",
}
EFFECTS_KEYS = {
...
def _ordered_f64_sum(values: Iterable[float]) -> float:
    """Match Rust's positive-zero f64 left fold without CPython compensation."""
...
def _verify_file_binding(
    ...
    *,
    ...
    unresolved = Path(contract.require_string(record["path"], f"{where}.path"))
    if (
        not unresolved.is_absolute()
        or not unresolved.is_file()
        or _is_reparse_point(unresolved)
    ):
        contract.fail(f"{where}.path must be an absolute non-reparse regular file")
    ...
    *,
    ...
    contract.exact_keys(
        receipt,
        {
            ...
            "packaging_preflight",
            "payload_sha256",
            "preserved_v1_evidence_after",
            "preserved_v1_evidence_before",
            "required_tests",
            "rust_controls",
            "runtime_tuple",
            "schema",
    ...
    if receipt["licensed_retry_diff"] != contract.verify_licensed_retry_diff(repo):
        contract.fail("build receipt licensed-retry diff binding mismatch")
    ...
    if require_windows:
        preserved_v1_current = contract.verify_preserved_v1_evidence()
        preserved_v1_before = contract.verify_preserved_v1_evidence_record(
            receipt["preserved_v1_evidence_before"],
            current=preserved_v1_current,
            where="build receipt.preserved_v1_evidence_before",
        )
        preserved_v1_after = contract.verify_preserved_v1_evidence_record(
            receipt["preserved_v1_evidence_after"],
            current=preserved_v1_current,
            where="build receipt.preserved_v1_evidence_after",
        )
        if dict(preserved_v1_before) != dict(preserved_v1_after):
            contract.fail("preserved v1 evidence changed during the recorded v2 build")

    executable_record = contract.exact_keys(
        receipt["executable"], {"path", "sha256"}, "build receipt.executable"
    )
    unresolved_executable = Path(
        contract.require_string(executable_record["path"], "build receipt executable path")
    )
    if (
        not unresolved_executable.is_absolute()
        or not unresolved_executable.is_file()
        or _is_reparse_point(unresolved_executable)
    ):
        contract.fail("build receipt executable must be a non-reparse regular file")
    executable = unresolved_executable.resolve()
    ...
    unresolved_cargo_executable = contract.resolve_lib_test_executable(cargo_messages)
    if _is_reparse_point(unresolved_cargo_executable):
        contract.fail("recorded Cargo executable must not be a reparse point")
    cargo_executable = unresolved_cargo_executable.resolve()
...
def _parse_probe_output(
    ...
    *,
    ...
    if len(stdout) > contract.MAX_STREAM_BYTES or len(stderr) > contract.MAX_STREAM_BYTES:
        contract.fail(f"{where} stream exceeds frozen size cap")
    if stderr.count(marker):
        contract.fail(f"{where} marker is forbidden on stderr")
    if stdout.count(marker) != 1:
        contract.fail(f"{where} stdout must contain exactly one admission marker")
    harness_prefix = b"test " + test_identity.encode("ascii") + b" ... " + marker
    lines = stdout.splitlines()
    indices = [
        index for index, line in enumerate(lines) if line.startswith(harness_prefix)
    ]
    if len(indices) != 1:
        contract.fail(f"{where} marker must have the exact libtest prefix")
    index = indices[0]
    if index == 0 or lines[index - 1] != b"running 1 test":
        contract.fail(f"{where} marker must follow the one-test harness header")
    if index + 1 >= len(lines) or lines[index + 1] != b"ok":
        contract.fail(f"{where} marker must be followed by libtest ok")
    envelope_raw = lines[index][len(harness_prefix) :]
    envelope_decimal = contract.parse_json_bytes(envelope_raw, f"{where} envelope")
    contract.exact_keys(
        envelope_decimal,
    ...
    contract.require_string(
        envelope_decimal["schema"], f"{where} envelope.schema", envelope_schema
    )
    payload_sha = contract.require_sha256(
        envelope_decimal["payload_sha256"], f"{where} envelope.payload_sha256"
    )
    prefix = (
        b'{"schema":"'
    ...
    if not envelope_raw.startswith(prefix) or not envelope_raw.endswith(b"}"):
        contract.fail(f"{where} envelope differs from exact compact Rust field order")
    payload_raw = envelope_raw[len(prefix) : -1]
    if contract.sha256_bytes(payload_raw) != payload_sha:
        contract.fail(f"{where} raw producer payload SHA-256 mismatch")
    payload = contract.parse_json_bytes_native_numbers(payload_raw, f"{where} payload")
    envelope = contract.parse_json_bytes_native_numbers(
        envelope_raw, f"{where} envelope"
    )
    if envelope["payload"] != payload:
        contract.fail(f"{where} envelope payload differs from extracted raw payload")
...
def parse_probe_output(stdout: bytes, stderr: bytes) -> ParsedProbe:
    return _parse_probe_output(
        ...
        envelope_schema=contract.PROBE_ENVELOPE_SCHEMA,
        where="probe",
    )


def _canonical_retry_projection(
    payload: Mapping[str, Any],
    *,
    is_v2: bool,
) -> bytes:
    projection = dict(payload)
    for field in ("schema", "label", "test_identity"):
        if field not in projection:
            contract.fail(f"retry projection payload omits top-level {field}")
        projection.pop(field)
    contribution = projection.get("first_layer_contribution_rms")
    if not isinstance(contribution, Mapping):
        contract.fail("retry projection contribution block must be an object")
    contribution_projection = dict(contribution)
    rows = contribution_projection.get("per_action_row")
    if not isinstance(rows, list):
        contract.fail("retry projection contribution rows must be an array")
    ...
    for index, raw in enumerate(rows):
        if not isinstance(raw, Mapping):
            contract.fail(f"retry projection contribution row {index} must be an object")
        row = dict(raw)
        for field in squared_fields:
            if is_v2:
                if field not in row:
                    contract.fail(
                        f"v2 retry projection row {index} omits licensed field {field}"
                    )
                row.pop(field)
            elif field in row:
                contract.fail(
                    f"preserved v1 retry projection unexpectedly contains {field}"
                )
        projected_rows.append(row)
    contribution_projection["per_action_row"] = projected_rows
    projection["first_layer_contribution_rms"] = contribution_projection
    return contract.canonical_json_bytes(projection)


def verify_preserved_v1_raw_projection(
    payload: Mapping[str, Any],
    *,
    preserved_v1_artifact_root: Path | None = None,
) -> dict[str, Any]:
    root = (
        preserved_v1_artifact_root
        if preserved_v1_artifact_root is not None
        else Path(contract.PRESERVED_V1_ARTIFACT_ROOT_WINDOWS)
    )
    stdout_path = root / PRESERVED_V1_RAW_STDOUT_RELATIVE_PATH
    stderr_path = root / PRESERVED_V1_RAW_STDERR_RELATIVE_PATH
    stdout = _read_bytes(stdout_path, "preserved v1 raw stdout")
    stderr = _read_bytes(stderr_path, "preserved v1 raw stderr")
    if contract.sha256_bytes(stdout) != PRESERVED_V1_RAW_STDOUT_SHA256:
        contract.fail("preserved v1 raw stdout SHA-256 mismatch")
    if stderr:
        contract.fail("preserved v1 raw stderr must be empty")
    preserved = _parse_probe_output(
        stdout,
        stderr,
        marker=PRESERVED_V1_MARKER,
        test_identity=PRESERVED_V1_PROBE_TEST,
        envelope_schema=PRESERVED_V1_ENVELOPE_SCHEMA,
        where="preserved v1 raw probe",
    )
    if preserved.envelope["payload_sha256"] != PRESERVED_V1_RAW_PAYLOAD_SHA256:
        contract.fail("preserved v1 raw payload SHA-256 mismatch")
    contract.require_string(
        preserved.payload.get("schema"),
        "preserved v1 payload.schema",
        PRESERVED_V1_PAYLOAD_SCHEMA,
    )
    contract.require_string(
        preserved.payload.get("label"),
        "preserved v1 payload.label",
        PRESERVED_V1_LABEL,
    )
    contract.require_string(
        preserved.payload.get("test_identity"),
        "preserved v1 payload.test_identity",
        PRESERVED_V1_PROBE_TEST,
    )
    v1_projection = _canonical_retry_projection(preserved.payload, is_v2=False)
    v2_projection = _canonical_retry_projection(payload, is_v2=True)
    for name, projection in (
        ("preserved v1", v1_projection),
        ("v2 raw", v2_projection),
    ):
        if len(projection) != RETRY_PROJECTION_BYTE_COUNT:
            contract.fail(
                f"{name} retry projection byte count mismatch: "
                f"expected={RETRY_PROJECTION_BYTE_COUNT} actual={len(projection)}"
            )
        digest = contract.sha256_bytes(projection)
        if digest != RETRY_PROJECTION_SHA256:
            contract.fail(
                f"{name} retry projection SHA-256 mismatch: "
                f"expected={RETRY_PROJECTION_SHA256} actual={digest}"
            )
    if v2_projection != v1_projection:
        contract.fail("v2 raw payload differs from the preserved v1 canonical projection")
    return {
        "canonical_byte_count": RETRY_PROJECTION_BYTE_COUNT,
        "canonical_sha256": RETRY_PROJECTION_SHA256,
        "identity": RETRY_PROJECTION_IDENTITY,
        "preserved_v1_payload_sha256": PRESERVED_V1_RAW_PAYLOAD_SHA256,
        "preserved_v1_stdout_sha256": PRESERVED_V1_RAW_STDOUT_SHA256,
    }
...
def verify_probe_payload(payload: dict[str, Any], spec: contract.ModelSpec) -> dict[str, Any]:
    ...
    direct_sum = _ordered_f64_sum(
        float(row["direct_squared_norm"]) for row in stat_rows
    )
    digest_sum = _ordered_f64_sum(
        float(row["digest_squared_norm"]) for row in stat_rows
    )
    exact_statistics = {
    ...
    for index, row in enumerate(contribution_rows):
        for rms_field, squared_norm_field in (
            ("direct_contribution_rms", "direct_contribution_squared_norm"),
            ("digest_contribution_rms", "digest_contribution_squared_norm"),
        ):
            expected = math.sqrt(float(row[squared_norm_field]) / 64)
            if float(row[rms_field]) != expected:
                contract.fail(
                    f"payload first-layer row {index} {rms_field} is inconsistent "
                    f"with {squared_norm_field}"
                )
    for field, squared_norm_field in (
        ("direct_contribution_rms", "direct_contribution_squared_norm"),
        ("digest_contribution_rms", "digest_contribution_squared_norm"),
    ):
        reconstructed = math.sqrt(
            _ordered_f64_sum(
                float(row[squared_norm_field]) for row in contribution_rows
            )
            / (1_115 * 64)
        )
        ...
        if actual != reconstructed:
            contract.fail(
                f"payload first-layer {field} is inconsistent with ordered "
                f"per-row squared norms"
            )
...
def _model_environment(spec: contract.ModelSpec) -> dict[str, str]:
    result = {
        "ACTION_INGRESS_V2_MODEL_ROLE": spec.identity,
        "CUDA_VISIBLE_DEVICES": "",
        "MTG_KERNEL_DEVICE": "cpu",
    }
    if spec.store_root is not None:
        result["ACTION_INGRESS_V2_STORE_ROOT"] = spec.store_root
    return result
...
def run(
    *,
    ...
    for spec in contract.MODEL_SPECS:
        ...
        for key in (
            "ACTION_INGRESS_MODEL_ROLE",
            "ACTION_INGRESS_STORE_ROOT",
            "ACTION_INGRESS_V2_MODEL_ROLE",
        ...
        preserved_v1_raw_projection_gate = (
            verify_preserved_v1_raw_projection(parsed.payload)
            if spec.kind == "raw"
            else None
        )
        ...
        receipt: dict[str, Any] = {
            ...
            "preserved_v1_raw_projection_gate": preserved_v1_raw_projection_gate,
    ...
    if preserved_v1_after != preserved_v1_before:
        contract.fail("preserved v1 evidence changed during v2 execution")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ACTION_INGRESS_ADMISSION_V2_SCREEN_INVALID: {error}", file=sys.stderr)
        raise
