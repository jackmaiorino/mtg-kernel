"""Focused tests for the cycle-5 payoff panel runner.
...
def plain_locator(prefix: str = "/stores/slot-") -> dict[int, SlotLocation]:
    """Every slot a bare store root, the control-v3 shape."""
    return {
...
class PanelAndBtInputAssemblyTests(unittest.TestCase):
    ...
    def test_panel_document_both_orderings_and_rank_sums(self):
        ...
        self.assertEqual(panel["schema"], "mtg-kernel-cycle5-payoff-panel/v1")
...
class StoreGenerationTranslationTests(unittest.TestCase):
    """The 2048-offset translation, mirroring the launcher's
    ...
    def test_own_run_label_translates_by_the_896_offset(self):
        for update in (0, 128, 512, 2048):
            slot = self.own_run_slot(2048 + update)
            self.assertEqual(
                store_generation_for_slot(slot, self.TRAINEE_RUN),
                update,
                "an own-run slot loads at label - 2048",
            )
    ...
    def test_other_run_slots_keep_their_labels(self):
        ...
        # Including labels far below 2048, which are ordinary generations in
...
class ManifestAndSlotLocatorLoadingTests(unittest.TestCase):
    def test_load_manifest_translates_own_run_slot_generations(self):
        ...
        document["slots"][5]["source_generation"] = 2048 + 128
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "manifest.json"
            path.write_bytes(canonical_bytes(document))
            _, _, _, slots = load_manifest(path)
        self.assertEqual(slots[5]["source_generation"], 2048 + 128)
...
class PanelFilenameTests(unittest.TestCase):
    def test_panel_filename_matches_the_fixed_naming_scheme(self):
        # Matches the Rust chain builder's `cycle5_chain_panel_filename_v1`
        # exactly -- see `native_population_refresh_builder_cycle5_v1.rs`.
...
# absolute paths.
ABS_SLOT_0 = str(Path(tempfile.gettempdir()).resolve() / "cycle5-slot-0")
ABS_SLOT_5 = str(Path(tempfile.gettempdir()).resolve() / "cycle5-slot-5")
ABS_CHAIN = str(Path(tempfile.gettempdir()).resolve() / "cycle5-chain")
...
class SlotLocatorChainDirTests(unittest.TestCase):
    ...
    def test_control_r_and_other_run_slots_must_not_carry_a_chain_dir(self):
        slots = self.own_run_slots()
        # control-v3 is a v3 arm: even its own-run slot takes the plain walk.
        validate_slot_chain_dirs("control-v3", slots, plain_locator(), self.TRAINEE_RUN)
        with self.assertRaises(PanelRunnerError):
            validate_slot_chain_dirs("control-v3", slots, self.v4_locator(), self.TRAINEE_RUN)
...
class WrapperEmittedLocatorTests(unittest.TestCase):
    ...
    def test_the_reader_accepts_the_wrappers_own_treatment_rb_locator(self):
        ...
        suite = (
            Path(__file__).resolve().parent / "run-cycle5-arm-tests.ps1"
        )
