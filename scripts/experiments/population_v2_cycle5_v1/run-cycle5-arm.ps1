<#
.SYNOPSIS
Cycle-5 arm launcher wrapper (docs/native_cycle5_arm_launcher_v1.md Section 6).

.DESCRIPTION
Drives one cycle-5 arm end to end, or the CONTROL preflight ladder that must
...
The arm's run record is DERIVED, not supplied: cycle5_run_record_v1 builds it
from the arm kind, the immutable cycle-4 routing record (-RoutingRecord, whose
bytes must hash to the pinned record that names cycle 5's parent), the parent
Store that record names, -ArmExecutable's own build provenance, and the
compiled cycle-5 literals, and refuses to replace a DIFFERENT record already
at -RunRecord. Running it on
...
Formal mode:
  ...
  0a. derive (or re-prove) the run record with cycle5_run_record_v1
  0. if the Store holds no genesis, run cycle5_arm_v1 --bootstrap-genesis,
     which seeds it from the pinned parent and trains nothing
  0b. if refresh-00.manifest.json does not exist, build it with
     cycle5_refresh_build_v1 --genesis from the operator's pinned roster with
...
then per interval, through refresh 16:
  ...
  2. run cycle5_arm_v1 with --stop-generation = interval end
...
param(
    ...
    [Parameter(Mandatory = $true, ParameterSetName = 'Inline')][ValidateSet('control-v3', 'centered-v5')][string]$Arm,
    ...
    # The immutable cycle-4 routing record (routing-record.json) that names
    # cycle 5's parent. The run-record builder refuses any file whose bytes
    # hash differently from the pinned record, and cross-checks the parent
    # Store against the record's parent fields.
    [Parameter(Mandatory = $true, ParameterSetName = 'Inline')][string]$RoutingRecord,
    # Poll-the-mailbox-before-every-arm-transition (poll-the-codex-mailbox.md),
    # enforced in code for cycle 5: a formal, non-dry launch requires a
    # receipt JSON the operator writes after reading the mailbox, with
    # read_at_utc (ISO 8601, at most 30 minutes old), last_entry (the newest
    # entry read) and hold_open (must be false). A preflight or dry run does
    # not require one.
    [string]$MailboxReadReceipt,
    ...
    [ValidateRange(1, 64)][int]$PanelMatchupWorkers = 12,
...
# pin-campaign-to-p-cores.md: Windows scheduled cycle-4 campaign processes
# onto the efficiency cores (2.1x slower). Pin this wrapper to the sixteen
# P-core logical processors (mask 0xFFFF); every child it spawns inherits
# the mask. A reversible host scheduling control that changes no command,
# parameter, seed, model or output (CODEX #80).
try {
    [System.Diagnostics.Process]::GetCurrentProcess().ProcessorAffinity = [IntPtr]0xFFFF
}
catch {
    Write-Warning "could not set the P-core affinity mask on this process: $($_.Exception.Message)"
}
...
$script:Cycle5WrapperParameterNames = @(
...
if ($PSCmdlet.ParameterSetName -ceq 'ParameterFile') {
    $loadedParameters = Read-Cycle5ParameterFile `
        -Path $ParameterFile `
        -KnownNames $script:Cycle5WrapperParameterNames
    ...
    $missing = @(
        foreach ($name in @('Mode', 'Arm', 'EvidenceRoot', 'RunRecord', 'RefreshChainDir',
                ...
                'GenesisParentGeneration', 'RoutingRecord', 'ArmExecutable',
                'RefreshBuilderExecutable', 'PanelExecutable', 'PythonExecutable',
                'PanelBaseSeed')) {
...
if ($ThroughRefreshIndex -lt [uint64]1 -or $ThroughRefreshIndex -gt $script:Cycle5MaxRefreshIndex) {
    throw "-ThroughRefreshIndex must be 1..$($script:Cycle5MaxRefreshIndex), got $ThroughRefreshIndex"
}
if ($Mode -ceq 'formal') {
    ...
    if ($Arm -cne 'control-v3') {
        throw "the preflight ladder is the CONTROL ladder; -Arm must be control-v3, got $Arm"
    }
    if ($PreflightUpdates -gt $script:Cycle5PreflightMaxUpdates) {
        throw "-PreflightUpdates must be 0 (derive) or 1..$($script:Cycle5PreflightMaxUpdates), got $PreflightUpdates"
    }
}
if (@($SlotStoreRoots).Count -ne $script:Cycle5SlotCount) {
    throw "-SlotStoreRoots must name exactly $($script:Cycle5SlotCount) store roots in slot order 0..7, got $(@($SlotStoreRoots).Count)"
}
$historicalOneRootCount = @($HistoricalOneStoreRoots).Count
# Every cycle-5 arm refreshes (there is no static-pool arm), so a formal run
# through any refresh index needs the rotation roots.
$requiresHistoricalOneRotation = ($Mode -ceq 'formal' -and $ThroughRefreshIndex -ge [uint64]1)
if ($requiresHistoricalOneRotation -and $historicalOneRootCount -ne [int]$script:Cycle5HistoricalRotationPeriod) {
    throw "-HistoricalOneStoreRoots is required for formal $Arm through refresh $ThroughRefreshIndex and must name exactly $($script:Cycle5HistoricalRotationPeriod) store roots in rotation order (refresh_index mod $($script:Cycle5HistoricalRotationPeriod)); got $historicalOneRootCount"
}
if (-not $requiresHistoricalOneRotation -and $historicalOneRootCount -ne 0 -and $historicalOneRootCount -ne [int]$script:Cycle5HistoricalRotationPeriod) {
    throw "-HistoricalOneStoreRoots must name exactly $($script:Cycle5HistoricalRotationPeriod) store roots in rotation order (refresh_index mod $($script:Cycle5HistoricalRotationPeriod)), got $historicalOneRootCount"
}
...
if (-not $UseExistingRunRecord -and [string]::IsNullOrWhiteSpace($RunRecordExecutable)) {
    throw 'a cycle-5 launch requires -RunRecordExecutable (cycle5_run_record_v1.exe), or -UseExistingRunRecord to take -RunRecord exactly as given'
}
if (-not (Test-Path -LiteralPath $RoutingRecord -PathType Leaf)) {
    throw "-RoutingRecord does not exist: $RoutingRecord"
}

function Assert-Cycle5MailboxReadReceipt {
    param(
        [Parameter(Mandatory = $true)][string]$Path,
        [int]$MaxAgeMinutes = 30
    )
    if ([string]::IsNullOrWhiteSpace($Path)) {
        throw 'a formal launch requires -MailboxReadReceipt: read the mailbox, then write the receipt JSON (read_at_utc, last_entry, hold_open) and pass its path'
    }
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "-MailboxReadReceipt does not exist: $Path"
    }
    $receipt = Get-Content -LiteralPath $Path -Raw | ConvertFrom-Json
    foreach ($field in @('read_at_utc', 'last_entry', 'hold_open')) {
        if (-not ($receipt.PSObject.Properties.Name -ccontains $field)) {
            throw "mailbox read receipt $Path lacks the field $field"
        }
    }
    $readAt = [DateTimeOffset]::Parse([string]$receipt.read_at_utc, [System.Globalization.CultureInfo]::InvariantCulture)
    $age = [DateTimeOffset]::UtcNow - $readAt
    if ($age.TotalMinutes -gt $MaxAgeMinutes -or $age.TotalMinutes -lt -5) {
        throw "mailbox read receipt $Path is $([int]$age.TotalMinutes) minutes old; read the mailbox again (limit $MaxAgeMinutes minutes)"
    }
    if ([bool]$receipt.hold_open) {
        throw "mailbox read receipt $Path records an open hold ($($receipt.last_entry)); a hold is binding until the owner rules"
    }
    if ([string]::IsNullOrWhiteSpace([string]$receipt.last_entry)) {
        throw "mailbox read receipt $Path names no last_entry"
    }
    return [ordered]@{
        path = $Path
        read_at_utc = $readAt.ToUniversalTime().ToString('O')
        age_minutes = [math]::Round($age.TotalMinutes, 1)
        last_entry = [string]$receipt.last_entry
        hold_open = $false
        file = Get-Cycle5FileRecord -Path $Path
    }
}

$mailboxReadReceiptRecord = $null
if ($Mode -ceq 'formal' -and -not $DryRun) {
    $mailboxReadReceiptRecord = Assert-Cycle5MailboxReadReceipt -Path $MailboxReadReceipt
    Write-Host "inputs: mailbox read receipt accepted (last entry $($mailboxReadReceiptRecord.last_entry), $($mailboxReadReceiptRecord.age_minutes) minutes old)"
}

$gateName = "cycle5-$Arm-$Mode"
$gateRoot = Join-Path $EvidenceRoot $gateName
$root = New-Cycle5AttemptRoot -EvidenceRoot $EvidenceRoot -GateName $gateName
$phase = 'inputs'
$commandLog = Join-Path $root 'commands.jsonl'

function Add-Cycle5CommandRecord {
...
try {
    ...
    # -----------------------------------------------------------------------
    Assert-Cycle5Arm -Arm $Arm | Out-Null
    ...
    $genesisManifestPath = Join-Path $RefreshChainDir (Get-Cycle5ChainManifestName -RefreshIndex ([uint64]0))
    ...
    if ($Mode -ceq 'formal') {
        ...
        $advancedGeneration = Get-Cycle5StoreLatestGeneration -StoreRoot $StoreRoot
    ...
    # -------------------------------------------------------------------
    # The run record is DERIVED, not supplied. cycle5_run_record_v1 builds
    # it from the arm kind, the pinned parent Store, and the compiled
    # cycle-5 literals, and refuses to replace a different record already at
    ...
    if ($UseExistingRunRecord) {
        ...
        $result = Invoke-Cycle5Process `
            ...
            -Label 'run-record' `
            -DryRun:$DryRun
        Add-Cycle5CommandRecord -Result $result | Out-Null
    ...
    if (-not (Test-Path -LiteralPath $RunRecord -PathType Leaf)) {
        ...
        Write-Host "CYCLE5 DRY RUN PLANNED arm=$Arm stopped_after=run-record evidence=$root"
        return
    }

    $runRecordDocument = Read-Cycle5Json -Path $RunRecord
    ...
    $inputRecords = [ordered]@{
        run_record = Get-Cycle5FileRecord -Path $RunRecord
        genesis_slot_identities_roster = Get-Cycle5FileRecord -Path $genesisRosterPath
        arm_executable = Get-Cycle5FileRecord -Path $ArmExecutable
        run_record_executable = $(if ($UseExistingRunRecord) { $null } else { Get-Cycle5FileRecord -Path $RunRecordExecutable })
        refresh_builder_executable = Get-Cycle5FileRecord -Path $RefreshBuilderExecutable
        panel_executable = Get-Cycle5FileRecord -Path $PanelExecutable
        python_executable = Get-Cycle5FileRecord -Path $PythonExecutable
        panel_runner = Get-Cycle5FileRecord -Path (Join-Path $PSScriptRoot 'run_payoff_panel_v1.py')
        wrapper = Get-Cycle5FileRecord -Path (Join-Path $PSScriptRoot 'run-cycle5-arm.ps1')
        wrapper_common = Get-Cycle5FileRecord -Path (Join-Path $PSScriptRoot 'common.ps1')
    }
    ...
    if (-not $SkipHostAssertions) {
        $gitRecord = Get-Cycle5GitRecord -RepoRoot $RepoRoot
        ...
        if ($Mode -ceq 'formal' -and -not $DryRun) {
            ...
            $armBuildIdentity = Get-Cycle5ArmBuildIdentity `
                ...
                -WorkingDirectory $RepoRoot
            Add-Cycle5CommandRecord -Result $armBuildIdentity.command | Out-Null
            ...
            $panelBuildIdentity = Assert-Cycle5PanelBuildIdentity `
    ...
    $launchManifest = [ordered]@{
        schema = 'mtg-kernel-cycle5-arm-launch-manifest/v1'
        mode = $Mode
        arm = $Arm
        ...
        refresh_interval = $script:Cycle5RefreshInterval
        panel_games_per_matchup = $script:Cycle5PanelGamesPerMatchup
        ...
        routing_record = Get-Cycle5FileRecord -Path $RoutingRecord
        mailbox_read_receipt = $mailboxReadReceiptRecord
        ...
        parameter_file = $(if ($PSCmdlet.ParameterSetName -ceq 'ParameterFile') { Get-Cycle5FileRecord -Path $ParameterFile } else { $null })
    }
    Write-Cycle5JsonFile -Value $launchManifest -Path (Join-Path $root 'launch-manifest.json')

    $genesisAuthority = Get-Cycle5GenesisAuthorityRecord `
        -Arm $Arm `
    ...
    $genesisAuthorityPath = Join-Path $genesisAuthorityHome "cycle5-genesis-authority-$Arm.json"
    $genesisAuthorityRecord = Assert-OrCreateCycle5GenesisAuthority -Path $genesisAuthorityPath -Record $genesisAuthority
    Write-Cycle5JsonFile -Value $genesisAuthorityRecord -Path (Join-Path $root 'genesis-authority-binding.json')

    # WHICH parent this arm is seeded from is the run record's claim, not the
    # command line's, and the run record itself was bound to the routing
    # record by the builder. Cross-checking them here means a
    # -GenesisParentGeneration that names the wrong generation (anything but
    # the routed 2048) stops the launch before a Store prefix is claimed.
    $genesisParentBinding = Assert-Cycle5GenesisParentBinding `
        -RunRecordDocument $runRecordDocument `
        -GenesisAuthority $genesisAuthority `
        -ParentGeneration $GenesisParentGeneration `
        -RunRecordPath $RunRecord
    Write-Cycle5JsonFile -Value $genesisParentBinding -Path (Join-Path $root 'genesis-parent-binding.json')

    # The genesis boundary's table. Every later boundary rebuilds it for its
    # own refresh index, because slot 3 rotates.
    $slotTable = Get-Cycle5SlotTableForRefresh `
    ...
    #
    # `cycle5_arm_v1 --check-slot-locator` is a read-only, device-free mode
    ...
    foreach ($refresh in $consumedRefreshIndices) {
        foreach ($entry in @(Get-Cycle5SlotTableForRefresh `
                    -SlotStoreRoots $SlotStoreRoots `
                    -HistoricalOneStoreRoots $HistoricalOneStoreRoots `
    ...
    $seenRotations = @()
    foreach ($refresh in $consumedRefreshIndices) {
        $rotation = Get-Cycle5HistoricalOneRotationIndex -RefreshIndex $refresh
    ...
    foreach ($representative in $representatives) {
        ...
        $checkTable = Get-Cycle5SlotTableForRefresh `
            -SlotStoreRoots $SlotStoreRoots `
            -HistoricalOneStoreRoots $HistoricalOneStoreRoots `
            -RefreshIndex $refresh
        $checkLocatorPath = Join-Path $root ('inputs-check-slot-locator-rotation-{0}.json' -f $rotation)
        $inputsCheckLocator = New-Cycle5InputsCheckLocator `
    ...
    $ownRunPlaceholder = [string]@($SlotStoreRoots)[$script:Cycle5ArmOwnedSlotIndex]
    ...
    Write-Cycle5JsonFile -Value ([ordered]@{
        schema = 'mtg-kernel-cycle5-inputs-check-binding/v1'
        through_refresh_index = $ThroughRefreshIndex
    ...
    # ---------------------------------------------------------------------
    function Invoke-Cycle5Bootstrap {
        ...
        if ($null -ne (Get-Cycle5StoreLatestGeneration -StoreRoot $TargetStoreRoot)) {
            return $null
        }
        $locator = Join-Path $Prefix 'bootstrap-slot-locator.json'
        New-Cycle5BootstrapLocator `
            -SlotTable $slotTable `
            -RosterPath $genesisRosterPath `
        ...
        Assert-Cycle5ProcessSucceeded -Result $result | Out-Null
        if (-not $DryRun) {
            Assert-Cycle5ResumePosition -StoreRoot $TargetStoreRoot -ExpectedGeneration ([uint64]0) | Out-Null
        }
        return $result
    }

    function Invoke-Cycle5GenesisManifestBuild {
        ...
        $origin = $null
        if (Test-Path -LiteralPath (Join-Path $TargetChainDir $script:Cycle5ArmOriginRecordFileName) -PathType Leaf) {
            $origin = Read-Cycle5ArmOriginRecord -ChainDir $TargetChainDir
        ...
        $manifest = Read-Cycle5Manifest -Path $OutputManifest
        if ($null -eq $origin) { throw "cannot verify $OutputManifest without an arm-origin record" }
        return Assert-Cycle5GenesisManifestBinding -Manifest $manifest -Origin $origin -ArmStoreRoot $TargetStoreRoot
    }
    ...
    if ($Mode -ceq 'preflight') {
        ...
        if ($window -lt [uint64]1 -or $window -gt $script:Cycle5PreflightMaxUpdates) {
            throw "no admissible preflight window: checkpoint_segment_updates=$checkpointSegmentUpdates needs a window of $window, but the arm bin bounds --preflight-updates to 1..$($script:Cycle5PreflightMaxUpdates)"
        }
        ...
        foreach ($rung in $rungs) {
            ...
            if (Test-Cycle5ArmUsesBaselineChain -Arm $Arm) {
            ...
            $rungManifest = Read-Cycle5Manifest -Path $rung.manifest
            New-Cycle5SlotLocatorPair `
                -SlotTable $slotTable `
        ...
        if ($DryRun) {
            ...
            $manifestA = Get-Cycle5Sha256 -Path $rungs[0].manifest
            $manifestB = Get-Cycle5Sha256 -Path $rungs[1].manifest
            ...
            $endpoints = @()
            foreach ($rung in $rungs) {
                $generation = Get-Cycle5StoreLatestGeneration -StoreRoot $rung.store_root
                if ($generation -ne $window) {
                    throw "preflight rung $($rung.name) stopped at generation $generation, not $window"
                }
                $endpoints += Get-Cycle5CheckpointIdentity -StoreRoot $rung.store_root -StoreGeneration $window
            }
        ...
        $result = [ordered]@{
            schema = 'mtg-kernel-cycle5-preflight-ladder-result/v1'
        ...
        Write-Cycle5JsonFile -Value $result -Path (Join-Path $root 'result.json')
        if ($DryRun) {
            Write-Host "CYCLE5 CONTROL PREFLIGHT DRY RUN PLANNED evidence=$root"
        }
        else {
            Write-Cycle5Marker -Root $root -Name 'PREFLIGHT_COMPLETE' | Out-Null
            Write-Host "CYCLE5 CONTROL PREFLIGHT COMPLETE evidence=$root"
        }
    }
    else {
        # -------------------------------------------------------------------
        ...
        if (Test-Path -LiteralPath $genesisManifestPath -PathType Leaf) {
            if (Test-Path -LiteralPath (Join-Path $ChainDir $script:Cycle5ArmOriginRecordFileName) -PathType Leaf) {
                $genesisBinding = Assert-Cycle5GenesisManifestBinding `
                    -Manifest (Read-Cycle5Manifest -Path $genesisManifestPath) `
                    -Origin (Read-Cycle5ArmOriginRecord -ChainDir $ChainDir) `
                    -ArmStoreRoot $StoreRoot
            }
            elseif (-not $DryRun) {
            ...
            $genesisBinding = Invoke-Cycle5GenesisManifestBuild `
                -Prefix $root `
                -TargetStoreRoot $StoreRoot `
                -TargetChainDir $ChainDir `
        ...
        if ($null -ne $genesisBinding) {
            Write-Cycle5JsonFile -Value $genesisBinding -Path (Join-Path $root 'genesis-binding.json')
        }
        ...
        if ($null -eq $dryRunStoppedAfter) {
            $genesisManifest = Read-Cycle5Manifest -Path $genesisManifestPath
        ...
        $endGeneration = $ThroughRefreshIndex * $script:Cycle5RefreshInterval
        $storeGeneration = Get-Cycle5StoreLatestGeneration -StoreRoot $StoreRoot
        ...
        $journals = Read-Cycle5IntervalJournals -GateRoot $gateRoot
        ...
        for ($candidate = [uint64]0; $candidate -lt $ThroughRefreshIndex; $candidate++) {
            $candidateStop = ($candidate + [uint64]1) * $script:Cycle5RefreshInterval
            $recorded = Get-Cycle5IntervalPhase -Journals $journals -IntervalIndex $candidate
            ...
            if ($Arm -ceq 'static-rb') {
                ...
                $panelDone = Test-Path -LiteralPath (Join-Path $RefreshChainDir (Get-Cycle5ChainPanelName -RefreshIndex ($candidate + [uint64]1))) -PathType Leaf
                $manifestDone = Test-Path -LiteralPath (Join-Path $RefreshChainDir (Get-Cycle5ChainManifestName -RefreshIndex ($candidate + [uint64]1))) -PathType Leaf
        ...
        foreach ($step in $plan) {
            ...
            $manifestPath = Join-Path $RefreshChainDir (Get-Cycle5ChainManifestName -RefreshIndex $refreshIndex)
            ...
            $manifest = Read-Cycle5Manifest -Path $manifestPath
            ...
            if ($Arm -ceq 'static-rb') {
                $advanced = Join-Path $RefreshChainDir (Get-Cycle5ChainManifestName -RefreshIndex ([uint64]1))
            ...
            $intervalSlotTable = Get-Cycle5SlotTableForRefresh `
                -SlotStoreRoots $SlotStoreRoots `
                -HistoricalOneStoreRoots $HistoricalOneStoreRoots `
            ...
            $intervalStart = $interval * $script:Cycle5RefreshInterval
            if (-not $step.train) {
            ...
            Write-Cycle5Phase -IntervalIndex $interval -RefreshIndex $refreshIndex -StopGeneration $stopGeneration -Transition 'training-started'
            ...
            if ($refreshIndex -ge [uint64]1) {
                $boundPanel = Join-Path $RefreshChainDir (Get-Cycle5ChainPanelName -RefreshIndex $refreshIndex)
            ...
            Assert-Cycle5ProcessSucceeded -Result $result | Out-Null
            if (-not $DryRun) {
                Assert-Cycle5ResumePosition -StoreRoot $StoreRoot -ExpectedGeneration $stopGeneration | Out-Null
            }
            Write-Cycle5Phase -IntervalIndex $interval -RefreshIndex $refreshIndex -StopGeneration $stopGeneration -Transition 'training-complete'
            }
            ...
            $panelSeed = $PanelBaseSeed + ($panelIndex * $script:Cycle5PanelSeedStridePerRefresh)
            $panelArguments = @(
                ...
                '--arm', $Arm,
                '--games-per-matchup', [string]$script:Cycle5PanelGamesPerMatchup,
            ...
            Add-Cycle5CommandRecord -Result $result | Out-Null
            Assert-Cycle5ProcessSucceeded -Result $result | Out-Null
            $producedPanel = Join-Path $panelOutputDir (Get-Cycle5ChainPanelName -RefreshIndex ($manifest.refresh_index + [uint64]1))
            ...
            if ($Arm -ceq 'static-rb') {
                ...
                Write-Cycle5Phase -IntervalIndex $interval -RefreshIndex $refreshIndex -StopGeneration $stopGeneration -Transition 'panel-complete'
                continue
            }
            ...
            # authoritative.
            $chainPanel = Join-Path $RefreshChainDir (Get-Cycle5ChainPanelName -RefreshIndex $panelIndex)
            if ($DryRun) {
            ...
            $chainPanel = Join-Path $RefreshChainDir (Get-Cycle5ChainPanelName -RefreshIndex $panelIndex)
            $nextManifest = Join-Path $RefreshChainDir (Get-Cycle5ChainManifestName -RefreshIndex $panelIndex)
            ...
            Assert-Cycle5ProcessSucceeded -Result $result | Out-Null
            if (-not $DryRun) {
                $built = Read-Cycle5Manifest -Path $nextManifest
            ...
            Write-Cycle5Phase -IntervalIndex $interval -RefreshIndex $refreshIndex -StopGeneration $stopGeneration -Transition 'manifest-complete'
        }
        ...
        elseif ($Arm -ceq 'static-rb') {
            Assert-Cycle5ResumePosition -StoreRoot $StoreRoot -ExpectedGeneration $endGeneration | Out-Null
            $advanced = Join-Path $RefreshChainDir (Get-Cycle5ChainManifestName -RefreshIndex ([uint64]1))
            if (Test-Path -LiteralPath $advanced -PathType Leaf) {
                throw "static-rb never advances the manifest past genesis, but $advanced exists"
            }
            for ($index = [uint64]0; $index -lt $ThroughRefreshIndex; $index++) {
                $recorded = Get-Cycle5IntervalPhase -Journals $journals -IntervalIndex $index
            ...
            for ($index = [uint64]1; $index -le $ThroughRefreshIndex; $index++) {
                $linkPanel = Join-Path $RefreshChainDir (Get-Cycle5ChainPanelName -RefreshIndex $index)
                $linkManifest = Join-Path $RefreshChainDir (Get-Cycle5ChainManifestName -RefreshIndex $index)
                foreach ($required in @($linkPanel, $linkManifest)) {
                    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
                        throw "refusing to publish TRAINING_COMPLETE: $required is missing"
                    }
                }
                $link = Read-Cycle5Manifest -Path $linkManifest
                ...
                $links += [ordered]@{
                    ...
                    panel_sha256 = (Get-Cycle5Sha256 -Path $linkPanel)
                }
            }
            $chainVerification = [ordered]@{
                arm = $Arm
        ...
        $trainingResult = [ordered]@{
            ...
            final_store_generation = $(if ($DryRun) { $null } else { Get-Cycle5StoreLatestGeneration -StoreRoot $StoreRoot })
            arm_origin_record = $(if ($DryRun -or -not (Test-Path -LiteralPath (Join-Path $ChainDir $script:Cycle5ArmOriginRecordFileName) -PathType Leaf)) { $null } else { Get-Cycle5FileRecord -Path (Join-Path $ChainDir $script:Cycle5ArmOriginRecordFileName) })
            store_mode_marker = $(if ($DryRun -or -not (Test-Path -LiteralPath (Join-Path (Split-Path -Parent $StoreRoot) $script:Cycle5ModeMarkerFileName) -PathType Leaf)) { $null } else { Get-Cycle5FileRecord -Path (Join-Path (Split-Path -Parent $StoreRoot) $script:Cycle5ModeMarkerFileName) })
            command_log = $commandLog
            nonclaim = 'Training completion is not playing-strength evidence; the derived metric is the payoff panel and its BT ratings.'
        }
        Write-Cycle5JsonFile -Value $trainingResult -Path (Join-Path $root 'result.json')
        if ($DryRun) {
            Write-Host "CYCLE5 DRY RUN PLANNED arm=$Arm evidence=$root"
        }
        else {
            Write-Cycle5Marker -Root $root -Name 'TRAINING_COMPLETE' | Out-Null
            Write-Host "CYCLE5 ARM TRAINING COMPLETE arm=$Arm evidence=$root"
        }
    }
}
catch {
    ...
    Write-Cycle5RunFailed -Root $root -Phase $phase -Message $failureMessage | Out-Null
