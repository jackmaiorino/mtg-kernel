...
pub fn matmul<R: CubeRuntime>(
    ...
    match strategy {
        MatmulStrategy::Cube => {
            ...
            // QUALIFICATION FORK (cell zero): force the SimpleUnit register
            // f32 route instead of the Auto selector's CMMA-first order, so
            // no matmul operand is truncated to TF32. Surrounding stock CUDA
            // math (tanh, reductions, scatter, bias order) is unchanged.
            launch_matmul(
