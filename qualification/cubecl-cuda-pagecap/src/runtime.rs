...
impl DeviceService for CudaServer {
    fn init(device_id: cubecl_common::device::DeviceId) -> Self {
        ...
        let mem_properties = MemoryDeviceProperties {
            max_page_size: capped_page_size_v1(max_memory),
...
/// mtg-kernel qualification patch (docs/multirun_qualified_launcher_v1.md,
/// "Binding constraint"): an opt-in, per-process cap on the memory pool page
/// size. Upstream sizes the largest page at a quarter of device memory, so an
/// allocation above the largest sub-slice pool reserves 3 GB on a 12 GB card
/// even for a small model, which caps how many training processes fit per
/// GPU. `MTG_KERNEL_CUBECL_MAX_PAGE_MIB` (a positive integer) lowers the page
/// size to at most that many MiB; unset, zero or unparsable means exactly the
/// upstream value. Only allocation bookkeeping changes: no kernel, launch
/// shape or numerical path reads the page size.
fn capped_page_size_v1(max_memory: u64) -> u64 {
    let upstream = max_memory / 4;
    match std::env::var("MTG_KERNEL_CUBECL_MAX_PAGE_MIB")
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
    {
        Some(mib) if mib > 0 => upstream.min(mib.saturating_mul(1024 * 1024)),
        _ => upstream,
    }
}
